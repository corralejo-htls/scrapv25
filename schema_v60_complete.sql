-- =============================================================================
-- BookingScraper Pro — Schema Completo v6.0.0 build 60
-- PostgreSQL 14+ / Windows 11 Single-Node Deployment
-- =============================================================================
--
-- INSTRUCCIONES DE EJECUCIÓN (Windows 11):
--   psql -U postgres -f schema_v60_complete.sql
--   O desde pgAdmin 4: Tools > Query Tool > Ejecutar este archivo
--
-- CAMBIOS v60 respecto a v59:
--
--   BUG-DB-002-FIX  : scraper_service.py — _upsert_legal() refactorizado.
--                     SÍNTOMA: 4 URLs con 6/6 idiomas en hotels pero 0 registros
--                              en hotels_legal (hoteles BR/UY en Booking.com).
--                     CAUSA:   `if legal: self._upsert_legal(...)` — llamada
--                              condicional que silenciaba la ausencia de sección
--                              legal sin insertar ningún registro ni trazar log.
...
--                     Índice diagnóstico: ix_hlegal_has_content.
--
--   MODEL-002       : models.py — HotelLegal.has_legal_content añadido.
--
--   BUILD-SYNC-002  : Todos los encabezados Python y SQL sincronizados a build 60.
--
-- NOTA: La base de datos se elimina y recrea en cada arranque del sistema.
--       Este archivo es para instalación limpia, NUNCA para migración.
--
-- CAMBIOS v59 respecto a v58:
--
--   FIX-FP-CONTENT-001   : extractor.py — _extract_fine_print() corregido.
--                          ANTES: almacenaba el HTML completo del bloque
--                                 incluyendo wrappers React (<div><section>...).
--                          AHORA: extrae solo el contenido entre <p>...</p>
--                                 desde el contenedor interno div.c85a1d1c49.
--                          No requiere cambios en SQL — tabla hotels_fine_print
--                          sin cambios de estructura.
--
--   FIX-PH-SELECTOR-001  : extractor.py — _extract_property_highlights()
--                          Selector real DOM añadido como Estrategia 0:
--                            #hp_facilities_box > section >
--                            div[data-testid=property-section--content] >
--                            div.e43cb5a00e
--                          Validado contra HTML en pruebas/
--                          _HTML-view-source__manaus-hoteis-millennium_*.md
--
--   FIX-PH-STRUCTURE-001 : hotels_property_highlights — columnas rediseñadas.
--                          ANTES (v57-v58): columna 'highlight' VARCHAR(512).
--                          AHORA (v59): 2 columnas:
--                            highlight_category VARCHAR(256) — nombre del grupo
--                            highlight_detail   VARCHAR(512) — ítem individual
--                          UniqueConstraint actualizado:
--                            uq_hph_hotel_lang_cat_detail
--                            (hotel_id, language, highlight_category, highlight_detail)
--                          Índice adicional: ix_hph_category (highlight_category)
--                          Vista v_hotels_full actualizada: highlights como JSONB array
--                          con pares {category, detail}.
--
--   HIGH-2-LOG-001       : scraper_service.py — _process_url() añade log WARNING
--                          cuando lang_results y actual_count divergen.
--                          No requiere cambios en SQL.
--
--   BUILD-SYNC-001       : Todos los encabezados de archivos Python y SQL
--                          sincronizados a build 59. BUILD_VERSION = 59.
--
-- NOTA: La base de datos se elimina y recrea en cada arranque del sistema.
--       Este archivo es para instalación limpia, NUNCA para migración.
--
-- CAMBIOS v55 respecto a v54:
-- CAMBIOS v57 respecto a v56:
--
--   BUG-FP-SELECTOR-001  : Corregido en extractor.py — selectores de _extract_fine_print()
--                          actualizados al DOM real de Booking.com.
--                          Selector primario: data-testid="PropertyFinePrintDesktop-wrapper"
--                          Selector secundario: id="important_info"
--                          Validado contra HTML en pruebas/_HTML-view-source__*__.md
--                          No requiere cambios en SQL — tabla hotels_fine_print sin cambios.
--
--   BUG-PH-NORMALIZATION-001: hotels_property_highlights reestructurada.
--                          ANTES (v53-v55): 1 fila/hotel/idioma con HTML blob, pk UUID.
--                          AHORA (v57): N filas/hotel/idioma, texto plano, pk BIGSERIAL.
--                          UniqueConstraint: (hotel_id, language, highlight).
--                          Vista v_hotels_full actualizada: highlights como JSONB array
--                          con pares {category, detail} (FIX-PH-STRUCTURE-001, v59).
--
--   BUG-VPN-IMPORT-001   : Corregido en vpn_manager_windows.py — import añadido.
--                          No requiere cambios en SQL.
--
--   BUG-TEST-METHOD-001  : Corregido en vpn_manager_windows.py — método try_acquire()
--                          añadido a VPNCircuitBreaker.
--                          No requiere cambios en SQL.
--
--   BUG-TEST-VERSION-001 : Corregido en tests/test_config.py — BUILD_VERSION 48→56.
--   BUG-TEST-VERSION-002 : Corregido en tests/test_models.py — BUILD_VERSION 49→56.
--
--
--   BUG-EXTR-010 : Corregido en extractor.py — selectores de _extract_guest_reviews()
--                  actualizados a data-testid="review-subscore". La tabla
--                  hotels_guest_reviews ahora se puebla correctamente.
--                  No requiere cambios en SQL — estructura de tabla sin cambios.
--
--   FIX-LEGAL-003: Corregido en extractor.py — validación post-extracción para
--                  detectar legal == legal_info y limpiar el duplicado.
--                  No requiere corrección SQL ya que la DB se recrea siempre.
--
--   BUG-LANG-001 : Corregido en scraper_service.py — 'en' siempre procesado primero.
--
--   BUG-IMG-UNPACK: Corregido en scraper_service.py — download_photo_batch()
--                   retorna Dict[str, int], no tuple.
--
--   CLEANUP-001  : Eliminado bloque FIX-LEGAL-002 de corrección de datos v54
--                  (innecesario: la DB se recrea en cada arranque).
--
-- NOTA: La base de datos se elimina y recrea en cada arranque del sistema.
--       Este archivo es para instalación limpia, NUNCA para migración.
--
...
--
--   STRUCT-015 : hotels_faqs — nueva tabla. BUG-FAQ-ANSWERS (v56): columna answer añadida.
...
--   4.  hotels_policies              — Políticas del hotel (STRUCT-006, v51)
...
--  17.  hotels_property_highlights — Highlights normalizados, 1 fila/highlight (STRUCT-017, v57 fix)
--
-- VISTAS:
--   v_hotels_full      — Hotel completo denormalizado (con popular_services, legal, etc.)
...
COMMENT ON TABLE  url_queue IS 'Cola de URLs a scrapeear con estado y prioridad (v58 — Strategy E)';
...
COMMENT ON TABLE  hotels IS 'Datos principales del hotel por idioma (v55)';
...
-- =============================================================================
-- 6. HOTELS_LEGAL (STRUCT-007, v51 / FIX-LEGAL-001, v52 / BUG-DB-002-FIX, v60)
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS hotels_legal (
    ...
    -- BUG-DB-002-FIX (v60): campo diagnóstico — distingue ausencia de sección
    -- legal en HTML (FALSE) de presencia con contenido real (TRUE).
    -- Antes de v60 la ausencia producía 0 registros (fallo silencioso).
    -- Ahora siempre existe 1 registro por hotel/idioma.
...
-- BUG-DB-002-FIX (v60): índice para consultas de auditoría de cobertura legal
CREATE INDEX IF NOT EXISTS ix_hlegal_has_content ON hotels_legal (has_legal_content);

COMMENT ON TABLE  hotels_legal IS
    'Información legal del alojamiento por idioma (STRUCT-007, v51). FIX-LEGAL-001 en extractor.py (v52). BUG-DB-002-FIX v60: inserción incondicional con has_legal_content.';
...
-- Particiones 2027 (PARTITION-2027, v54)
...
-- =============================================================================
-- Almacena TODOS los servicios/instalaciones del hotel.
-- Una fila por servicio — complementa hotels_popular_services
...
COMMENT ON TABLE  hotels_faqs IS
    'Preguntas frecuentes del hotel — una fila por pregunta (STRUCT-015, v53). BUG-FAQ-ANSWERS fix (v56): columna answer añadida.';
...
COMMENT ON COLUMN hotels_faqs.answer IS
    'Texto de la respuesta a la pregunta (extraído del accordion. NULL si no disponible en DOM)';
...
-- BUG-PH-NORMALIZATION-001 FIX (v57): Estructura normalizada.
-- Un registro por highlight individual por hotel/idioma.
-- Patrón idéntico a hotels_popular_services (BIGSERIAL pk, VARCHAR highlight).
-- UniqueConstraint: (hotel_id, language, highlight) — evita duplicados por re-scrape.
CREATE TABLE IF NOT EXISTS hotels_property_highlights (
    id                 BIGSERIAL    NOT NULL,
    hotel_id           UUID         NOT NULL,
    url_id             UUID         NOT NULL,
    language           VARCHAR(10)  NOT NULL,
    -- FIX-PH-STRUCTURE-001 (v59): estructura categoría/detalle
    -- highlight_category: nombre del grupo (e.g. "Ideal para tu estancia", "Baño")
    highlight_category VARCHAR(256) NOT NULL DEFAULT '',
    -- highlight_detail: ítem individual del grupo (e.g. "Baño privado", "Parking")
...
COMMENT ON COLUMN hotels_property_highlights.highlight_category IS
    'Nombre del grupo de highlight extraído del DOM de Booking.com (e.g. "Ideal para tu estancia")';
...
-- BUG-PH-NORMALIZATION-001 FIX (v57): Trigger updated_at eliminado.
-- hotels_property_highlights ya no tiene columna updated_at (filas inmutables).
...
SELECT
    ...
    -- Legal (BUG-DB-002-FIX v60: has_legal_content añadido)
    jsonb_build_object(
        'legal',             hl.legal,
...
-- BUG-PH-NORMALIZATION-001 FIX (v57): LEFT JOIN hph eliminado.
-- highlights ahora se obtiene via subquery en el SELECT (array_agg).
LEFT JOIN hotels_legal hl
       ON hl.hotel_id = h.id AND hl.language = h.language;

COMMENT ON VIEW v_hotels_full IS
    'Vista denormalizada de hotel con description, fine_print, highlights (jsonb array de {category,detail}), '
    'all_services, faqs, popular_services, policies, guest_reviews, legal (con has_legal_content) y url_queue data (v65)';
...
-- =============================================================================
-- NOTA v55: Bloque FIX-LEGAL-002 eliminado (CLEANUP-001).
-- La corrección de datos no es necesaria porque la base de datos se recrea
-- en cada arranque. El bug de legal == legal_info está corregido en
-- extractor.py (FIX-LEGAL-003) y no se reproducirá en datos nuevos.
-- =============================================================================
...
ORDER BY table_name;
-- Esperado: 16 filas (v65: hotels_amenities eliminada)
