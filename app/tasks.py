...
def _disk_usage_percent() -> float:
    ...
    [FIX MED-40-005] disk_path initialised before the try block so the except
    handler can safely reference it even if settings.BASE_DATA_PATH raises.
    """
    disk_path = "C:\\"  # safe default; overwritten below
    try:
...
def reset_stale_urls(self, stale_minutes: int = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if reset_ids:
            # [FIX CRIT-40-002] SQLAlchemy text() with :param does NOT support
            # Python tuples for IN clause — PostgreSQL interprets them as row
            # constructors, raising "operator does not exist: integer = record".
            # Correct approach: use sqlalchemy.bindparam with expanding=True,
            # which renders IN (:ids_0, :ids_1, ...) at execution time.
