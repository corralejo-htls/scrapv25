"""
tasks.py — BookingScraper Pro v6.0.0 build 58
Fixes applied:
...
def reset_stale_processing_urls(self, stale_minutes: int = 60) -> dict:
    ...
    FIX-ERR-001 (v58): language tracking fields are now cleared on reset.
    Previously, languages_completed and languages_failed retained stale values
    from the interrupted run, causing partial retry logic to skip languages
    that were never actually committed to the database.

    Fields reset:
      - status              → 'pending'         (worker can re-claim)
      - last_error          → descriptive msg   (audit trail)
      - languages_completed → ''               (no languages confirmed done)
      - languages_failed    → ''               (no languages confirmed failed)
    ...
    with get_db() as session:
        ...
        for url in stale_urls:
            ...
            # FIX-ERR-001: clear partial language tracking so retry starts clean
            url.languages_completed = ""
            url.languages_failed    = ""
            url.updated_at = datetime.now(timezone.utc)
            reset_count += 1

    logger.info("Reset %d stale processing URLs (language tracking cleared).", reset_count)
