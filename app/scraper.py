# =============================================================================
# app/scraper.py — BookingScraper Pro v6.0.0 Build 125
# =============================================================================
...
class SeleniumEngine:
    ...
    def _build_chrome_options(self) -> webdriver.ChromeOptions:
        ...
        options.add_argument(
            ...
            # FINDING-D-FIX (Build 126): este literal es ahora solo un FALLBACK.
            # _get_driver() lo sobre-escribe dinámicamente vía CDP con la versión
            # MAYOR real del driver (Network.setUserAgentOverride), eliminando la
            # recurrencia de BUG-USER-AGENT-STALE-001 en cada auto-update de Brave.
    ...
    def _get_driver(self) -> webdriver.Chrome:
        ...
        if self._driver is None:
            ...
            # ── FINDING-001-FIX (Build 125): timeout a NIVEL DE COMANDO ───────
            # set_page_load_timeout() / NAVIGATION_TIMEOUT_S solo cubren
            # driver.get() (la navegación). NO cubren find_element/find_elements,
            # click(), execute_script(), current_url ni la interacción del modal
            # de galería (GalleryModalExtractor). Cada uno de esos comandos envía
            # una petición HTTP a ChromeDriver y, si el pipe IPC ChromeDriver↔Brave
            # se atasca en Windows 11 (confirmado por RemoteDisconnected en logs),
            # bloquea la lectura del socket SIN LÍMITE → la tarea queda viva hasta
            # que el watchdog dispara os._exit(1) a los 600 s (5 kills en la
            # sesión auditada). Como Chrome()/get() ya completaron, sus timeouts
            # no aplican. Aquí fijamos un timeout de lectura del executor remoto
            # que cubre TODOS los comandos posteriores.
            try:
                ...
                logger.info(
                    "Selenium: command timeout set to %ss (FINDING-001-FIX)",
                    _cmd_timeout,
                )
            except Exception:
                try:
                    # Fallback para versiones donde vive en _client_config
                    self._driver.command_executor._client_config.timeout = _cmd_timeout
                    logger.info(
                        "Selenium: command timeout set via _client_config to %ss",
                        _cmd_timeout,
                    )
                except Exception:
                    logger.warning(
                        "FINDING-001-FIX: no se pudo fijar el command timeout "
                        "(la versión de Selenium no expone command_executor.set_timeout "
                        "ni _client_config.timeout); comandos Selenium quedan sin "
                        "timeout de socket."
                    )
            ...
            # ── FINDING-D-FIX (Build 126): User-Agent DINÁMICO vía CDP ────────
            # El UA estático de _build_chrome_options() (Chrome/149.0.0.0) vuelve
            # a desincronizarse en cuanto webdriver-manager actualiza Brave/
            # ChromeDriver a 150+ (BUG-USER-AGENT-STALE-001 recurrente: Build 123
            # solo subió el literal 131→149). Un desfase entre el UA del header y
            # la versión real reportada por CDP es una señal fuerte de bot para
            # Cloudflare. Aquí derivamos la versión MAYOR real del driver ya
            # creado y la inyectamos con Network.setUserAgentOverride, de modo que
            # el UA siempre coincide con el binario en uso, sin mantenimiento.
            try:
                _bv = self._driver.capabilities.get("browserVersion") \
                    or self._driver.capabilities.get("version") or "149"
                _major = str(_bv).split(".")[0]
                _dyn_ua = (
                    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
                    "AppleWebKit/537.36 (KHTML, like Gecko) "
                    f"Chrome/{_major}.0.0.0 Safari/537.36"
                )
                self._driver.execute_cdp_cmd(
                    "Network.setUserAgentOverride", {"userAgent": _dyn_ua}
                )
                logger.info(
                    "FINDING-D-FIX: User-Agent dinámico fijado vía CDP "
                    "(browserVersion=%s → Chrome/%s.0.0.0)", _bv, _major,
                )
            except Exception as _ua_exc:
                # No fatal: el UA estático de las opciones sigue activo como
                # fallback. Solo perdemos la sincronización automática.
                logger.warning(
                    "FINDING-D-FIX: no se pudo fijar UA dinámico vía CDP (%s) — "
                    "se mantiene el UA estático de _build_chrome_options().",
                    _ua_exc,
                )
    ...
    def _fetch_with_selenium(
        ...
        with self._lock:
            ...
            # ── FINDING-005-FIX (Build 125): carga parcial sin núcleo JSON-LD ─
            # Cuando 'loaded' es False llegamos aquí por la rama "proceeding with
            # partial load" (timeout de contenido sin indicador de challenge).
            # Si esa carga parcial NO contiene el bloque JSON-LD del hotel
            # (núcleo de name/address/geo/rating del que dependen todas las
            # tablas), persistir produciría una ficha casi vacía que se podría
            # marcar como 'done', contaminando la métrica de completitud de
            # Strategy E (caso 'minihotel-graz': 30× partial load + RoomTypes
            # vacío pese a que rt-name-link SÍ existe en el HTML completo → la
            # tabla de habitaciones, renderizada por JS, no estaba en la carga
            # parcial). Tratamos la lengua como fallo limpio para que se reintente.
            if not loaded:
                _html_low = html.lower()
                _has_core = (
                    "application/ld+json" in _html_low
                    and ('"hotel"' in _html_low or '"lodgingbusiness"' in _html_low)
                )
                if not _has_core:
                    logger.warning(
                        "FINDING-005-FIX: carga parcial SIN bloque JSON-LD de "
                        "hotel (attempt %d/%d) para %s — se descarta para que "
                        "Strategy E reintente la lengua en vez de persistir una "
                        "ficha pobre.",
                        attempt, max_retries, url,
                    )
                    _save_debug_html(html, url, f"partial_no_jsonld_a{attempt}")
                    return None
