...
class SeleniumEngine:
    ...
    def _expand_facilities(self, driver: webdriver.Chrome) -> bool:
        ...
        # Modern Booking.com renders facility-group-container items via an
        # IntersectionObserver that fires when the container scrolls into the
        # viewport.  No "See all" button exists in this layout variant.
        # Steps:
        #   4a. Locate the property-facilities-block-container.
        #   4b. Scroll it into view (triggers IntersectionObserver).
        #   4c. Dispatch a synthetic scroll event as a secondary trigger.
        #   4d. Wait up to fac_lazy_wait_s for facility-group-container.
        try:
            fac_container = driver.find_element(
                By.CSS_SELECTOR,
                "[data-testid='property-facilities-block-container']",
            )
            # Scroll to element and trigger IntersectionObserver
            driver.execute_script(
                "arguments[0].scrollIntoView({behavior: 'instant', block: 'center'});",
                fac_container,
            )
            # Dispatch scroll on window to help IntersectionObserver fire
            driver.execute_script("window.dispatchEvent(new Event('scroll'));")
            # Small settle pause for JS microtask queue
            import time as _time
            _time.sleep(0.5)

            WebDriverWait(driver, fac_lazy_wait_s).until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, "[data-testid='facility-group-container']")
                )
            )
            ...
            logger.info(
                "BUG-SVC-LAZY-001-FIX: facilities lazy-loaded via scroll trigger "
    ...
    def _fetch_with_selenium(
        ...
        with self._lock:
            ...
            # ── BUG-SVC-LAZY-001-FIX: Pre-scroll facilities into viewport ────
            # Booking.com uses IntersectionObserver to lazy-render the
            # property-facilities-block-container.  Scrolling to the element
            # BEFORE _expand_facilities() triggers the observer so facility-
            # group-container children are injected into the DOM.
            # Must happen AFTER window.stop() (which halts navigation) but
            # before _expand_facilities() reads the DOM state.
            try:
                ...
                driver.execute_script("window.dispatchEvent(new Event('scroll'));")
                logger.debug(
                    "BUG-SVC-LAZY-001-FIX: pre-scrolled facilities container into view"
                )
