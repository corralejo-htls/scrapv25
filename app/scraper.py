# =============================================================================
# app/scraper.py — BookingScraper Pro v6.0.0 Build 64
# =============================================================================
# BUILD 64 — BUG-PERF-001 FIX
#   Root cause: WebDriverWait iterated 4 CSS selectors × SCRAPER_REQUEST_TIMEOUT
#   (30 s) each → worst case 120 s per language pass when no selector matched.
#   Observed: it=144.0 s and it=124.6 s (2026-04-01 run, 15.4% Italian outliers).
#
#   Fix 1: SELENIUM_CONTENT_WAIT_TIMEOUT_S (new cfg field, default 10 s)
#     Per-selector WebDriverWait now uses content_wait_s (10 s) instead of
#     page_timeout (30 s) → worst case 4 × 10 s = 40 s (was 120 s).
#   Fix 2: driver.set_page_load_timeout(page_timeout)
#     Added before driver.get() to cap total navigation time.
#     Previously absent — driver.get() could block indefinitely on slow pages.
#   Fix 3: LANG_SCRAPE_DELAY_IT (new cfg field, default 5.0 s)
#     Extra pre-scrape pause for Italian only, applied BEFORE acquiring _lock.
#     Italian avg 73.7 s vs 49-64 s for other languages (stddev 29.5 s).
#
# BUILD 63 — CLOUDSCRAPER ELIMINATED
#   Root cause: CloudScraper had 0% success rate across 78+ attempts (logs
#   2026-03-29/30). Every request returned a Cloudflare challenge page
#   (3962 bytes). Keeping dead code that actively degrades the system
#   (wastes retries, reinforces IP blocks) violates SRP and clean-code
#   principles. SeleniumEngine is now the sole scraping engine.
#
#   Lines removed:  ~250  (entire CloudScraperEngine class + imports)
#   Lines kept:      all  (SeleniumEngine, utilities)
#
# BUILD 62 — BUG-LANG-002 FIX
#   _set_language_headers(): CDP Accept-Language injection + cookie wipe
#   reset_browser(): clean driver after VPN rotation
#
# BUILD 61 — BUG-LANG-001 FIX
#   Short HTML fast-fail: return None immediately on <SHORT_HTML_THRESHOLD bytes
# =============================================================================
