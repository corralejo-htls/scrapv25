...
# [FIX HIGH-40-001] DB_PASSWORD validation REMOVED from module-level.
# Previously, raising EnvironmentError here crashed alembic, pytest, and
# celery inspect on import — before any application context existed.
# Validation now occurs inside _build_database_url() which is called only
# when the engine is actually created (application startup path), not on
# every import.
def _build_database_url() -> str:
    """
    Construct DATABASE_URL from environment, validating credentials at
    call time (not at import time). Raises EnvironmentError only when
    a live DB connection is actually being established.
    """
    ...
    name = os.getenv("DB_NAME", "booking_scraper")
...
def get_olap_db():
    ...
    """
    # [FIX HIGH-40-005] Use validated integer, not f-string interpolation.
    # SET LOCAL does not support parameterized binding (:param syntax).
    # int() conversion prevents injection; unquoted integer form is canonical.
    _olap_timeout = max(1000, min(int(os.getenv("STMT_TIMEOUT_OLAP_MS", "300000")), 3_600_000))
    db = SessionLocal()
    try:
        db.execute(text("SET LOCAL statement_timeout = " + str(_olap_timeout)))
        yield db
        db.commit()
    except Exception:
        db.rollback()
