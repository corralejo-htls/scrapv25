...
-- Version: v43 (Audit v37 + Execution Errors v41/v42 Fixed 2026-03-06)
--
-- TESTED ENVIRONMENT (confirmed working):
--   OS          : Windows 11 x64
--   PostgreSQL  : 18.1  (x86_64-windows, msvc-19.44.35221)
--   psql        : 18.1
...
--   PostgreSQL (not even via EXECUTE). Uses psql \if / \gset metacommands
...
--   calls are wrapped inside DO $$ BEGIN ... END $$.
--
...
--
-- [FIX NEW-EXEC-004] pg_stat_statements extension creates two system views
--   (pg_stat_statements, pg_stat_statements_info) in the public schema.
--   The view count query previously included them, causing a false 4/2 mismatch.
--   Fix: count only application views by excluding extension-owned objects
--   using pg_depend (deptype = 'e').
--
-- CANONICAL FILE LOCATION (relative to project root C:\BookingScraper):
--   app/install_clean_v43.sql
--
-- EXECUTION METHOD (Windows 11):
--
--   STEP 1 - From postgres database (drops and creates booking_scraper):
--     psql -U postgres -f app/install_clean_v43.sql
--
--   STEP 2 - Only if Step 1 aborted at \connect (rare on PG18):
...
--   5.  Indexes
--   6.  Triggers
--   7.  Views (DROP IF EXISTS before each CREATE)
--   7b. Schema version tracking
--   8.  Grants
--   9.  Post-install verification (extension views excluded from count)
-- =============================================================================
...
COMMENT ON DATABASE booking_scraper IS
    'BookingScraper Pro - hotel data extraction. Created v43 2026-03-06.';
...
BEGIN
    RAISE EXCEPTION
        ...
        '  Step 1: psql -U postgres -f app/install_clean_v43.sql\n'
        '  Step 2: psql -U postgres -d booking_scraper -f app/install_clean_v43.sql',
        current_database();
END $$;
...
VALUES (
    'v43.0.0-clean-install',
    'BookingScraper Pro v43 - extension views excluded from count, versions confirmed 2026-03-06',
    md5('v43.0.0-clean-install-2026-03-06')
...
BEGIN
    ...
    -- [FIX NEW-EXEC-004] Count only APPLICATION views in public schema.
    -- pg_stat_statements extension installs two system views into the schema
    -- where the extension is created (pg_stat_statements, pg_stat_statements_info).
    -- information_schema.views includes them, causing a false 4/2 mismatch.
    --
    -- Fix: exclude views that have an extension dependency (deptype = 'e').
    -- pg_depend.deptype = 'e' means the object is owned by an extension and
    -- will be dropped automatically when the extension is dropped.
    -- Application views have no such dependency record.
    ...
    RAISE NOTICE '=====================================================';
    RAISE NOTICE '  BookingScraper Pro v43 - Install Verification';
    RAISE NOTICE '=====================================================';
...
DO $$ BEGIN RAISE NOTICE '  BookingScraper Pro v43 is ready.'; END $$;
