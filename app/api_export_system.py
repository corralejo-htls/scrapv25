"""
api_export_system.py — BookingScraper Pro v6.0.0 Build 88
==========================================================
...
from app.models import Hotel, URLQueue
...
class ExportSelection:
    ...
    @classmethod
    def from_db_all_pending(cls, db: Session) -> "ExportSelection":
        rows = db.query(URLQueue).filter(URLQueue.status == "done").all()
        sel = cls(description="All done URLs from database")
        for r in rows:
            sel.add_url_id(r.id)
        return sel
    ...
    def _send(self, hotel_id: str, payload: Dict[str, Any]) -> int:
        ...
        for attempt in range(1, self.config.retry_attempts + 1):
            try:
                ...
                if resp.ok:
                    logger.info(
                        "API PATCH hotel_id=%s → %d (attempt %d)",
                        hotel_id, last_status, attempt,
                    )
                    return last_status
                logger.warning(
