"""
export_ui.py — BookingScraper Pro v6.0.0 Build 127
...
<p class="sub">Visualizaci&oacute;n y exportaci&oacute;n de datos a la API externa &middot; v6.0.0 Build 94</p>
...
const FIELD_DEFS=[
  ...
  {key:"roomsQuantity",cov:69,on:false,can:true},{key:"rooms",cov:69,on:false,can:true},
...
function renderPreviewResults(data){
  ...
  // Store full payloads for download
  items.forEach(item=>{if(item.payload)_payloads[item.external_ref]=item.payload;});
  el.innerHTML=items.map(item=>{
    ...
    return`<div class="prev-hotel">${head}<div style="padding:4px 12px 2px;font-size:11px;color:#6b7280">idiomas en payload: <b>${esc(locales)}</b></div><div class="prev-fields-grid">${fieldsHtml}</div>${json?`<details style="padding:4px 12px 8px" id="det-${esc(item.external_ref)}"><summary style="font-size:11px;cursor:pointer;color:#6b7280;display:flex;align-items:center;gap:10px"><span>Ver JSON</span><button onclick="event.preventDefault();event.stopPropagation();downloadJSON('${esc(item.external_ref)}','${esc(item.hotel_name||item.external_ref)}')" style="font-size:11px;padding:2px 10px;border:1px solid #d1d5db;border-radius:5px;background:#fff;cursor:pointer;color:#1a1d23;font-family:inherit">Descargar JSON</button></summary><div class="json-box" id="json-${esc(item.external_ref)}">${esc(json)}</div></details>`:""}</div>`;
  }).join("");
}
...
function downloadJSON(ref,name){
  const raw=_payloads[ref];
  if(!raw){alert('Payload no disponible para '+ref);return;}
  const blob=new Blob([JSON.stringify(raw,null,2)],{type:'application/json'});
  const a=document.createElement('a');
  a.href=URL.createObjectURL(blob);
  const safeName=String(name||ref).replace(/[^a-z0-9_\-]/gi,'_').toLowerCase();
  a.download='payload_'+safeName+'_'+ref+'.json';
