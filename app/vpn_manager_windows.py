"""
vpn_manager_windows.py — BookingScraper Pro v6.0.0 Build 124
# BUILD 124 — BUG-VPN-LOCK-DEADLOCK-001-FIX
#   Causa raíz: rotate() usaba "with self._lock:" (bloqueo indefinido).
#   _rotate_vpn_with_timeout() lanzaba un daemon thread para llamar rotate()
#   con join(VPN_ROTATE_TIMEOUT_S=90s). Si el thread superaba el timeout,
#   join() retornaba pero el daemon thread continuaba vivo sosteniendo self._lock.
#   Toda rotación posterior intentaba adquirir el mismo lock → bloqueo eterno
#   → watchdog disparaba os._exit(1) 600s después. Evidencia: 6 watchdog kills
#   en la sesión 2026-06-14 (03:25, 05:11, 06:28, 07:53, 08:58, 10:22).
#   Logs confirmaban "posible deadlock en rotación VPN" como causa registrada.
#
#   Fix: sustituir "with self._lock:" por lock.acquire(timeout=N) con try/finally.
#   Si el lock no se puede adquirir en VPN_LOCK_ACQUIRE_TIMEOUT_S (default=80s),
#   rotate() retorna False inmediatamente sin bloquear. Esto evita que un zombie
#   daemon thread con el lock sostenido cause deadlock en el ciclo siguiente.
#   El timeout de adquisición se fija por debajo de VPN_ROTATE_TIMEOUT_S (90s)
#   para garantizar que el wrapper externo detecte primero el fallo.
#
# BUILD 118 — BUG-VPN-001-FIX (sub-defecto: Unknown-IP como fallo de CB)
#   Causa raíz: _connect_via_cli() devolvía False (→ record_failure()) cuando
#   get_current_ip() retornaba "Unknown", incluso si el CLI de NordVPN confirmó
#   la conexión con rc=0 / "connected" en stdout. Esto hacía que un timeout DNS
#   transitorio en el servicio de verificación IP (api.ipify.org, etc.) disparase
#   el circuit breaker exactamente igual que un fallo VPN real.
#
#   Evidencia (logs_Celery_Worker.csv Build 115):
#     Línea 360: "VPN: get_current_ip deadline reached — using Unknown"
#     Línea 556: "VPN: get_current_ip deadline reached — using Unknown"
#     Línea 567: "VPN: get_current_ip deadline reached — using Unknown"
#     → 3 Unknowns consecutivos → record_failure() x3 → CB abierto (línea 570)
#     Línea 3614: "VPN CLI connected but IP unchanged (current=Unknown)" →
#     NordVPN conectó realmente (rc=0), pero Unknown se contaba como fallo.
#
#   Fix A — _connect_via_cli(): nueva rama para new_ip == "Unknown" cuando
#     el CLI confirmó la conexión (rc=0 / "connected"). Si
#     VPN_CB_UNKNOWN_IP_AS_SUCCESS=true (default), se loguea WARNING pero
#     se retorna True (→ record_success()). Esto evita que timeouts del
#     servicio de verificación IP disparen el circuit breaker.
#     Si VPN_CB_UNKNOWN_IP_AS_SUCCESS=false, comportamiento anterior.
#
#   Fix B — VPNCircuitBreaker: cooldown_seconds ahora lee
#     VPN_CB_COOLDOWN_S desde config (default 300s). Permite ajuste
#     operacional sin cambiar código.
#
#   Fix C — NordVPNManager.__init__: pasa cfg.VPN_CB_COOLDOWN_S al
#     constructor de VPNCircuitBreaker en lugar de hardcodear 300.0.
#
# BUG-DNS-RACE-001-FIX (Build 116): DNS stabilization delay after VPN connect.
#   NordVPN reports "Connected" before OS DNS routing is fully updated.
#   Queries in the first ~2-3s after connection may resolve via the old interface.
#   Fix: time.sleep(VPN_DNS_STABILIZE_DELAY_S=5.0) after successful rotate().
#   Evidence: 6 NameResolutionError failures for cf.bstatic.com in
#   INC-2026-0602-BROWSER-FREEZE-001 (L2843-2845, VPN→Netherlands 23:49:32).
# BUG-VPN-HANG-001-FIX (Build 114): _subprocess_run_safe() — evita cuelgues
#   indefinidos de subprocess.run(capture_output=True) en Windows 11 cuando
#   nordvpn.exe crea procesos hijo que mantienen pipes abiertos.
#   Incidente: INC-2026-0602-RADISSON-001 (6 horas, 02:16:23-08:16:07).
=============================================================
BUG-VPN-POPUP-DIRECT-001-FIX (Build 113): popup «¿Pausar la conexión
  automática en esta sesión?» en cada rotación VPN.
  Causa raíz: rotate() llamaba nordvpn.exe -d antes de conectar al nuevo
  servidor. NordVPN interpreta el -d explícito como intención de pausar la
  auto-conexión y muestra el diálogo de confirmación. La maquinaria de
  descarte existente (BM_CLICK pywin32, hilo de fondo) no descartaba el
  popup con suficiente fiabilidad en todos los tiempos de renderizado.
  Evidencia: el script de validación externo pruebas/extraer_imagenes.py usa
  nordvpn -c -g "country" directamente, SIN nordvpn -d previo, y nunca genera
  el popup (verificado con 141 hoteles). NordVPN gestiona el cambio de servidor
  internamente sin activar el diálogo de pausa de auto-conexión.
  Fix: rotate() ya NO llama self.disconnect(). Llama self.connect() directamente
  (= nordvpn -c -g "country"). El hilo de descarte de popup ya no se activa en
  la ruta normal. La maquinaria de descarte se conserva como red de seguridad.
  Nuevo toggle: VPN_ROTATE_SKIP_DISCONNECT (default True).
  Beneficios colaterales: reduce el tiempo de rotación ~5 s (elimina el
  sleep(5) de espera de teardown de túnel).
...
class VPNCircuitBreaker:
    ...
    BUG-VPN-001-FIX (Build 118 — Fix B):
    cooldown_seconds is now passed from config (VPN_CB_COOLDOWN_S, default 300s)
    instead of being hardcoded. This allows operational tuning without code changes.
    Added force_reset() for programmatic recovery.
    """
    ...
    def is_open(self) -> bool:
        with self._lock:
            ...
            if elapsed >= self._cooldown:
                ...
                logger.info(
                    "VPN circuit breaker CLOSED (cooldown %.0fs elapsed).",
                    elapsed,
                )
                return False
            remaining = self._cooldown - elapsed
            logger.debug(
                "VPN circuit breaker OPEN — %.0fs remaining in cooldown.",
                remaining,
            )
            return True
    ...
    def force_reset(self) -> None:
        """
        BUG-VPN-001-FIX (Build 118): Force-reset to CLOSED state.
        Used when the operator manually recovers VPN connectivity.
        Callable via API or diagnostic scripts.
        """
        with self._lock:
            prev_state = "OPEN" if self._opened_at is not None else "CLOSED"
            self._failures  = 0
            self._opened_at = None
        logger.info(
            "BUG-VPN-001-FIX: VPN circuit breaker force-reset "
            "from %s to CLOSED.", prev_state,
        )
...
class NordVPNManager:
    ...
    def __init__(self, interactive: bool = False) -> None:
        ...
        # BUG-VPN-001-FIX (Build 118 — Fix C): cooldown desde config en vez de hardcodeado.
        _cb_cooldown: float = float(getattr(self._cfg, "VPN_CB_COOLDOWN_S", 300.0))
    ...
    def disconnect(self) -> bool:
        ...
        # BUG-VPN-HANG-001-FIX (Build 114): _subprocess_run_safe() en lugar de
        # subprocess.run(capture_output=True) para evitar cuelgue por pipes.
        try:
            rc, stdout, stderr = self._subprocess_run_safe(
                [self._NORDVPN_EXE, "-d"],
                timeout=30,
                label="disconnect",
            )
            success = rc == 0 or "disconnected" in stdout.lower()
            if success:
                self._current_country = None
                logger.info("VPN disconnected.")
            elif "TIMEOUT" in stderr and rc == -1:
                logger.warning("VPN disconnect subprocess timed out — treating as disconnected")
                success = True   # Tunnel will drop; treat as success to unblock flow
            else:
                logger.warning("VPN disconnect returned code %d: %r", rc, stderr)
    ...
    def rotate(self, force: bool = False) -> bool:
        ...
        BUG-VPN-POPUP-DIRECT-001-FIX (Build 113):
          Con VPN_ROTATE_SKIP_DISCONNECT=True (default):
            Omite nordvpn -d y llama nordvpn -c -g "country" directamente.
            NordVPN cambia de servidor sin mostrar el popup
            "¿Pausar la conexión automática?". Mismo patrón que el benchmark
            externo validado pruebas/extraer_imagenes.py (141 hoteles, 0 popups).
            Reduce el tiempo de rotación ~5 s (elimina sleep de teardown).

          Con VPN_ROTATE_SKIP_DISCONNECT=False (legacy):
            Comportamiento anterior: disconnect() + sleep(5) + connect().
            Genera el popup si NordVPN tiene auto-connect activo.
        """
        # BUG-VPN-LOCK-DEADLOCK-001-FIX (Build 124):
        # Usar acquire(timeout=N) en lugar de "with self._lock:" para evitar
        # deadlock permanente cuando un daemon thread zombi de una rotación
        # previa sigue sosteniendo el lock tras expirar VPN_ROTATE_TIMEOUT_S.
        # VPN_LOCK_ACQUIRE_TIMEOUT_S (default=80) < VPN_ROTATE_TIMEOUT_S (default=90)
        # para que _rotate_vpn_with_timeout() detecte el fallo antes.
        ...
        if not acquired:
            logger.error(
                "BUG-VPN-LOCK-DEADLOCK-001-FIX: rotate() no pudo adquirir self._lock "
                "en %ds — posible zombie thread sosteniendo el lock. "
                "Omitiendo rotación para desbloquear el ciclo de scraping.",
        ...
        try:
            ...
            skip_disconnect = bool(getattr(self._cfg, "VPN_ROTATE_SKIP_DISCONNECT", True))

            if skip_disconnect:
                # BUG-VPN-POPUP-DIRECT-001-FIX (Build 113):
                # No nordvpn -d. NordVPN switches server internally — no popup.
                # Validated by pruebas/extraer_imagenes.py (same CLI pattern).
                logger.info(
                    "VPN: direct reconnect to %s (no disconnect — popup-free)",
                    self.COUNTRY_NAMES.get(new_country, new_country),
                )
                success = self.connect(new_country)

            else:
                # Legacy mode: explicit disconnect + reconnect (BUG-VPN-002-FIX Build 69).
                # May trigger "¿Pausar la conexión automática?" if auto-connect is on.
                logger.info(
                    "VPN: disconnect + reconnect to %s (legacy mode, VPN_ROTATE_SKIP_DISCONNECT=False)",
                    self.COUNTRY_NAMES.get(new_country, new_country),
                )
                self.disconnect()   # → starts background dismiss thread internally

                # Wait for tunnel teardown while background dismiss thread handles popup.
                time.sleep(5)
            ...
            if success:
                ...
                # BUG-DNS-RACE-001-FIX (Build 116): esperar a que el routing DNS
                # del SO esté estabilizado antes de retornar True al caller.
                # NordVPN reporta "Connected" antes de que /etc/resolv.conf (o el
                # equivalente en Windows) esté actualizado. Cualquier consulta DNS
                # en los primeros ~2-3s puede resolver contra la interfaz anterior.
                # Evidencia: 6 fallos NameResolutionError para cf.bstatic.com en
                # INC-2026-0602-BROWSER-FREEZE-001 (L2843-2845, VPN→Netherlands).
                _dns_delay: float = getattr(self._cfg, "VPN_DNS_STABILIZE_DELAY_S", 5.0)
                if _dns_delay > 0:
                    logger.info(
                        "BUG-DNS-RACE-001-FIX: DNS stabilization delay %.1fs "
                        "after VPN connect to %s",
                        _dns_delay, self.COUNTRY_NAMES.get(new_country, new_country),
                    )
                    time.sleep(_dns_delay)
            else:
    ...
    def _subprocess_run_safe(
        ...
        """
        BUG-VPN-HANG-001-FIX (Build 114): Windows-safe subprocess execution.

        Problema:
          subprocess.run(capture_output=True, timeout=N) en Windows 11 puede
          colgarse indefinidamente más allá de su parámetro timeout cuando
          nordvpn.exe crea procesos hijo que heredan los handles de los pipes
          stdout/stderr. Al expirar el timeout, subprocess.run mata el proceso
          padre pero los hijos mantienen los pipes abiertos; Python espera EOF
          de communicate() para siempre. El worker Celery queda congelado.

          Incidente INC-2026-0602-RADISSON-001: última entrada de log 02:16:23,
          worker congelado durante 6+ horas hasta error USB Chromium a las 08:16:07.

        Solución:
          Ejecutar el subprocess dentro de un daemon thread.
          threading.Thread.join(timeout+5) proporciona un corte de tiempo
          garantizado independientemente del estado de los pipes.
          Si el thread sigue vivo después del join, se abandona (daemon=True lo
          limpia automáticamente al salir el proceso) y se devuelve
          (-1, "", "TIMEOUT:{N}s"). El caller lo trata como fallo de conexión.

          Usamos Popen + communicate(timeout) dentro del thread para tener
          el timeout de Popen además del join externo como red de seguridad.

        Devuelve:
          (returncode: int, stdout: str, stderr: str)
          En timeout o error interno: (-1, "", "TIMEOUT:{N}s")
        """
        result: dict = {
            "rc": -1,
            "stdout": "",
        ...
        def _target() -> None:
            try:
                ...
                try:
                    ...
                    result["stdout"] = (out_bytes  or b"").decode(errors="replace")
                    result["stderr"] = (err_bytes  or b"").decode(errors="replace")
                    result["done"]   = True
                except subprocess.TimeoutExpired:
                    # Matar el proceso en cuanto expire; NO llamar communicate()
                    # otra vez porque también puede colgar si hay hijos.
                    try:
                        proc.kill()
                    except Exception:
                        pass
                    result["stderr"] = f"TIMEOUT:{timeout}s (Popen.communicate)"
                    result["done"]   = True
            except FileNotFoundError:
                result["stderr"] = f"FileNotFoundError: {cmd[0] if cmd else 'N/A'}"
                result["done"]   = True
            except Exception as exc:
                result["stderr"] = str(exc)
                result["done"]   = True

        t = threading.Thread(target=_target, daemon=True, name=f"nordvpn-{label}")
        t.start()
        t.join(timeout + 5)    # +5s margen por encima del timeout interno de Popen

        if not result["done"]:
            label_str = f" [{label}]" if label else ""
            logger.error(
                "BUG-VPN-HANG-001-FIX: nordvpn subprocess%s timed out after %ds "
                "— daemon thread abandonado (se limpiará al reiniciar proceso). "
                "cmd=%s",
                label_str, timeout + 5, cmd,
            )
    ...
    def _connect_via_cli(self, country_name: str) -> bool:
        ...
        BUG-VPN-003-FIX (Build 69):
          IP validation checks new_ip != self._prev_vpn_ip AND != self._original_ip.

        BUG-VPN-HANG-001-FIX (Build 114):
          subprocess.run(capture_output=True, timeout=60) reemplazado por
          _subprocess_run_safe() que ejecuta el subprocess en un daemon thread.
          Garantiza corte de tiempo real en Windows independientemente de pipes.
        """
        # BUG-VPN-HANG-001-FIX (Build 114): _subprocess_run_safe() en lugar de
        # subprocess.run(capture_output=True, timeout=60). El timeout de
        # subprocess.run puede ignorarse si nordvpn.exe crea hijos que mantienen
        # los pipes stdout/stderr abiertos — communicate() cuelga para siempre.
        _sub_timeout = getattr(self._cfg, "VPN_SUBPROCESS_TIMEOUT_S", 45)
        try:
            rc, stdout, stderr = self._subprocess_run_safe(
                [self._NORDVPN_EXE, "-c", "-g", country_name],
                timeout=_sub_timeout,
                label=f"connect:{country_name}",
            )
        except FileNotFoundError:
            logger.error(
        ...
        if "TIMEOUT" in stderr and rc == -1:
            logger.error(
                "BUG-VPN-HANG-001-FIX: nordvpn connect subprocess timed out "
                "after %ds for country=%s — returning False to unblock loop",
                _sub_timeout, country_name,
            )
            return False

        connected = rc == 0 or "connected" in stdout.lower()
        if connected:
            # FIX-v2.3-C: dismiss popup; short stabilisation wait.
            self._dismiss_nordvpn_popup()
            time.sleep(2)
            self._dismiss_nordvpn_popup()
            ...
            # BUG-VPN-003-FIX: new IP must differ from BOTH original AND prev VPN IP.
            ...
            if new_ip != "Unknown" and ip_changed_from_home and ip_changed_from_prev:
                # Normal success: IP resolved and changed correctly.
                logger.info("VPN connected to %s — IP: %s", country_name, new_ip)
                self._prev_vpn_ip = new_ip
                return True

            elif new_ip == self._prev_vpn_ip and new_ip != "Unknown":
                # IP resolved but unchanged — auto-connect likely re-used same server.
                logger.error(
                ...
                # BUG-VPN-001-FIX (Build 118 — Fix A):
                # IP check service returned Unknown, but NordVPN CLI confirmed
                # connection (rc=0 / "connected" in stdout). This is a DNS
                # verification timeout, NOT a VPN failure.
                #
                # Root cause of Build 115 circuit breaker lockout:
                #   3 consecutive "Unknown" IPs from api.ipify.org (slow DNS after
                #   VPN reconnect) each triggered record_failure() → CB opened.
                #   Audit log confirmed NordVPN actually connected each time.
                #
                # If VPN_CB_UNKNOWN_IP_AS_SUCCESS=true (default):
                #   Treat as success → record_success() → CB stays/goes CLOSED.
                #   WARNING is logged so the operator can monitor frequency.
                # If VPN_CB_UNKNOWN_IP_AS_SUCCESS=false:
                #   Legacy behaviour — treat as failure → record_failure().
                _accept_unknown: bool = bool(
                    getattr(self._cfg, "VPN_CB_UNKNOWN_IP_AS_SUCCESS", True)
                )
                if _accept_unknown:
                    logger.warning(
                        "BUG-VPN-001-FIX: VPN CLI connected to %s (rc=0) but "
                        "IP check returned Unknown — treating as SUCCESS "
                        "(VPN_CB_UNKNOWN_IP_AS_SUCCESS=true). "
                        "Circuit breaker will NOT be incremented. "
                        "(original=%s prev_vpn=%s)",
                        country_name, self._original_ip, self._prev_vpn_ip,
                    )
                    # Do NOT update _prev_vpn_ip — we don't have the new IP.
                    # Next successful IP check will update it.
                    return True
                else:
                    logger.error(
                        "VPN CLI connected but IP verification failed "
                        "(original=%s prev_vpn=%s current=Unknown) country=%s "
                        "[VPN_CB_UNKNOWN_IP_AS_SUCCESS=false — counting as failure]",
                        self._original_ip, self._prev_vpn_ip, country_name,
                    )
                    return False

            else:
                # IP resolved but equals original (VPN not protecting traffic).
                logger.error(
            ...
            logger.error(
                "NordVPN connect failed: country=%s rc=%d stderr=%r stdout=%r",
                country_name, rc, stderr, stdout,
            )
            return False
    ...
    def get_current_ip(self) -> str:
        ...
        # BUG-VPN-HANG-001-FIX (Build 114): deadline global de 30s entre todos los
        # servicios para que get_current_ip() nunca tarde más de 30s total.
        # Cada servicio tiene timeout=6s (reducido de 8s); si el sistema DNS no
        # resuelve a tiempo, la excepción es capturada y se prueba el siguiente.
        services = [
        ...
        result = "Unknown"
        _ip_deadline = time.monotonic() + 30   # 30s máx en total
        for svc in services:
            if time.monotonic() >= _ip_deadline:
                logger.warning("VPN: get_current_ip deadline reached — using Unknown")
                break
            _remaining = _ip_deadline - time.monotonic()
            try:
                resp = requests.get(svc, timeout=min(6.0, _remaining))
                if resp.status_code == 200:
