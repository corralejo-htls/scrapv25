"""
vpn_manager_windows.py — BookingScraper Pro v6.0.0 Build 72
=============================================================
...
BUG-VPN-008-FIX (Build 72): NordVPN popup NOT dismissed by UI Automation.
  Root cause: FindAll(Children, Name='NordVPN') returns zero results because
  the «¿Pausar la conexión automática?» dialog has an EMPTY window title.
  Fix: Win32 EnumWindows + EnumChildWindows + GetWindowText finds the
  «Cancelar» button by its control text and sends BM_CLICK directly.
  Independent of window title, accessibility tree, or rendering framework.
  Fallback: AppActivate(nordvpn) + ESC (no {TAB}{ENTER}).
...
class NordVPNManager:
        ...
        try:
            ...
            if connected:
                ...
                if new_ip != "Unknown" and ip_changed_from_home and ip_changed_from_prev:
                    ...
                    self._close_nordaccount_browser_tabs()  # BUG-VPN-005-FIX
                    return True
    ...
    def _dismiss_nordvpn_popup(self) -> None:
        """
        BUG-VPN-008-FIX (Build 72): Close the NordVPN popup using Win32
        EnumWindows + EnumChildWindows + BM_CLICK on the «Cancelar» button.

        Previous approach (UI Automation, Build 71) FAILED because:
          - FindAll(TreeScope::Children, Name='NordVPN') searches top-level
            windows by their accessibility Name property.
          - The NordVPN «¿Pausar la conexión automática?» dialog has an EMPTY
            window title / Name — confirmed by production log showing no
            «Cancelar clicked» message despite background thread running.
          - UI Automation never found the window → popup stayed open.

        New approach (Win32 API, independent of window title):
          1. P/Invoke: EnumWindows + EnumChildWindows + GetWindowText.
          2. Enumerate ALL top-level windows → enumerate ALL child controls.
          3. Find any child whose visible text is exactly «Cancelar».
          4. Send BM_CLICK (0x00F5) directly to that HWND.
          5. This works for Win32, WinForms, Qt, Electron — any framework
             that renders standard Windows button controls.
          Fallback: AppActivate('nordvpn') + ESC (no TAB, no ENTER).
        """
        try:
            ps_script = r"""
...
public class NordPopup {
    ...
    public static List<IntPtr> cancelBtns = new List<IntPtr>();
    public static bool FindCancelBtn(IntPtr hChild, IntPtr lParam) {
        var sb = new StringBuilder(256);
        GetWindowText(hChild, sb, 256);
        string t = sb.ToString().Trim();
        if ((t == "Cancelar" || t == "Cancel") && IsWindowVisible(hChild))
            cancelBtns.Add(hChild);
        return true;
    }
    public static bool EnumTopLevel(IntPtr hWnd, IntPtr lParam) {
        EnumChildWindows(hWnd, FindCancelBtn, IntPtr.Zero);
        return true;
    }
}
'@

[NordPopup]::cancelBtns.Clear()
[NordPopup]::EnumWindows([NordPopup+EnumProc]{ param($h,$l) [NordPopup]::EnumTopLevel($h,$l) }, [IntPtr]::Zero) | Out-Null
$count = [NordPopup]::cancelBtns.Count
if ($count -gt 0) {
    foreach ($btn in [NordPopup]::cancelBtns) {
        [NordPopup]::PostMessage($btn, [NordPopup]::BM_CLICK, [IntPtr]::Zero, [IntPtr]::Zero) | Out-Null
    }
    Write-Output "BUG-VPN-008-FIX: BM_CLICK sent to $count Cancelar button(s)"
} else {
    Write-Output "BUG-VPN-008-FIX: no Cancelar button found"
}
"""
            result = subprocess.run(
            ...
            if result.stdout:
                ...
                if "no Cancelar button found" in out:
                    logger.debug("BUG-VPN-008-FIX: No NordVPN popup found.")
                else:
                    logger.info("BUG-VPN-008-FIX: %s", out)
            if result.returncode != 0 and result.stderr:
                # Fallback: AppActivate + ESC (no TAB, no ENTER)
                logger.debug("BUG-VPN-008-FIX: P/Invoke failed, trying ESC fallback")
                self._dismiss_nordvpn_popup_esc_fallback()
        except Exception as exc:
            logger.debug("BUG-VPN-008-FIX: popup dismiss skipped: %s", exc)
            self._dismiss_nordvpn_popup_esc_fallback()

    def _dismiss_nordvpn_popup_esc_fallback(self) -> None:
        """ESC fallback: AppActivate nordvpn + send ESC. No TAB, no ENTER."""
        try:
            ps_esc = (
    ...
    def _close_nordaccount_browser_tabs(self) -> None:
        """
        BUG-VPN-005-FIX (Build 72 — máquina dedicada):
        Mata todos los procesos brave.exe que NO pertenecen a la sesión Selenium.

        Contexto operativo:
          - Esta máquina ejecuta ÚNICAMENTE BookingScraper Pro.
          - Brave se usa EXCLUSIVAMENTE para esta aplicación.
          - Brave debe tener exactamente UNA pestaña activa.
            Si hay más de una pestaña, es un error del sistema.

        Identificación del proceso Selenium vs NordVPN:
          - El Brave de Selenium tiene «brave_profile» en su línea de comandos
            porque se inicia con --user-data-dir=data/brave_profile (BUG-VPN-006-FIX).
          - Cualquier otro proceso brave.exe (abierto por NordVPN para nordaccount.com,
            páginas de soporte, OAuth, etc.) NO tiene «brave_profile» en su cmdline.

        Acción: Stop-Process -Force sobre todos los brave.exe no-Selenium.
        No se usa matching por título — innecesario y propenso a falsos positivos.
        El Brave de Selenium nunca es afectado.
        """
        cfg = self._cfg
        selenium_profile: str = getattr(cfg, "SELENIUM_BRAVE_PROFILE_DIR", "brave_profile")
        # Usar solo el nombre del directorio final como clave de búsqueda
        profile_key = (
            selenium_profile.replace('\\\\', '/').replace('\\', '/').rstrip('/').split('/')[-1]
            or "brave_profile"
        )
        ...
        try:
            ps_script = (
                ...
                "$killed = 0; "
                "$skipped = 0; "
                "Get-WmiObject Win32_Process -Filter \"Name='brave.exe'\" | ForEach-Object { "
                "  $cmdline = $_.CommandLine; "
                "  if ($cmdline -notmatch $profileKey) { "
                "    try { "
                "      Stop-Process -Id $_.ProcessId -Force -ErrorAction Stop; "
                "      $killed++; "
                "      Write-Output ('KILLED brave PID=' + $_.ProcessId); "
                "    } catch { "
                "      Write-Output ('KILL-FAILED PID=' + $_.ProcessId + ' ' + $_.Exception.Message); "
                "    } "
                "  } else { "
                "    $skipped++; "
                "  } "
                "}; "
                "Write-Output ('BUG-VPN-005-FIX: killed=' + $killed + ' skipped(selenium)=' + $skipped)"
            )
            result = subprocess.run(
                ["powershell", "-NoProfile", "-NonInteractive", "-Command", ps_script],
                capture_output=True, text=True, timeout=10, shell=False,
            ...
            if result.stdout:
                for line in result.stdout.strip().splitlines():
                    ...
                    if not line:
                        continue
                    if "killed=0" in line:
                        logger.debug("BUG-VPN-005-FIX: No rogue Brave processes found.")
                    elif line.startswith("KILLED"):
                        logger.info("BUG-VPN-005-FIX: %s", line)
                    elif "killed=" in line:
                        logger.info("BUG-VPN-005-FIX: %s", line)
                    elif line.startswith("KILL-FAILED"):
                        logger.warning("BUG-VPN-005-FIX: %s", line)
            if result.returncode != 0 and result.stderr:
                logger.debug("BUG-VPN-005-FIX: ps stderr: %s", result.stderr[:300])
        except Exception as exc:
            logger.debug("BUG-VPN-005-FIX: rogue Brave cleanup skipped: %s", exc)
