"""
vpn_manager_windows.py — BookingScraper Pro v6.0.0 Build 71
=============================================================
...
class NordVPNManager:
    ...
    def _dismiss_nordvpn_popup(self) -> None:
        """
        BUG-VPN-005-FIX (Build 71): Close the NordVPN popup using Windows UI
        Automation instead of SendKeys.

        Previous implementation (Build 69) used WScript.Shell.AppActivate +
        SendKeys {TAB}{ENTER}. {TAB} moved focus to a hyperlink inside the
        NordVPN UI («Get help» / «Learn more») and {ENTER} opened
        nordaccount.com in Brave — producing 11+ unwanted tabs per session.

        New strategy (UI Automation):
          1. Enumerate all top-level windows matching «NordVPN» or «Pausar».
          2. Find the «Cancelar»/«Cancel» button by AutomationId or Name.
          3. Invoke it directly via InvokePattern — zero keyboard simulation.
          Fallback: send ESC only (no {TAB}{ENTER}) if UI Automation fails.
        """
        try:
            ps_script = (
                ...
                "  [System.Windows.Automation.AutomationElement]::NameProperty, 'NordVPN'); "
                "$windows = $desktop.FindAll("
                "  [System.Windows.Automation.TreeScope]::Children, $cond); "
                "foreach ($win in $windows) { "
                "  $btnCond = New-Object System.Windows.Automation.OrCondition("
                "    (New-Object System.Windows.Automation.PropertyCondition("
                "      [System.Windows.Automation.AutomationElement]::NameProperty,'Cancelar')),"
                "    (New-Object System.Windows.Automation.PropertyCondition("
                "      [System.Windows.Automation.AutomationElement]::NameProperty,'Cancel'))); "
                "  $btn = $win.FindFirst("
                "    [System.Windows.Automation.TreeScope]::Descendants, $btnCond); "
                "  if ($btn -ne $null) { "
                "    $pat = $btn.GetCurrentPattern("
                "      [System.Windows.Automation.InvokePattern]::Pattern); "
                "    $pat.Invoke(); "
                "    Write-Output 'BUG-VPN-005-FIX: Cancelar clicked via UI Automation'; "
                "  } else { "
            ...
            if result.stdout and "Cancelar clicked" in result.stdout:
                logger.debug("BUG-VPN-005-FIX: NordVPN popup dismissed via UI Automation.")
        except Exception:
            pass  # Non-critical — popup TTL is short and VPN still connects

    def _close_nordaccount_browser_tabs(self) -> None:
        """
        BUG-VPN-005-FIX (Build 71 rev3 — máquina dedicada):
