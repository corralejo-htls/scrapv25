...
class HotelExtractor:
    ...
    def _extract_amenities(self) -> List[str]:
        ...
        Fallback: selector legacy data-testid="facility-list-item".

        BUG-DATA-001-FIX (Build 64):
          Fallback 1 (popular-facilities-wrapper) ELIMINADO intencionalmente.
          Razón: data-testid="property-most-popular-facilities-wrapper" es la fuente
          EXCLUSIVA de _extract_popular_services() (STRUCT-008).  Al tenerlo también
          aquí como Fallback 1, AMBAS funciones retornaban la misma lista cuando
          property-section--facilities no estaba presente, provocando que
          hotels_amenities y hotels_popular_services almacenasen datos idénticos
          (confirmado: 78/78 pares url×idioma con conteo igual, 0 excepciones).
          La responsabilidad de popular-facilities-wrapper pertenece exclusivamente
          a _extract_popular_services(); _extract_amenities() NO debe tocarlo.
        ...
        # BUG-DATA-001-FIX: Fallback 1 (popular-facilities-wrapper) ELIMINADO.
        # Ver docstring completo arriba para justificación técnica.

        # Fallback: selector legacy
        try:
