"""
extractor.py — BookingScraper Pro v6.0.0 Build 124
===================================================
Cambios v124:

  BUG-EXTRAINFO-SELECTOR-001-FIX (Build 124):
    _extract_extra_info() fallaba en el 100% de los hoteles austriacos auditados
    (840/840 filas vacías confirmadas en pruebas/_table__hotels_extra_info__.csv).
    Causa raíz: el selector principal data-testid="property-important-info" NO
    existe en el DOM actual de Booking.com para la región AT. El bloque
    "Need-to-know information" está integrado en la sección id="important_info"
    (clase cbdacf5131), que el extractor de fine_print ya captura correctamente.
    El extractor de extra_info NO incluía id="important_info" entre sus selectores.
    Fix: añadida Strategy 5 que busca el heading "need to know" / equivalente
    por idioma en el DOM y extrae su contenido; Strategy 6 como fallback final
    que accede directamente a id="important_info" si las estrategias previas fallan.
    Validado contra HTML_1001_villa-dvor_en-gb.html (offset 589,193 confirmado).
    Impacto esperado: extra_info poblado en todos los hoteles con sección
    "Need-to-know" / "Fine Print". Campo API extraInfo dejará de ser null.

  BUG-FAQ-FP-001-FIX (Build 124):
    _extract_faqs() generaba ~8 registros falsos positivos por hotel procedentes
    del widget de disponibilidad de Booking.com. El escáner global de
    aria-expanded (Strategy 4) capturaba botones del selector de fechas y número
    de personas ("Check-in date - Check-out date", "2 adults · 0 children · 1 room",
    etc.) como si fueran preguntas FAQ. Evidencia: 61/6.104 respuestas vacías
    en pruebas/_table__hotels_faqs__.csv; preguntas identificadas incluyen
    "Check-in date - Check-out date" y "Fecha de entrada" (español).
    Fix: añadido _FAQ_DENYLIST regex en _add_question() que filtra estos textos
    antes de añadirlos a la lista. El filtro actúa en TODAS las estrategias
    (accordion, headings, aria-expanded global) garantizando cobertura completa.

Cambios v123:

  BUG-NEARBY-CATMAP-001-FIX (Build 123):
    _CATEGORY_MAP tenía 5 gaps para idiomas DE e IT verificados contra los
    9 HTML de prueba disponibles en pruebas/ (análisis DOM 2026-06-13):
      DE: "Nächste Flughäfen" → regex "flughafen" no matcheaba "Flughäfen" (ä ≠ a)
           Fix: añadido r"flugh.fen" (dot matches ä or a).
      DE: "Öffentliche Verkehrsmittel" → sin keyword en regex transport.
           Fix: añadido "verkehrsmittel".
      DE: "Naturschönheiten" → sin keyword en regex nature.
           Fix: añadido r"natursch.nheit".
      IT: "Aeroporti più vicini" → "aeroporti" (plural) no matcheaba "aeroporto".
           Fix: añadido "aeroporti".
      IT: "Trasporti pubblici" → sin keyword en regex transport.
           Fix: añadido "trasporti".
    Impacto: aeropuertos y transporte DE/IT mapeaban a "attraction" (code 6)
    en lugar de "airport" (1) y "transport" (4). Naturaleza DE mapeaba a
    "attraction" en lugar de "nature" (5). Corregido para las 6 sesiones
    de idiomas que se scrapen.

  BUG-NEARBY-SKELETON-001-FIX (Build 123):
    El warning genérico "no poi-block nor location-highlight elements found"
    no distinguía entre dos causas raíz:
      A) Skeleton/lazy-loading: surroundings_block existe pero su contenido
         son React skeleton placeholders (class ab6126d40e) no hidratados.
         Evidencia: Hotel Topazz (76033) en pruebas/ muestra 12 spans
         skeleton, cero poi-blocks. Ocurre cuando window.stop() interrumpe
         la hidratación React antes de que el contenido se renderice.
      B) Sin datos: Booking.com no tiene nearby_places para esa propiedad
         (común en hoteles pequeños/remotos).
      C) Sin sección: surroundings_block ausente de la página.
    Fix: Strategy 2 ahora detecta el patrón skeleton (ab6126d40e),
    distingue las 3 causas y emite logs con nivel e información apropiados.
    El WARNING solo se emite en el caso A (skeleton); B y C son INFO.
    El comportamiento de extracción no cambia (retorna [] en todos los casos).
Cambios v102:

  BUG-CAT-SPACE-001-FIX (Build 102):
    Root cause: div.get_text() en la Strategy E45 extraía el texto del div
    que contiene el SVG de la categoría + el texto de categoría + el badge
    "Free!" (o variante localizada) sin insertar espacio entre ellos.
    Resultado: "Indoor swimming pool" + "Free!" → "Indoor swimming poolFree!".
    Se confirmó en BD: 169 filas EN con categorías malformadas.
    Variantes afectadas por idioma:
      EN: "Indoor/Outdoor swimming poolFree!"
      IT: "Piscina coperta/scopertaGratis!"
      PT: "Piscina interior/exteriorGrátis!"
      ES: "Piscina interior¡Gratis!", "Piscina al aire libre¡Gratis!"
      DE: "Innenpool/AußenpoolKostenlos!"
    Fix 1: _normalize_category_heading() — nuevo método estático.
      Aplica re.sub para insertar espacio antes del badge si falta.
      Cubre: Free!, Gratis!, Kostenlos!, Grátis!, ¡Gratis!
    Fix 2: get_text(separator=" ") en los 3 puntos de extracción de
      category_title en Strategy E45 (líneas ~2068, ~2076, ~2084).
      El separator=" " garantiza espacio entre nodos inline del DOM.
    Fix 3: _normalize_category_heading() aplicada también en
      Strategy 1 y Strategy 1.5 (candidate = child.get_text()).
    Sin cambios en lógica de negocio, esquema, ORM ni API contract.

Cambios v101:

  BUG-SVC-NULL-001-FIX (Build 101): NULL service_category epidemic.
    BUILD-103: _infer_service_category() eliminado (v4.5 verbatim DOM).
    categorías clave como Parking, Safety & security, Accessibility, Reception
    services, Pets, Languages spoken, Internet (descripciones largas), ni General
    (lift, hardwood floors, heating). Además, la función _add() solo aplicaba
    la inferencia cuando la estrategia DOM no proporcionaba categoría, pero NO
    la ejecutaba como post-proceso cuando el heading del facility-group era vacío.
    Fix 1: _add() ahora ejecuta _infer_service_category() como fallback
      automático cuando category="" (vacía), ANTES de persistir el item.
      Esto garantiza que TODOS los items con heading vacío obtienen categoría.
    Fix 2: _SERVICE_CATEGORY_RULES ampliada con 7 grupos nuevos:
      - Languages spoken (24): nombres de idiomas en 6 idiomas, antes del
        grupo Services para evitar que "English/Inglés" cayera en Services (3).
      - Pets (6): mascotas, admite mascotas, animales permitidos.
      - Safety & security (14): extintores, CCTV, alarmas de humo, tarjeta.
      - Accessibility (17): acceso en ascensor, parking accesible.
      - Reception services (19): proporciona factura, recepción 24h.
      - Internet descriptive (11): "WiFi is available in all areas...",
        "WiFi gratis", "WiFi gratuit", etc.
      - Parking extended (16): EV charging, parking gratis/privado/accesible.
      - General extended (1): lift/ascensor, hardwood floors, heating,
        calefacción, suelo de madera/parquet, key card (moved to Safety).
    Fix 3: _extract_services_from_json() también procesa GenericFacilityHighlight
      objects del Apollo JSON cache. Estos tienen título pero sin groupId;
      se aplica _infer_service_category() para asignar categoría.
    Impacto esperado: reducción de NULL service_category de ~50-60% a <10%.

  BUILD-101-SYNC: config.py y __init__.py sincronizados a Build 101.

Cambios v99:

  BUG-SVC-DOM-001-FIX: DOM strategies primary; Apollo JSON fallback.
    Root cause: Strategy 0 (Apollo JSON) se ejecutaba antes que las estrategias
    DOM y retornaba early con solo 5-20 items. El bloque expandido (post-clic
    Selenium) tiene 30-45+ items pero nunca se procesaba.
    Fix: estrategias DOM (1, 1.5, 2) son PRIMARIAS. Apollo JSON es Strategy 3
    (fallback). Solo se ejecuta si las estrategias DOM producen 0 items.
...
def extract_hotel_photos_from_html(html: str) -> List[Dict[str, Any]]:
    """
    Parsea fotos del hotel desde el HTML de Booking.com.

    BUG-PHOTO-LIMIT-001-FIX (Build 90):
      Booking.com limita hotelPhotos JS a ~45 entradas en el HTML estático.
      Las demás fotos (hasta ~270+) están presentes como <img> tags con
      URLs autenticadas (k= token). Esta función las extrae todas.

    Estrategia dual:
      1. hotelPhotos JS: extrae hasta ~45 fotos con metadatos ricos
         (alt, orientation, grid dimensions). Fuente primaria.
      2. Supplement <img> URLs: extrae todos los max1024x768 y max200 URLs
         con sus k= tokens del HTML. Añade las fotos extra no cubiertas por
         el JS (generalmente fotos de galería adicionales).
    ...
    Returns lista de dicts con claves:
      ...
      highres_url : str  — max1280x900 URL (solo en primeras ~45)
      alt         : str  — texto alternativo (solo en primeras ~45)
      orientation : str  — landscape|portrait|square (solo en primeras ~45)
      created     : str  — timestamp (solo en primeras ~45)
      photo_width : int  — ancho original (solo en primeras ~45)
      photo_height: int  — alto original (solo en primeras ~45)
    """
    photos: List[Dict[str, Any]] = []
    if not html:
        return photos

    # ── Fuente 1: hotelPhotos JS (metadatos ricos, ~45 fotos) ────────────────
    match = re.search(
        r'hotelPhotos\s*:\s*(\[(?:[^\[\]]|\[(?:[^\[\]]|\[[^\[\]]*\])*\])*\])',
        html,
        re.DOTALL,
    ...
    if match:
        ...
        for obj in parsed:
            ...
            photo: Dict[str, Any] = {"id_photo": pid_str}
    ...
    logger.debug("hotelPhotos JS: %d photos parsed", len(photos))

    # ── Fuente 2: Supplement de URLs <img> ───────────────────────────────────
    # BUG-PHOTO-LIMIT-001-FIX (Build 90):
    # Extrae URLs autenticadas (k= token) directamente del HTML. Cubre las
    # ~200+ fotos adicionales no incluidas en el array hotelPhotos JS.
    #
    # BUG-IMG-DERIVE-001-FIX (Build 107):
    # Auditoría de datos (image_downloads): highres_url solo cubría el 49,3%
    # de las fotos y thumb_url el 93,1%, mientras large_url alcanzaba el 100%.
    # Causa raíz: la Fuente 2 reconstruía SOLO max1024x768 y max200; nunca
    # generaba max1280x900. El highres quedaba limitado a las ~45 fotos del
    # array JS. NO era un problema de timeout (solo 81 errores de descarga en
    # 28.544). Las URLs de Booking comparten el MISMO token k= entre tallas y
    # solo difieren en el segmento de tamaño, por lo que las tres tallas son
    # derivables de cualquier URL autenticada. Esta corrección recolecta el
    # token por id_photo desde cualquier talla presente en el HTML (o en las
    # URLs ya capturadas por la fuente JS) y deriva las tres tallas para TODA
    # foto. Cumple el requisito explícito: "no limitar la extracción de imágenes".
    #
    # Regex: captura hasta el primer caracter especial que termina la URL
    # (comilla, espacio, backslash, ampersand HTML).
    _SIZE_SEGMENTS = {
        "thumb_url": "max200",
    ...
    _token_pattern = re.compile(
        r'https://cf\.bstatic\.com/xdata/images/hotel/'
        r'(?:max200|max300|max500|max1024x768|max1280x900|square\d+)/'
        r'(\d+)\.jpg\?k=([^"\'&\\\s]+)'
    )

    def _build_url(pid: str, category: str, token: str) -> str:
        seg = _SIZE_SEGMENTS[category]
        return f"https://cf.bstatic.com/xdata/images/hotel/{seg}/{pid}.jpg?k={token}"

    # 1) Recolecta el token k= por id_photo desde TODAS las tallas del HTML.
    token_by_pid: Dict[str, str] = {}
    for m in _token_pattern.finditer(html):
        pid, k = m.group(1), m.group(2)
        token_by_pid.setdefault(pid, k)

    # 2) Complementa con tokens embebidos en las URLs ya capturadas por la
    #    fuente JS (cubre IDs que no aparezcan sueltos como <img> en el HTML).
    _embedded = re.compile(r'/hotel/[^/]+/(\d+)\.jpg\?k=([^"\'&\\\s]+)')
    for photo in photos:
        pid = photo["id_photo"]
        ...
        for cat in ("large_url", "highres_url", "thumb_url"):
            val = photo.get(cat)
            if val:
                em = _embedded.search(val)
                if em and em.group(1) == pid:
                    token_by_pid.setdefault(pid, em.group(2))
                    break

    # 3) Deriva las tres tallas para las fotos de la fuente JS.
    for photo in photos:
        pid = photo["id_photo"]
        token = token_by_pid.get(pid)
        if not token:
            continue
        for cat in _SIZE_SEGMENTS:
            if not photo.get(cat):
                photo[cat] = _build_url(pid, cat, token)

    # 4) Añade fotos supplement (no presentes en hotelPhotos JS) con las tres
    #    tallas derivadas del token.
    supplement_ids = set(token_by_pid.keys()) - js_ids
    for pid in supplement_ids:
        token = token_by_pid[pid]
        supp: Dict[str, Any] = {"id_photo": pid}
        for cat in _SIZE_SEGMENTS:
            supp[cat] = _build_url(pid, cat, token)
        photos.append(supp)

    logger.debug(
        "hotelPhotos total: %d (JS=%d supplement=%d, 3 tallas derivadas)",
        len(photos), len(js_ids), len(supplement_ids),
    )
    return photos
...
class HotelExtractor:
    ...
    def extract_all(self) -> Dict[str, Any]:
        ...
        data: Dict[str, Any] = {
            ...
            # STRUCT-CITY-001 (Build 88): metadatos urbanos de booking.env script tag
            **self._extract_city_metadata(),
    ...
    # Términos genéricos en breadcrumb posición [2] que desplazan la jerarquía.
    _GENERIC_BC_TERMS: frozenset = frozenset({
        "all resorts", "todos los hoteles", "todas las opciones de resorts",
        "todas las opciones", "tous les hotels", "alle hotels",
        "tutti gli hotel", "todas as opcoes",
    })

    def _extract_city_metadata(self) -> Dict[str, Optional[str]]:
        """
        STRUCT-CITY-001 (Build 88) / BUG-ENV-REGEX-001-FIX (Build 89):
        Extrae metadatos urbanos de Booking.com.

        Correcciones Build 89 — regexes:
          Build 88 buscaba b_atnm_en y b_city_name (con prefijo b_).
          El HTML real usa atnm_en y city_name SIN prefijo b_.
          Confirmado en 13 HTMLs reales de pruebas/ (100 pct cobertura).

        Campos nuevos Build 89:
          dest_id      — context_dest_id en el bloque autocomplete_vars
          region_name  — breadcrumb items[3] cuando NO hay genérico en [2]
          district_name — breadcrumb items[5] cuando n==7 y NO genérico

        Retorna dict con 6 claves (siempre presentes, None si no encontradas):
          city_name, dest_ufi, dest_id, atnm_en, region_name, district_name
        """
        ...
        # Estrategia 1: campos JS inline del HTML
        # Booking.com distribuye campos en múltiples bloques JS.
        # Se escanea el HTML completo en una sola pasada.
        try:
            fh = self.html
            # city_name (sin prefijo b_): city_name: 'Manaus'
            m = re.search(r'\bcity_name\s*:\s*[\x27"]((?:[^\x27"]{1,120}))[\x27"]', fh)
            if m:
                result["city_name"] = m.group(1).strip()
            # b_ufi (con prefijo b_): b_ufi : '-666610'
            m = re.search(r'\bb_ufi\s*:\s*[\x27"]((?:[^\x27"]{1,40}))[\x27"]', fh)
            if m:
                result["dest_ufi"] = m.group(1).strip()
            # context_dest_id: context_dest_id = '-666610'
            m = re.search(r'context_dest_id\s*[=:]\s*[\x27"]((?:[^\x27"]{1,40}))[\x27"]', fh)
            if m:
                result["dest_id"] = m.group(1).strip()
            # atnm_en (sin prefijo b_): atnm_en: 'hotel'
            m = re.search(r'\batnm_en\s*:\s*[\x27"]((?:[^\x27"]{1,64}))[\x27"]', fh)
            if m:
                result["atnm_en"] = m.group(1).strip()
        except Exception as exc:
            logger.debug("_extract_city_metadata js: %s", exc)

        # Estrategia 2: breadcrumb DOM
        # n=6 (estándar): Home>Hotels>Country>Region>City>Hotel
        # n=7 (distrito): Home>Hotels>Country>Region>City>District>Hotel
        # n=6/7 (genérico): Home>Hotels>"All resorts">Country>City[>District]>Hotel
        #   Genérico en items[2] = sin nivel Region fiable → no extraer region_name.
        try:
            bc = self.soup.select_one("#subheader-wrap nav ol")
            if bc:
                items = bc.find_all("li")
                n = len(items)

                def _li_text(li) -> Optional[str]:
                    a = li.find("a")
                    if a:
                        sp = a.find("span")
                        t = sp.get_text(strip=True) if sp else a.get_text(strip=True)
                    else:
                        sp = li.find("span")
                        t = sp.get_text(strip=True) if sp else li.get_text(strip=True)
                    return t if t and len(t) < 120 else None

                is_generic = (
                    n >= 3 and
                    (_li_text(items[2]) or "").lower() in self._GENERIC_BC_TERMS
                )

                if not result["city_name"] and n >= 2:
                    result["city_name"] = _li_text(items[-2])
                if not is_generic and n >= 6 and not result["region_name"]:
                    result["region_name"] = _li_text(items[3])
                if not is_generic and n >= 7 and not result["district_name"]:
                    result["district_name"] = _li_text(items[5])
        except Exception as exc:
            logger.debug("_extract_city_metadata breadcrumb: %s", exc)
    ...
    # BUILD-103: _FACILITY_GROUP_MAP and _SERVICE_CATEGORY_RULES removed.
    # Facility service_category is stored verbatim from DOM (extraer4.py v4.5).
    # No keyword inference or groupId translation is performed.
    ...
    # BUILD-103: _infer_service_category() removed.
    # service_category is verbatim DOM text from extraer4.py v4.5 strategy.

    @staticmethod
    def _normalize_category_heading(text: str) -> str:
        """
        BUG-CAT-SPACE-001-FIX (Build 102):
        Normaliza el encabezado de categoría insertando espacio antes del
        badge "Free!" (y variantes localizadas) cuando está directamente
        pegado al nombre de la categoría sin espacio.

        Problema: div.get_text() en Booking.com concatena el nombre de la
        categoría con el badge inline sin nodo de texto intermedio:
          "Indoor swimming pool" + "Free!" → "Indoor swimming poolFree!"

        El separator=" " en get_text() ya corrige la mayoría de los casos,
        pero este método actúa como safety net para instancias residuales.

        Variantes cubiertas:
          Free!    (EN)
          Gratis!  (IT, DE parcial)
          Kostenlos! (DE)
          Grátis!  (PT)
          ¡Gratis! (ES — el signo ¡ va antes del badge)

        Ejemplos:
          "Indoor swimming poolFree!"       → "Indoor swimming pool Free!"
          "Piscina copertaGratis!"          → "Piscina coperta Gratis!"
          "InnenpoolKostenlos!"             → "Innenpool Kostenlos!"
          "Piscina interiorGrátis!"         → "Piscina interior Grátis!"
          "Piscina interior¡Gratis!"        → "Piscina interior ¡Gratis!"
        """
        if not text:
            return text
        return re.sub(
            r'([^\s!¡])(Free!|Gratis!|Kostenlos!|Grátis!|¡Gratis!)',
            r'\1 \2',
            text,
        )

    def _popular_facilities_category(self, wrapper: Any) -> str:
        """
        BUG-SVC-POPULAR-CAT-001-FIX (Build 111):
        Extrae VERBATIM la cabecera del bloque
        ``property-most-popular-facilities-wrapper`` para usarla como
        service_category en el fallback Strategy 4.

        Auditoría (verificada contra pruebas/_table__hotels_all_services__.csv):
        cuando TODAS las estrategias de facility-group-container fallan, los
        únicos servicios provienen de este wrapper y quedaban con
        service_category="". El DOM real SÍ contiene un encabezado, p. ej.:
            <div data-testid="property-most-popular-facilities-wrapper">
              <div><h3><div>Most popular facilities</div></h3><ul>…</ul></div>
            </div>
        (verificado en pruebas/HTML_76033__Hotel-Topazz.html y
        HTML_1001_Villa-Dvor_Ohrid.html).

        Se devuelve el texto del primer <h3> (sin <ul>/<li>), normalizado y
        truncado a 128 chars. Es texto LITERAL del DOM — no hay inferencia ni
        traducción (coherente con la política Build 103). Devuelve "" si no se
        encuentra cabecera (degradación segura al comportamiento anterior).
        """
        try:
            for h3 in wrapper.find_all("h3"):
                # Evita capturar h3 que contenga la propia lista de servicios.
                if h3.find(["ul", "li", "ol"]):
                    continue
                txt = self._normalize_category_heading(
                    re.sub(r"\s+", " ", h3.get_text(separator=" ")).strip()
                )
                if txt:
                    return txt[:128]
        except Exception as exc:
            logger.debug(
                "popular_facilities_category extraction failed lang=%s: %s",
                self.language, exc,
            )
        return ""
    ...
    def _extract_all_services(self) -> List[Dict[str, str]]:
        """
        STRUCT-014 (Build 103 — BUG-SVC-EXTRAER4-001 v4.5):
        Extrae la lista COMPLETA de servicios/instalaciones del hotel.

        BUILD-103-EXTRAER4-V45:
        Strategy E45 (PRIMARY) basada en extraer4.py v4.5 (Main-Only / Flat-Row).
        CAMBIO CLAVE vs Build 102 (v4.4): Phase 1 ("Great for your stay") ELIMINADA.
        Solo se procesa property-facilities-block-container (Phase 2).

        service_category se importa VERBATIM desde el DOM de Booking.com.
        Sin inferencia por keywords, sin traducción por groupId.
        Sin _FACILITY_GROUP_MAP, sin _SERVICE_CATEGORY_RULES.

        Mapa de campos (extraer4.py v4.5 → hotels_all_services):
          extraer4.title_category -> service_category  (texto DOM verbatim)
          extraer4.all_services   -> service            (texto DOM verbatim)

        Estrategias:
          E45. Phase 2 ONLY: property-facilities-block-container
               → facility-group-container → h3+SVG + Option 1/2/3 (PRIMARY).
          1.   DOM: property-section--facilities + facility-group (legacy fallback).
          1.5. DOM: facility-group standalone (legacy fallback).
          2.   facilit* regex (legacy fallback).
          3.   Apollo/GraphQL JSON cache — service_category="" (last resort).
          4.   popular-wrapper — service_category="" (final fallback).
        ...
        def _add(text: str, category: str = "") -> None:
            ...
            if (
                ...
                seen.add(t)
                # BUILD-103: category is verbatim DOM text — no keyword inference.
        ...
        # ── Strategy E45: extraer4.py v4.5 Phase 2 ONLY (PRIMARY) ────────────
        # BUILD-103: Phase 1 ("Great for your stay") removed per extraer4.py v4.5.
        # Only property-facilities-block-container is processed.
        # Categories and services stored verbatim from Booking.com DOM.
        try:
            prop_block = self.soup.find(
                attrs={"data-testid": "property-facilities-block-container"}
            )
            if prop_block:
                seen_categories_e45: set = set()
                cat_groups = prop_block.find_all(
                    attrs={"data-testid": "facility-group-container"}
                )
                for cat_group in cat_groups:
                    try:
                        all_h3s = cat_group.find_all("h3")
                        if not all_h3s:
                            continue

                        # extraer4 v4.5: First h3 with SVG = category title container
                        category_h3 = None
                        for h3 in all_h3s:
                            if h3.find("svg"):
                                category_h3 = h3
                        ...
                        # extraer4 v4.5: EXTRACT TITLE from div[.//svg]
                        # SVGs in Booking.com carry no visible text — get_text() safe.
                        category_title = ""
                        for div in category_h3.find_all("div"):
                            if div.find("svg"):
                                # BUG-CAT-SPACE-001-FIX (Build 102):
                                # separator=" " evita que el badge "Free!" se pegue
                                # al nombre de categoría: "Indoor swimming poolFree!"
                                # → "Indoor swimming pool Free!"
                                txt = re.sub(
                                    r"\s+", " ", div.get_text(separator=" ")
                                ).strip()
                                txt = self._normalize_category_heading(txt)
                                if txt:
                                    category_title = txt
                                    break

                        # extraer4 v4.5: Fallback — first direct div with text
                        if not category_title:
                            for div in category_h3.find_all("div", recursive=False):
                                # BUG-CAT-SPACE-001-FIX (Build 102): separator=" "
                                txt = re.sub(
                                    r"\s+", " ", div.get_text(separator=" ")
                                ).strip()
                                txt = self._normalize_category_heading(txt)
                        ...
                        # extraer4 v4.5: Ultimate fallback — h3 text
                        if not category_title:
                            # BUG-CAT-SPACE-001-FIX (Build 102): separator=" "
                            category_title = self._normalize_category_heading(
                                re.sub(
                                    r"\s+", " ",
                                    category_h3.get_text(separator=" "),
                                ).strip()
                            )
                        ...
                        if category_title in seen_categories_e45:
                            logger.debug(
                                "AllServices E45: dup category %r lang=%s",
                                category_title, self.language,
                            )
                            continue

                        # ── PRE-COMPUTE Option 1 / 2 / 3 indicators ──────────
                        # extraer4 v4.5: Option 1 — extra div texts inside category_h3
                        direct_divs_h3 = category_h3.find_all("div", recursive=False)
                        div_texts: List[str] = []
                        for div in direct_divs_h3:
                            txt = re.sub(r"\s+", " ", div.get_text()).strip()
                            if txt:
                                div_texts.append(txt)
                        h3_service_divs = div_texts[1:] if len(div_texts) > 1 else []

                        # extraer4 v4.5: Option 2 — h3s without SVG = sub-headers
                        service_h3_texts: List[str] = []
                        for h in all_h3s:
                            if h is not category_h3 and not h.find("svg"):
                                txt = re.sub(r"\s+", " ", h.get_text()).strip()
                                if txt:
                                    service_h3_texts.append(txt)

                        # extraer4 v4.5: Option 3 — ul with at least 1 li
                        ul_with_li = [
                            ul for ul in cat_group.find_all("ul")
                            if ul.find("li")
                        ]

                        phase_found = False

                        # ── Option 1: h3+svg with service divs inside h3 ─────
                        if h3_service_divs:
                            seen_categories_e45.add(category_title)
                            for svc in h3_service_divs:
                                _add(svc, category=category_title)
                            phase_found = True
                            logger.debug(
                                "AllServices E45 Opt1: cat=%r -> %d svcs lang=%s",
                                category_title, len(h3_service_divs), self.language,
                            )

                        # ── Option 2: h3+svg + h3(no svg) + ul/li ────────────
                        elif service_h3_texts and ul_with_li:
                            seen_categories_e45.add(category_title)
                            for txt in service_h3_texts:
                                _add(txt, category=category_title)
                            for ul in ul_with_li:
                            ...
                            phase_found = True
                            logger.debug(
                                "AllServices E45 Opt2: cat=%r lang=%s",
                                category_title, self.language,
                            )

                        # ── Option 3: h3+svg + ul/li (no service h3s) ────────
                        elif ul_with_li:
                            seen_categories_e45.add(category_title)
                            for ul in ul_with_li:
                                for li in ul.find_all("li"):
                            ...
                            phase_found = True
                            logger.debug(
                                "AllServices E45 Opt3: cat=%r -> %d uls lang=%s",
                                category_title, len(ul_with_li), self.language,
                            )

                        # ── Fallback: service h3s without ul ─────────────────
                        elif service_h3_texts:
                            seen_categories_e45.add(category_title)
                            for txt in service_h3_texts:
                                _add(txt, category=category_title)
                            phase_found = True
                            logger.debug(
                                "AllServices E45 Fallback h3: cat=%r lang=%s",
                                category_title, self.language,
                            )

                        if not phase_found:
                            logger.debug(
                                "AllServices E45: no services for cat=%r lang=%s",
                                category_title, self.language,
                            )
                    except Exception as exc:
                        logger.debug(
                            "AllServices E45 group error lang=%s: %s",
                            self.language, exc,
                        )

                if services:
                    logger.debug(
                        "AllServices: strategy_E45 (extraer4 v4.5 Phase2-only): "
                        "%d items lang=%s",
            ...
            logger.debug(
                "AllServices strategy E45 failed lang=%s: %s", self.language, exc
            )

        # ── Strategy 1: property-section--facilities + facility-group (legacy) ─
        # Fallback cuando facility-group-container no está en el DOM.
        # category es texto DOM verbatim del heading del grupo (sin inferencia).
        try:
            section = self.soup.find(
            ...
            if section:
                ...
                if fac_groups_in_section:
                    for group in fac_groups_in_section:
                        ...
                        if content_div:
                            for child in content_div.find_all(
                                ...
                                # BUG-CAT-SPACE-001-FIX (Build 102): separator=" "
                                candidate = self._normalize_category_heading(
                    ...
                    if services:
                        logger.debug(
                            "AllServices: strategy1 (section+facility-group): %d items lang=%s",
                            len(services), self.language,
                        )
                        return services
                for li in section.find_all("li"):
                    _add(li.get_text(" ", strip=True), category="")
                if services:
                    logger.debug(
        ...
        # ── Strategy 1.5: facility-group standalone (legacy) ─────────────────
        try:
        ...
        # ── Strategy 2: facilit* regex (legacy) ──────────────────────────────
        try:
            groups = self.soup.find_all(
                attrs={"data-testid": re.compile(r"facilit", re.I)}
            )
            # BUG-SVC-POPULAR-CAT-001-FIX (Build 111): el regex 'facilit' también
            # casa con property-most-popular-facilities-wrapper. Cuando E45 no
            # encuentra grupos (p. ej. el bloque de instalaciones no se expandió
            # o el hotel sólo expone el bloque "Most popular facilities"), esta
            # estrategia capturaba sus <li> con category="" y retornaba ANTES de
            # Strategy 4. Ahora, si el grupo es el wrapper popular, se extrae su
            # cabecera <h3> VERBATIM como categoría (mismo texto literal del DOM,
            # sin inferencia). El resto de grupos mantiene el comportamiento
            # previo (category="").
            popular_cat_enabled = getattr(
                self._cfg, "SVC_POPULAR_FALLBACK_CATEGORY_ENABLED", True
            )
            for group in groups:
                group_cat = ""
                if (
                    popular_cat_enabled
                    and group.get("data-testid")
                    == "property-most-popular-facilities-wrapper"
                ):
                    group_cat = self._popular_facilities_category(group)
                for li in group.find_all("li"):
                    _add(li.get_text(" ", strip=True), category=group_cat)
            if services:
                logger.debug(
                    "AllServices: strategy2 (facilit* regex): %d items lang=%s",
                    len(services), self.language,
                )
                return services
        except Exception as exc:
        ...
        # ── Strategy 3: Apollo/GraphQL JSON cache (last resort) ───────────────
        # BUILD-103: service_category from JSON is "" (no _FACILITY_GROUP_MAP).
        try:
            json_services = self._extract_services_from_json()
            if json_services and len(json_services) >= 5:
                logger.debug(
                    "AllServices: strategy3-json (Apollo fallback): %d items lang=%s",
            ...
            logger.debug(
                "AllServices strategy3-json failed lang=%s: %s", self.language, exc,
            )

        # ── Strategy 4: popular-wrapper (final fallback) ──────────────────────
        # BUG-SVC-POPULAR-CAT-001-FIX (Build 111): cuando TODAS las estrategias
        # de facility-group-container fallan, los servicios sólo proceden de
        # este wrapper. Antes quedaban con service_category="". Ahora se extrae
        # VERBATIM la cabecera <h3> del bloque ("Most popular facilities" /
        # "Instalaciones más populares" / …) y se usa como categoría. Texto
        # literal del DOM — sin inferencia ni traducción (política Build 103).
        # Toggle: SVC_POPULAR_FALLBACK_CATEGORY_ENABLED (default True).
        try:
            wrapper = self.soup.find(
                attrs={"data-testid": "property-most-popular-facilities-wrapper"}
            )
            if wrapper:
                ...
                if getattr(
                    ...
                    popular_category = self._popular_facilities_category(wrapper)
                for li in wrapper.find_all("li"):
                    _add(li.get_text(" ", strip=True), category=popular_category)
                if services:
                    logger.debug(
                        "AllServices: strategy4 (popular fallback): %d items "
                        "lang=%s category=%r",
                        len(services), self.language, popular_category,
                    )
        except Exception as exc:
            logger.debug("AllServices strategy 4 failed lang=%s: %s", self.language, exc)
    ...
    def _extract_services_from_json(self) -> List[Dict[str, str]]:
        ...
        Bugs corregidos vs versión anterior:
          ...
          BUG-SVC-CAT-001-B (histórico, Build 87): No extraía groupId de cada BaseFacility.
            FIX original: mapeo a _FACILITY_GROUP_MAP[groupId][language].
            BUILD-103: _FACILITY_GROUP_MAP eliminado — service_category="".
        ...
        Estructura Apollo JSON verificada en HTML real (EN/ES/DE/IT/FR/PT):
          ...
          groupId → service_category="" (BUILD-103: _FACILITY_GROUP_MAP eliminado)
        ...
        generic_highlights: List[Dict[str, Any]] = []
        seen_generic_titles: set = set()
        ...
        for script in self.soup.find_all("script", type="application/json"):
            ...
            # BUG-SVC-NULL-001-FIX (Build 101): collect GenericFacilityHighlight
            # objects (Great for your stay highlights — have title, no groupId).
            for key2, val2 in data.items():
                if (key2.startswith("GenericFacilityHighlight:") and
                        isinstance(val2, dict)):
                    gh_title = val2.get("title", "")
                    if (gh_title and isinstance(gh_title, str) and
                            gh_title.strip() and
                            gh_title.strip() not in seen_generic_titles):
                        seen_generic_titles.add(gh_title.strip())
                        generic_highlights.append({"title": gh_title.strip()})
        ...
        if not base_facilities and not generic_highlights:
            logger.debug(
                "AllServices JSON: no BaseFacility or GenericFacilityHighlight objects found lang=%s",
                self.language,
            )
            return []

        # BUILD-103: _FACILITY_GROUP_MAP removed — service_category is "" for JSON strategy.
        # JSON fallback provides service names verbatim; no category translation.
        ...
        for fac in base_facilities:
            ...
            # BUILD-103: category always "" — no _FACILITY_GROUP_MAP available.
            result.append({"service": title, "service_category": ""})

        # GenericFacilityHighlight objects (no groupId, no inference in Build 103).
        for gh in generic_highlights:
            gh_title = gh["title"]
            if gh_title not in seen_titles:
                seen_titles.add(gh_title)
                result.append({"service": gh_title, "service_category": ""})

        if generic_highlights:
            logger.debug(
                "AllServices JSON: added %d GenericFacilityHighlight items lang=%s",
                len(generic_highlights), self.language,
            )
    ...
    def _extract_faqs(self) -> List[Dict[str, str]]:
        ...
        # BUG-FAQ-FP-001-FIX (Build 124): filtro de falsos positivos.
        # El escáner global de aria-expanded (Strategy 4) captura botones del
        # widget de disponibilidad de Booking.com como si fueran preguntas FAQ.
        # Patrones identificados en pruebas/_table__hotels_faqs__.csv:
        #   "Check-in date - Check-out date"
        #   "2 adults · 0 children · 1 room"
        #   "Fecha de entrada" / "Fecha de salida"
        # El regex filtra cualquier texto que coincida con estos patrones UI.
        _FAQ_DENYLIST: re.Pattern = re.compile(
            r"check.?in date|check.?out date"                # EN date picker labels
            r"|check.?in\s*[-–]\s*check.?out"               # EN combined date label
            r"|\d+\s+adults?|adults?\s*[·•,]\s*children"    # occupancy widget text
            r"|fecha de entrada|fecha de salida"             # ES date picker labels
            r"|einreisedatum|abreisedatum"                   # DE date picker labels
            r"|date d.arriv[eé]e|date de d[eé]part"         # FR date picker labels
            r"|data di arrivo|data di partenza"              # IT date picker labels
            r"|data de entrada|data de saída"                # PT date picker labels
            r"|^\s*\d+\s+noche|^\s*\d+\s+night"             # "N nights" widget text
            r"|selecciona tus fechas|select your dates"      # date CTA buttons
        ...
        def _add_question(text: str, answer_text: str = "") -> None:
            ...
            if t and len(t) > 5 and len(t) < 1000 and t not in seen:
                # BUG-FAQ-FP-001-FIX (Build 124): descartar textos del widget de búsqueda
                if _FAQ_DENYLIST.search(t):
                    logger.debug("FAQs: denylist match — descartando falso positivo: %r", t[:80])
                    return
                seen.add(t)
    ...
    def _extract_extra_info(self) -> Optional[str]:
        ...
        # Strategy 5: BUG-EXTRAINFO-SELECTOR-001-FIX (Build 124)
        # Buscar heading "Need to know" / equivalente por idioma y extraer su bloque.
        # En el DOM actual de Booking.com (AT, 2026-06-14) el contenido "Need-to-know
        # information" está dentro de id="important_info" (cbdacf5131) como sub-sección.
        # data-testid="property-important-info" no existe en los 140 hoteles auditados.
        _NTK_KEYWORDS: Dict[str, List[str]] = {
        ...
        try:
            lang_key = self.language[:2].lower()
            ntk_keywords = (
                _NTK_KEYWORDS.get(lang_key, []) +
                _NTK_KEYWORDS.get("en", [])
            )
            for kw in ntk_keywords:
                try:
                    el = self.soup.find(
                        ["h2", "h3", "h4", "h5", "div", "span", "p"],
                        string=re.compile(re.escape(kw), re.I | re.UNICODE),
                    )
                    if el:
                        # Subir al contenedor padre con párrafos de contenido
                        parent = el.parent
                        for ancestor in [parent, parent.parent if parent else None]:
                            ...
                            if text and len(text) > 20:
                                logger.debug(
                                    "ExtraInfo: strategy5 (need-to-know heading %r) "
                                    "len=%d lang=%s", kw, len(text), self.language,
                                )
                                return text
                except Exception:
                    continue
        except Exception as exc:
            logger.debug("ExtraInfo strategy5 failed lang=%s: %s", self.language, exc)

        # Strategy 6: BUG-EXTRAINFO-SELECTOR-001-FIX (Build 124) — fallback final.
        # id="important_info" (clase cbdacf5131) contiene el bloque completo
        # "The fine print / Need-to-know information" de Booking.com. Aunque
        # fine_print también lo captura, es preferible poblar extra_info con el
        # mismo contenido a dejarlo vacío para el 100% de los hoteles austriacos.
        try:
            ...
            if el:
                ...
                if text and len(text) > 10:
                    logger.debug(
                        "ExtraInfo: strategy6 (id=important_info fallback) "
                        "len=%d lang=%s", len(text), self.language,
                    )
                    return text
        except Exception as exc:
            logger.debug("ExtraInfo strategy6 failed lang=%s: %s", self.language, exc)
    ...
    def _extract_nearby_places(self) -> List[Dict[str, Any]]:
        ...
        # BUG-NEARBY-CATMAP-001-FIX (Build 123):
        # _CATEGORY_MAP tenía 5 gaps para idiomas DE e IT verificados contra los
        # 9 HTML de prueba de pruebas/:
        #   DE: "Nächste Flughäfen" → regex r"flughafen" no matchea "Flughäfen" (ä ≠ a)
        #   DE: "Öffentliche Verkehrsmittel" → sin keyword en transport regex
        #   DE: "Naturschönheiten" → sin keyword en nature regex
        #   IT: "Aeroporti più vicini" → "aeroporti" (plural) no matchea "aeroporto"
        #   IT: "Trasporti pubblici" → sin keyword en transport regex
        # Fix: añadir variantes plurales/idiomáticas para DE e IT en cada regex.
        _CATEGORY_MAP: List[tuple] = [
            (re.compile(
                r"airport|aeropuerto|flughafen|flugh.fen|aéroport|aeroporto|aeroporti",
                re.I
            ), "airport"),
            (re.compile(
                r"restaurant|cafe|cafes|caf.s|bar|food|essen|restaur|ristorante|ristoranti",
                re.I
            ), "restaurant"),
            (re.compile(
                r"beach|playa|strand|plage|praia|spiaggia|spiagge|playas",
                re.I
            ), "beach"),
            (re.compile(
                r"transport|transit|bus|metro|tram|train|bahn|verkehrsmittel|trasporti|commun",
                re.I
            ), "transport"),
            (re.compile(
                r"natural|nature|natursch.nheit|natur|park|lake|lago|see|montagne|berg"
                r"|montagna|bellezze|mont.e",
                re.I
            ), "nature"),
        ]
        ...
        try:
            ...
            if poi_blocks:
                ...
                if not highlights:
                    ...
                    #   A) Skeleton/lazy-loading: surroundings_block exists but
                    #      its content is React skeleton placeholders (class ab6126d40e)
                    #      not yet hydrated. This happens in static HTML captures or
                    #      when window.stop() was called before JS hydration completed.
                    #      Evidence: Hotel Topazz (76033) snapshot in pruebas/ shows
                    #      12 skeleton spans, zero poi-blocks.
                    #   B) No data: Booking.com has no nearby places for this property
                    #      (common for very small/remote hotels).
                    # Both modes are non-fatal — extractor returns []. Logging is improved
                    # to help distinguish during post-session analysis.
                    surr_block = self.soup.find(id="surroundings_block")
                    if surr_block:
                        skeleton_spans = surr_block.find_all(
                            "span", class_=lambda c: c and "ab6126d40e" in (c if isinstance(c, str) else " ".join(c))
                        )
                        if skeleton_spans:
                            logger.warning(
                                "NearbyPlaces: surroundings_block present but content "
                                "is skeleton/not-hydrated (%d skeleton spans) lang=%s "
                                "— JS hydration may have been interrupted by window.stop(). "
                                "Consider adding a surroundings scroll trigger in scraper.py.",
                                len(skeleton_spans), self.language
                            )
                        else:
                            logger.info(
                                "NearbyPlaces: surroundings_block present but empty "
                                "(no poi-block, no skeleton) lang=%s "
                                "— property likely has no nearby places data on Booking.com.",
                                self.language
                            )
                    else:
                        logger.info(
                            "NearbyPlaces: no surroundings_block found lang=%s "
                            "— section absent from page (property may not support nearby places).",
                            self.language
                        )
                    return places
    ...
    def _extract_individual_reviews(self) -> List[Dict[str, Any]]:
        ...
        # ── BUG-REVIEWS-DOM-001-FIX (Build 95) ────────────────────────────────
        # CAUSA RAÍZ: data-testid="review-card" NO existe en el DOM estático de
        # Booking.com. Las reseñas individuales se renderizan via React hydration
        # y NO están presentes como elementos HTML en el source inicial.
        # Sin embargo, los datos de reseñas SÍ están presentes en el Apollo JSON
        # cache embebido en <script type="application/json"> como objetos
        # FeaturedReview:ID con todos los campos necesarios.
        #
        # Confirmado en 2 HTMLs de pruebas/ (atlántico-rio EN, Art Hotel Spielberg EN):
        #   - data-testid="review-card" → 0 ocurrencias en todos los HTML
        #   - "FeaturedReview:ID" → 10 objetos por hotel con campos completos
        #
        # Estructura FeaturedReview verificada:
        #   { __typename, id, userId, isAnonymous, averageScore, completed,
        #     guestCountryCode, guestName, positiveText, negativeText, language,
        #     customerType, purposeType, title, reviewUrl, userAvatarUrl, ... }
        #
        # ESTRATEGIA 0 (Apollo JSON) es ahora PRIMARIA.
        # ESTRATEGIA 1 (data-testid="review-card") se mantiene como FALLBACK
        # por si Booking.com implementa SSR de reviews en alguna variante de DOM.

        # ── Estrategia 0: Apollo JSON cache (FeaturedReview objects) ──────────
        try:
            script_tags = self.soup.find_all("script", {"type": "application/json"})
            apollo_cache: Dict[str, Any] = {}
            for tag in script_tags:
                raw = tag.string or ""
                if "FeaturedReview:" in raw:
                    try:
                        apollo_cache = json.loads(raw)
                        break
                    except (json.JSONDecodeError, ValueError):
                        continue
            ...
            featured_reviews = {
                ...
                if k.startswith("FeaturedReview:") and isinstance(v, dict)
            }

            if featured_reviews:
                logger.debug(
                    "IndividualReviews: strategy0 (Apollo JSON): %d FeaturedReview objects lang=%s",
                    len(featured_reviews), self.language,
                )
                for _key, obj in featured_reviews.items():
                    reviewer_name: Optional[str] = obj.get("guestName") or None
                    raw_score = obj.get("averageScore")
                    score: Optional[float] = None
                    if raw_score is not None:
                        try:
                            val = float(raw_score)
                            # Booking.com averageScore is 0-10 but may come as 0-100
                            score = val if val <= 10.0 else val / 10.0
                        except (TypeError, ValueError):
                            pass
                    title_val: Optional[str] = obj.get("title") or None
                    positive_comment: Optional[str] = obj.get("positiveText") or None
                    negative_comment: Optional[str] = obj.get("negativeText") or None
                    # guestCountryCode is ISO 2-letter; keep as-is (e.g. "cl", "gb")
                    reviewer_country: Optional[str] = obj.get("guestCountryCode") or None
                    booking_id: Optional[str] = (
                        str(obj["id"]) if obj.get("id") is not None else
                        obj.get("reviewUrl") or None
                    )

                    key = (reviewer_name, title_val, positive_comment)
                    if key in seen:
                        continue
                    seen.add(key)
                ...
                if reviews:
                    logger.debug(
                        "IndividualReviews: strategy0 extracted %d reviews lang=%s",
                        len(reviews), self.language,
                    )
                    return reviews
            ...
            logger.debug(
                "IndividualReviews strategy0 (Apollo JSON) failed lang=%s: %s",
                self.language, exc,
            )

        # ── Estrategia 1: DOM data-testid="review-card" (fallback) ────────────
        # NOTA: Este testid no existe en el DOM estático actual de Booking.com
        # (confirmado 2026-04-30). Se mantiene como salvaguarda por si Booking.com
        # implementa SSR de reviews en el futuro.
        try:
            ...
            if not cards:
                logger.debug(
                    ...
                    "(expected — reviews are in Apollo JSON, not DOM)",
                    self.language,
                )
                return reviews
            ...
            if reviews:
                logger.debug(
                    "IndividualReviews: strategy1 (DOM) extracted %d reviews lang=%s",
                    len(reviews), self.language,
                )
            else:
