"""
extractor.py — BookingScraper Pro v6.0.0 Build 78
==================================================
Cambios v78:
...
class HotelExtractor:
    ...
    def _extract_room_types(self) -> List[Dict[str, Any]]:
        ...
        Retorna: List[Dict] con claves:
          ...
          - facilities  : List[str] — siempre [] (las facilidades de habitación
                          requieren clic en cada fila; fuera del alcance del
                          scraper estático actual)
        """
    ...
    def _extract_nearby_places(self) -> List[Dict[str, str]]:
        ...
        DOM REAL verificado contra archivos HTML en pruebas/ (Booking.com React 2025):
          ...
          <section id="surroundings_block">
            <div data-testid="poi-block">            ← un bloque por categoría
              <h3>Top attractions</h3>
              <ul data-testid="poi-block-list">      ← lista de lugares
                <li>
                  <span role="listitem">
                    <div>
                      <div class="...d1bc97eb82">Victoria Clock Tower</div>
                      <div>
                        <div class="...cbf0753d0c">3.6 km</div>
                      </div>
                    </div>
                  </span>
                </li>
        ...
        NOTA: el selector anterior "location-highlight" NO existe en el DOM actual.
        El selector "poi-block-list" tampoco incluye el tipo (category) vía SVG —
        la categoría se infiere del h3 del bloque padre.

        Retorna: List[Dict] con claves:
          - place_name : nombre del lugar
          - distance   : distancia textual (e.g. "2.1 km", "500 m")
          - category   : categoría inferida del h3 del bloque (e.g. "airport",
                         "restaurant", "attraction", "beach", "transport")
        """
        places: List[Dict[str, str]] = []
        seen: set = set()

        # Mapeo de keywords del h3 → categoría normalizada
            ...
            if poi_blocks:
                for block in poi_blocks:
                    ...
                    for item in list_el.find_all(lambda t: t.name == "span" and
                                                  t.get("role") == "listitem"):
                        divs = item.find_all("div", recursive=True)
                        if not divs:
                            continue

                        # Name: first div with substantial text (not a sub-type label)
                        name = ""
                        distance = ""
                        for div in divs:
                            text = div.get_text(strip=True)
                            # Distance pattern: number + unit
                            if re.search(r"^\d+[.,]?\d*\s*(km|m|mi|miles)$", text, re.I):
                                if not distance:
                                    distance = text
                            elif text and len(text) > 2 and len(text) <= 256 and not name:
                                # Skip sub-type labels (e.g. "Restaurant", "Peak") —
                                # they are short capitalised single words in a span.f0595bb7c6
                                parent_span = div.find_parent("span", class_=re.compile(r"f0595bb7c6"))
                                if parent_span:
                                    continue
                                name = text

                        # Fallback distance: any text matching pattern in entire item
                        if not distance:
                            for el in item.find_all(string=re.compile(
                                    r"\d+[.,]?\d*\s*(km|m|mi|miles)", re.I)):
                                t = el.strip()
                                if re.search(r"^\d+[.,]?\d*\s*(km|m|mi|miles)\*?$", t, re.I):
                                    distance = t
                                    break
                ...
                # Strategy 2: legacy fallback — location-highlight (older DOM)
