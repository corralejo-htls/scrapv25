"""
extractor.py — BookingScraper Pro v6.0.0 Build 82
==================================================
Cambios v82:
...
class HotelExtractor:
    ...
    def _extract_all_services(self) -> List[Dict[str, str]]:
        ...
        # AUDIT-HTML-001-FIX: data-testid="facility-group" contiene el nombre del
        # grupo (categoría) en el primer div tras el icono, y los servicios en <li>.
        # BUILD-82-FIX: se extrae el nombre del grupo como service_category.
        #
        # Estructura DOM verificada:
        #   <div data-testid="facility-group">
        #     <div data-testid="facility-group-icon">...</div>
        #     <div>
        #       <div>General</div>          ← nombre de categoría (grupo)
        ...
        try:
            ...
            if fac_groups:
                for group in fac_groups:
                    # Extraer y eliminar el icono para no contaminar el texto
                    icon_el = group.find(attrs={"data-testid": "facility-group-icon"})
                    if icon_el:
                        icon_el.extract()

                    # BUILD-82-FIX: nombre de categoría = primer texto del div
                    # hermano del icono (primer hijo directo que contenga texto)
                    group_category = ""
                    content_div = group.find("div")  # primer div restante
                    if content_div:
                        # El nombre de categoría está en el primer div/span hijo
                        # que NO contiene <ul>/<li> (es solo texto)
                        for child in content_div.find_all(
                            ["div", "span", "h3", "h4"], recursive=False
                        ):
                ...
                for block in poi_blocks:
                    ...
                    for item in list_el.find_all(lambda t: t.name == "span" and
                        ...
                        places.append({
                            ...
                            "category":      category,
                            "category_code": _category_code(category),  # BUILD-82-FIX
                        })
