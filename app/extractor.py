"""
extractor.py — BookingScraper Pro v6.0.0 build 55
==================================================
Cambios v55:
...
class HotelExtractor:
    ...
    # =========================================================================
    # STRUCT-014 (v53): ALL SERVICES EXTRACTOR
    # =========================================================================

    def _extract_all_services(self) -> List[str]:
        """
        STRUCT-014 (v53): Extrae la lista COMPLETA de servicios/instalaciones.

        Diferencia con _extract_amenities():
          - _extract_amenities() captura texto de elementos <span> del bloque
            principal de instalaciones (texto corto normalizado).
          - _extract_all_services() captura el texto completo de cada elemento
            <li> de la sección de instalaciones, incluyendo sub-categorías y
            elementos que no contienen <span> independiente.
            Objetivo: máxima cobertura de servicios.

        Estructura DOM típica:
          <div data-testid="property-section--facilities">
            <div class="..."> <!-- categoría: "Piscina" -->
              <ul>
                <li><span>Piscina al aire libre</span></li>
                <li><span>Piscina cubierta</span></li>
              </ul>
            </div>
            <div class="..."> <!-- categoría: "Internet" -->
              <ul>
                <li><span>WiFi gratuito en todas las habitaciones</span></li>
              </ul>
            </div>
          </div>
        ...
        def _add(text: str) -> None:
            t = text.strip()
            # Filtro: texto válido, no demasiado corto/largo, no links "Ver todos"
            if (
                t
                and len(t) > 2
                and len(t) < 300
                and t not in seen
                and not re.match(
                    r"(Ver |See |Alle |Voir |Ver )(todos|all|les|alle|\d)", t, re.I
                )
            ):
                seen.add(t)
                services.append(t)

        # Estrategia 1: sección facilities completa — texto de cada <li>
        try:
            section = self.soup.find(
                attrs={"data-testid": "property-section--facilities"}
            )
            if section:
                for li in section.find_all("li"):
                    # Obtener texto completo del <li>, eliminando duplicados de spans anidados
                ...
                if services:
                    logger.debug(
                        "AllServices: extracted via facilities section <li>: %d items lang=%s",
        ...
        # Estrategia 2: facility groups (data-testid con "facility")
        try:
            groups = self.soup.find_all(
                attrs={"data-testid": re.compile(r"facility", re.I)}
            )
            for group in groups:
            ...
            if services:
                logger.debug(
                    "AllServices: extracted via facility data-testid groups: %d items lang=%s",
                    len(services), self.language,
                )
                return services
        except Exception as exc:
        ...
        # Estrategia 3: fallback — popular-facilities-wrapper como mínimo
        try:
            wrapper = self.soup.find(
                attrs={"data-testid": "property-most-popular-facilities-wrapper"}
            )
            if wrapper:
                ...
                if services:
                    logger.debug(
                        "AllServices: extracted via popular-wrapper fallback: %d items lang=%s",
                        len(services), self.language,
                    )
        except Exception as exc:
