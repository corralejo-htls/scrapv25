"""
BookingScraper/app/extractor.py  v2.8
Extractor de datos HTML de Booking.com
Windows 11 + Python 3.14.3

CAMBIOS v2.8 [FIX DE IDIOMA EN CONTENIDO EXTRAÍDO]:
  [FIX CRÍTICO] extract_description(): eliminado find("p", data-testid=...)
    que nunca encontraba el elemento porque Booking.com 2025/2026 usa <div>.
    El extractor caía al fallback og:description que Booking.com sirve en el
    idioma de la sesión (siempre español) independientemente de la URL.
    Ahora se usa find(attrs={data-testid=...}) sin restricción de tag.
    Nuevo fallback: JSON-LD description (datos estructurados, idioma correcto).
    ELIMINADO: og:description como fallback (sirve idioma incorrecto).
  [FIX] extract_services(): añadido filtro _SERVICE_NOISE_RE para excluir
    texto de botones UI ("Ver disponibilidad", "Check availability", etc.)
    que era extraído como servicio por el selector data-testid="facilities".
  [FIX] extract_house_rules(): añadidos selectores modernos para la estructura
    Booking.com 2024-2026 (data-testid="HouseRules" y variantes).
...
class BookingExtractor:
    ...
    def extract_description(self) -> Optional[str]:
        """
        [v2.8] Extraer descripción del hotel.

        FIX CRÍTICO: Booking.com 2025/2026 usa <div> (no <p>) para el elemento
        data-testid="property-description". El selector anterior find("p", ...) nunca
        encontraba el elemento y caía al fallback og:description, que Booking.com sirve
        en el idioma de la sesión (español) independientemente del idioma de la página.

        ELIMINADO: og:description como fallback. Este meta-tag NO se traduce correctamente
        al idioma de la URL (.de.html, .fr.html, etc.) — siempre llega en el idioma de
        la sesión/cookie de la petición previa. Se usa JSON-LD como fallback alternativo
        ya que los datos estructurados SÍ se sirven en el idioma correcto.
        """
        if self.soup:
            # 1. data-testid="property-description" — CUALQUIER tag (div, p, section...)
            #    [FIX v2.8] Se eliminó find("p", ...) que nunca encontraba el elemento
            #    porque Booking.com usa <div> en 2025/2026.
            ...
            # 2. Variantes de testid para descripción (PropertyDescription, etc.)
            for testid in [
        ...
        # 3. XPath — cualquier tag (no solo <p>)
        ...
        # 5. div con clase hotel_desc (legacy)
        if self.soup:
        ...
        # 6. JSON-LD description — datos estructurados, se sirven en idioma correcto
        #    cuando la URL tiene el sufijo de idioma correcto (.de.html?lang=de, etc.)
        if self.soup:
            for script in self.soup.find_all("script", type="application/ld+json"):
                try:
                    data = json.loads(script.string or "")
        ...
        # NOTA: og:description eliminado intencionalmente — Booking.com sirve este
        # meta-tag en el idioma de la sesión del navegador, no del idioma de la URL.
        return None
    ...
    # Texto UI de Booking.com que NO son servicios reales — botones, CTAs, labels de navegación
    _SERVICE_NOISE_RE = re.compile(
        ...
        r'|ver\s+todas|see\s+all|alle\s+anzeigen|voir\s+tout|vedi\s+tutto'
        r'|reservar|book\s+now|buchen|réserver|prenota|boek\s+nu'
        r'|ver\s+mapa|show\s+on\s+map|auf\s+karte\s+zeigen|voir\s+sur\s+la\s+carte'
        r'|mostrar\s+más|show\s+more|mehr\s+anzeigen|montre\+plus'
        r'|\d+\s+opiniones?|\d+\s+reviews?|\d+\s+bewertungen?|\d+\s+avis'
        r'|check.?in|check.?out|eincheckzeit|heure\s+d.arrivée'
        r'|cancelación\s+gratuita|free\s+cancellation|kostenlose\s+stornierung'
        r'|no\s+se\s+requiere\s+tarjeta|no\s+credit\s+card\s+needed'
        r'|breakfast\s+included|desayuno\s+incluido|frühstück\s+inklusive)$',
        re.IGNORECASE
    )
    ...
    def extract_services(self) -> List[str]:
        ...
        def _is_valid_service(text: str) -> bool:
            """Filtra texto que no es un servicio real (botones, CTAs, etc.)."""
            if not text or len(text) < 3 or len(text) > 120:
                return False
            # Excluir texto de botones/UI detectado por patrón
            if self._SERVICE_NOISE_RE.match(text.strip()):
                return False
            # Excluir si es solo números o puntuación
        ...
        if box:
            for li in box.find_all(["li", "span"]):
                ...
                if _is_valid_service(text) and text not in services:
        ...
        for container in self.soup.find_all(attrs={"data-testid": re.compile(r"facilities|amenities|services", re.I)}):
            for elem in container.find_all(["li", "span", "div"]):
                # Evitar nodos anidados (tomar solo hojas del árbol)
                if elem.find(["li", "div"]):
                    continue
    ...
    def extract_house_rules(self) -> Optional[str]:
        if self.soup:
            ...
            # 2. data-testid con "policies" o "house-rules" (2024-2026)
            sec = self.soup.find(attrs={"data-testid": re.compile(r"policies|house.?rules|HouseRules|normas|regeln|règles|regole", re.I)})
            if sec:
                return sec.get_text(separator="\n", strip=True) or None

            # 3. Sección con id o clase que contenga "house" o "rule" o "polic"
            for attr_val in [re.compile(r"house.?rule|house.?policy|hotel.?rule", re.I)]:
                sec = self.soup.find(attrs={"id": attr_val})
                if sec:
                    return sec.get_text(separator="\n", strip=True) or None
                sec = self.soup.find(class_=attr_val)
                if sec:
                    return sec.get_text(separator="\n", strip=True) or None
