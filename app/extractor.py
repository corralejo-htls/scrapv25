"""
extractor.py — BookingScraper Pro v6.0.0 Build 101
===================================================
Cambios v101:

  BUG-SVC-NULL-001-FIX (Build 101): NULL service_category epidemic.
    Root cause: _infer_service_category() tenía reglas insuficientes — no cubría
...
def extract_hotel_photos_from_html(html: str) -> List[Dict[str, Any]]:
    ...
    # BUG-PHOTO-LIMIT-001-FIX (Build 90):
    # Extrae large_url (max1024x768) y thumb_url (max200) con sus k= auth tokens
    # directamente del HTML. Cubre las ~200+ fotos adicionales no incluidas
    # en el array hotelPhotos JS. Solo añade IDs no vistos en la fuente JS.
    #
    ...
    large_map: Dict[str, str] = {}   # pid -> full URL
    thumb_map: Dict[str, str] = {}
    for m in large_pattern.finditer(html):
        pid, k = m.group(1), m.group(2)
        if pid not in large_map:
            large_map[pid] = f"https://cf.bstatic.com/xdata/images/hotel/max1024x768/{pid}.jpg?k={k}"
    for m in thumb_pattern.finditer(html):
        pid, k = m.group(1), m.group(2)
        if pid not in thumb_map:
            thumb_map[pid] = f"https://cf.bstatic.com/xdata/images/hotel/max200/{pid}.jpg?k={k}"

    # Update JS photos with any missing URLs
    for photo in photos:
        pid = photo["id_photo"]
        if not photo.get("large_url") and pid in large_map:
            photo["large_url"] = large_map[pid]
        if not photo.get("thumb_url") and pid in thumb_map:
            photo["thumb_url"] = thumb_map[pid]

    # Add supplement photos (not in hotelPhotos JS)
    supplement_ids = set(large_map.keys()) - js_ids
    for pid in supplement_ids:
        supp: Dict[str, Any] = {"id_photo": pid}
        if pid in large_map:
            supp["large_url"] = large_map[pid]
        if pid in thumb_map:
            supp["thumb_url"] = thumb_map[pid]
        photos.append(supp)

    logger.debug(
        "hotelPhotos total: %d (JS=%d supplement=%d)",
...
class HotelExtractor:
    ...
    # BUG-SVC-DOM-001-FIX (Build 99): Added groupIds 2,4,5,6,9,10,14,17,19,24
    # corresponding to DOM-expanded categories visible in Booking.com facility block.
    # These groupIds appear in hotels with many amenities (post Selenium expansion).
    # Verified against: FINAL_Audit_Data_Extraction_EN.md (2026-05-05),
    # pruebas/78465__wild & bolz eMotel.html (screenshot evidence).
    _FACILITY_GROUP_MAP: Dict[int, Dict[str, str]] = {
        1:  {"en": "General",              "es": "General",                 "de": "Allgemein",
             "fr": "Général",              "it": "Generale",                "pt": "Geral"},
        2:  {"en": "Private bathroom",     "es": "Baño privado",            "de": "Eigenes Badezimmer",
             "fr": "Salle de bains privative", "it": "Bagno privato",       "pt": "Casa de banho privativa"},
        3:  {"en": "Services",             "es": "Servicios",               "de": "Dienstleistungen",
             "fr": "Services",             "it": "Servizi",                 "pt": "Serviços"},
        4:  {"en": "Bedroom",              "es": "Dormitorio",              "de": "Schlafzimmer",
             "fr": "Chambre",              "it": "Camera da letto",         "pt": "Quarto"},
        5:  {"en": "View",                 "es": "Vistas",                  "de": "Aussicht",
             "fr": "Vue",                  "it": "Vista",                   "pt": "Vista"},
        6:  {"en": "Pets",                 "es": "Mascotas",                "de": "Haustiere",
             "fr": "Animaux de compagnie", "it": "Animali domestici",       "pt": "Animais de estimação"},
        7:  {"en": "Food & Drink",         "es": "Alimentos y bebidas",     "de": "Speisen & Getränke",
             "fr": "Alimentation",         "it": "Cibo e bevande",          "pt": "Comida e bebidas"},
        9:  {"en": "Media & Technology",   "es": "Medios y tecnología",     "de": "Medien & Technologie",
             "fr": "Médias et technologie","it": "Media e tecnologia",      "pt": "Multimédia e tecnologia"},
        10: {"en": "Living Area",          "es": "Zona de estar",           "de": "Wohnbereich",
             "fr": "Coin salon",           "it": "Zona giorno",             "pt": "Zona de estar"},
        ...
        14: {"en": "Safety & security",    "es": "Seguridad",               "de": "Sicherheit",
             "fr": "Sécurité",             "it": "Sicurezza",               "pt": "Segurança"},
        ...
        17: {"en": "Accessibility",        "es": "Accesibilidad",           "de": "Barrierefreiheit",
             "fr": "Accessibilité",        "it": "Accessibilità",           "pt": "Acessibilidade"},
        19: {"en": "Reception services",   "es": "Servicios de recepción",  "de": "Rezeptionsleistungen",
             "fr": "Services de réception","it": "Servizi alla reception",  "pt": "Serviços de recepção"},
        ...
        24: {"en": "Languages spoken",     "es": "Idiomas hablados",        "de": "Gesprochene Sprachen",
             "fr": "Langues parlées",      "it": "Lingue parlate",          "pt": "Idiomas falados"},
    ...
    # =========================================================================
    # BUG-SVC-CAT-STR3-FIX (Build 86) — BUG-SVC-NULL-001-FIX (Build 101):
    # Mapa de inferencia de categoría por substrings.
    # Orden: MÁS ESPECÍFICO PRIMERO. Primera coincidencia gana.
    # Build 101: 7 grupos nuevos añadidos (Languages, Pets, Safety&Security,
    # Accessibility, Reception, Internet descriptive, Parking extended,
    # General extended). "english/inglés" movido a Languages spoken (24).
    # =========================================================================
    _SERVICE_CATEGORY_RULES: List[Tuple[Tuple[str, ...], int]] = [

        # ── Languages spoken (groupId=24) — PRIMERO para no caer en Services ─
        # BUG-SVC-NULL-001-FIX (Build 101): nuevo grupo. Nombres de idioma
        # standalone. "english"/"inglés" movido desde Services (3).
        (("german", "alemán", "alemão", "tedesco", "allemand", "deutsch",
          "french", "français", "francés", "francês", "francese", "französisch",
          "italian", "italiano", "italienne", "italienisch",
          "spanish", "español", "espagnol", "spagnolo", "spanisch", "castilian",
          "portuguese", "português", "portugais", "portugiesisch",
          "dutch", "holandés", "néerlandais", "olandese", "niederländisch",
          "russian", "ruso", "russe", "russo", "russisch",
          "polish", "polaco", "polonais", "polnisch",
          "croatian", "croate", "croata", "croato", "kroatisch",
          "serbian", "serbio", "serbe", "serbo", "serbisch",
          "bosnian", "bosnio", "bosnien", "bosnisch",
          "japanese", "japonés", "japonais", "giapponese", "japanisch",
          "chinese", "chino", "chinois", "cinese", "chinesisch",
          "korean", "coreano", "coréen", "koreanisch",
          "arabic", "árabe", "arabe", "arabisch",
          "czech", "checo", "tchèque", "ceco", "tschechisch",
          "hungarian", "húngaro", "hongrois", "ungherese", "ungarisch",
          "turkish", "turco", "turc", "türkisch",
          "swedish", "sueco", "suédois", "svedese", "schwedisch",
          "norwegian", "noruego", "norvégien", "norvegese", "norwegisch",
          "danish", "danés", "danois", "danese", "dänisch",
          "greek", "griego", "grec", "greco", "griechisch",
          "romanian", "rumano", "roumain", "rumeno", "rumänisch",
          "bulgarian", "búlgaro", "bulgare", "bulgaro", "bulgarisch",
          "english", "inglés", "inglese", "anglais", "englisch", "inglês",
          ), 24),

        # ── Pets (groupId=6) ─────────────────────────────────────────────────
        # BUG-SVC-NULL-001-FIX (Build 101): nuevo grupo.
        (("pets are allowed", "pet-friendly", "pets allowed", "animals allowed",
          "dogs allowed", "cats allowed",
          "admite mascotas", "se admiten mascotas", "mascotas permitidas",
          "mascotas bienvenidas", "animales admitidos",
          "animaux de compagnie", "animaux acceptés", "animaux autorisés",
          "haustiere erlaubt", "tiere erlaubt", "haustiere willkommen",
          "animali ammessi", "animali domestici ammessi",
          "animais de estimação aceites", "aceita animais",
          ), 6),

        # ── Safety & security (groupId=14) ───────────────────────────────────
        # BUG-SVC-NULL-001-FIX (Build 101): nuevo grupo.
        (("fire extinguisher", "extintores", "extincteur", "estintori",
          "feuerlöscher", "extintor",
          "cctv outside", "cctv in common", "cctv fuera", "cctv en zonas",
          "cctv", "security camera", "câmera de segurança", "cámaras de seguridad",
          "cameras de sécurité", "telecamere di sorveglianza", "überwachungskamera",
          "smoke alarm", "smoke detector", "detectores de humo",
          "detector de fumo", "détecteur de fumée", "rilevatore di fumo",
          "rauchmelder", "alarme incendie", "alarma de incendios",
          "key card access", "tarjeta de acceso", "carte d'accès",
          "tessera di accesso", "schlüsselkartenzugang", "cartão de acesso",
          "key pad access", "combination lock",
          "24-hour security", "seguridad 24 horas",
          "fire alarm", "first aid kit",
          ), 14),

        # ── Accessibility (groupId=17) ────────────────────────────────────────
        # BUG-SVC-NULL-001-FIX (Build 101): nuevo grupo.
        (("upper floors accessible by elevator", "upper floors accessible by lift",
          "acceso a pisos superiores en ascensor",
          "acceso a pisos superiores en elevador",
          "étages supérieurs accessibles par ascenseur",
          "piani superiori accessibili tramite ascensore",
          "obere etagen per aufzug erreichbar",
          "wheelchair accessible", "accesible para sillas de ruedas",
          "accessible en fauteuil roulant",
          "rollstuhlgerecht", "cadeira de rodas acessível",
          "disabled parking", "parking adaptado para personas",
          ), 17),

        # ── Reception services (groupId=19) ──────────────────────────────────
        # BUG-SVC-NULL-001-FIX (Build 101): nuevo grupo.
        (("invoice provided", "proporciona factura", "fattura disponibile",
          "facture fournie", "rechnung auf anfrage", "fatura disponível",
          "24-hour front desk", "recepción 24 horas", "réception 24h",
          "reception 24 ore", "24-stunden-rezeption", "receção 24 horas",
          "concierge service", "express check-in", "express check-out",
          ), 19),

        # ── Internet — descripciones largas (groupId=11) ──────────────────────
        # BUG-SVC-NULL-001-FIX (Build 101): cubre textos descriptivos del
        # bloque "Internet" (e.g. "WiFi is available in all areas and is free").
        (("wifi is available", "wi-fi is available",
          "wlan ist in allen bereichen", "wifi disponible en todas",
          "wifi disponibile in tutta", "wifi est disponible",
          "internet is available",
          "free wifi", "gratis wifi", "wifi gratis", "wlan gratis",
          "wifi gratuit", "wifi gratuito", "wi-fi gratuito",
          "wlan kostenlos", "internet gratis", "internet grátis",
          "broadband", "high-speed internet", "fibre internet",
          ), 11),

        # ── Food & Drink (groupId=7) ─────────────────────────────────────────
        (("breakfast", "desayuno", "colazione", "petit-déjeuner",
          "frühstück", "pequeno-almoço",
          "wine", "vino", "champagne", "champán",
          "minibar", "snack bar",
          ), 7),

        # ── Outdoors & View (groupId=13) ─────────────────────────────────────
        (("outdoor furniture", "gartenmöbel", "muebles de exterior",
          "mobilier d'extérieur", "mobilier d\u2019ext\u00e9rieur",
          "mobiliário de exterior", "arredamento da esterno",
          "sea view", "meerblick", "vista mar", "vista mare",
          "vue sur la mer", "vistas al mar",
          "mountain view", "vistas a la montaña", "vue sur la montagne",
          "vista sulla montagna", "bergblick",
          "garden view", "pool view", "city view",
          "view", "vista", "vistas", "aussicht", "vue"), 13),

        # ── Parking (groupId=16) — extended ───────────────────────────────────
        # BUG-SVC-NULL-001-FIX (Build 101): añadidos EV charging y keywords
        # en múltiples idiomas para parking gratuito/privado/accesible.
        (("electric vehicle charging", "ev charging", "charging station",
          "estación de carga de vehículos", "borne de recharge",
          "caricatore per veicoli elettrici", "ladestation für elektrofahrzeuge",
          "free parking", "parking gratis", "gratisparkplatz",
          "parking gratuit", "parcheggio gratuito", "estacionamento gratuito",
          "parking grátis",
          "private parking", "parking privado", "parking privé",
          "parcheggio privato", "privatparkplatz", "estacionamento privado",
          "estacionamento privativo",
          "accessible parking", "parcheggio disabili",
          "on-site parking", "free on-site parking", "secure parking",
          "parking available", "parking is available",
          "estacionamiento", "aparcamiento disponible",
          ), 16),

        # ── Services (groupId=3) — sin idiomas (movidos a Languages spoken) ──
        # BUG-SVC-NULL-001-FIX (Build 101): eliminados "english","inglés" etc.
        (("wake-up service", "servicio de despertador", "sveglia telefonica",
          "service de réveil", "weckservice", "serviço de despertar",
          "shared lounge", "zona tv", "salón de uso compartido",
          "zona tv / salón", "tv lounge", "lounge area",
          "salon commun", "salon partagé", "salotto comune", "gemeinschaftslounge",
          "lounge compartilhada",
          "vending machine", "máquina expendedora", "distributeur automatique",
          "distributore automatico", "verkaufsautomat",
        ...
        # ── General (groupId=1) — extended ───────────────────────────────────
        # BUG-SVC-NULL-001-FIX (Build 101): añadidos lift/elevator, hardwood
        # floors, heating, designated smoking area.
        (("non-smoking", "prohibido fumar", "struttura interamente non fumatori",
          "établissement entièrement non-fumeurs", "nichtraucherunterkunft",
          "proibido fumar", "sin humo", "habitaciones sin humo",
          "chambres non-fumeurs", "camere non fumatori",
          "hypoallergenic", "hipoalergénico", "antialérgico",
          "allergikerfreundlich", "chambre anti-allergie", "soluzioni anallergiche",
          "private entrance", "entrada privada", "ingresso indipendente",
          "entrée privée", "privater eingang",
          "entire unit", "toda la unidad", "intera unità",
          "logement entièrement", "gesamte wohneinheit", "toda a unidade",
          "tile/marble", "fliesen-/marmor", "fliesen/marmor",
          "pavimento in marmo", "sol carrelé", "suelo de baldosa",
          "piso em azulejo", "marble floor",
          "carpeted", "teppichboden", "suelo de moqueta", "piso em carpete",
          "moquette",
          "hardwood or parquet floors", "hardwood floor", "parquet floor",
          "suelo de madera o parquet", "suelo de madera", "parquet",
          "holzboden", "plancher en bois", "pavimento in parquet",
          "heating", "calefacción", "chauffage", "riscaldamento", "heizung",
          "aquecimento", "central heating",
          "lift", "elevator", "ascensor", "aufzug", "ascensore",
          "élévateur", "elevador",
          "designated smoking area", "zona para fumadores",
          "espace fumeur", "area fumatori",
          ), 1),

        # ── Room Amenities (groupId=15) — catch-all de habitación ────────────
        (("bathrobe", "albornoz", "accappatoio", "peignoir", "bademantel",
          "roupão de banho",
          "bidet", "bidé",
          "shower", "ducha", "doccia", "douche", "dusche", "duche",
          "bañera", "baignoire", "badewanne", "banheira",
          "private bathroom", "baño privado", "bagno privato",
          "salle de bains privative", "eigenes badezimmer",
          "casa de banho privativa",
          "hairdryer", "secador de pelo", "secador de cabelo",
          "asciugacapelli", "sèche-cheveux", "haartrockner",
          "linen", "ropa de cama", "biancheria da letto", "linge de maison",
          "bettwäsche", "roupa de cama",
          "wardrobe", "armario", "armadio", "armoire", "roupeiro",
          "coffee machine", "cafetera", "macchina da caffè",
          "machine à café", "kaffeemaschine", "máquina de café",
          "electric kettle", "bollitore elettrico", "bouilloire électrique",
          "wasserkocher", "hervidor eléctrico", "chaleira elétrica",
          "flat-screen tv", "tv de pantalla plana", "tv a schermo piatto",
          "télévision à écran plat", "flachbild-tv", "televisão de ecrã plano",
          "télévision", "televisão", "televisor",
          "satellite channels", "canales vía satélite", "canali satellitari",
          "chaînes satellite", "satellitenempfang", "canais por satélite",
          "telephone", "teléfono", "telefono", "téléphone", "telefon", "telefone",
          "desk", "bureau", "scrivania", "schreibtisch", "escritorio", "secretária",
          "sofa", "sofá", "canapé", "divano",
          "seating area", "zona de estar", "coin salon", "sitzbereich",
          "zona soggiorno", "área de estar",
          "safety deposit box", "caja fuerte", "cassaforte",
          "coffre-fort", "cofre",
          "ironing facilities", "bügeleinrichtungen", "ferro e asse da stiro",
          "matériel de repassage", "utensilios de planchado",
          "comodidades de engomadoria",
          "fan", "ventilador", "ventilatore", "ventilateur", "ventilator",
          "ventoinha",
          "air conditioning", "aire acondicionado", "aria condizionata",
          "climatisation", "klimaanlage", "ar condicionado",
          "toilet paper", "papel higiénico", "carta igienica",
          "papier toilette", "toilettenpapier",
          "toilet", "toilettes", "inodoro",
          "free toiletries", "artículos de aseo gratis",
          "articles de toilette gratuits", "produits de toilette",
          "kostenlose pflegeprodukte", "produtos de higiene pessoal gratuitos",
          "prodotti da bagno in omaggio",
          "towels", "toallas", "asciugamani", "serviettes",
          "handtücher", "toalhas",
          "extra long beds", "camas extralargas", "letti lunghi",
          "très grands lits", "extralange betten", "camas extralongas",
          "socket near the bed", "enchufe cerca de la cama",
          "presa elettrica vicino al letto", "prise près du lit",
          "steckdose in bettnähe", "tomada perto da cama",
          "drying rack for clothing", "tendedero", "stendibiancheria",
          "étendoir", "wäscheständer", "estendal para roupa",
          "safe", "cofre",
          "entire unit located on ground floor",
          "hypoallergen", "antialérg",
          "schrank",), 15),
    ]
    ...
    def _infer_service_category(self, text: str, lang_key: str) -> str:
        """
        BUG-SVC-CAT-STR3-FIX (Build 86):
        Infiere la categoría de un servicio usando _SERVICE_CATEGORY_RULES
        cuando la fuente DOM no proporciona grupo (Strategy 1-flat, 2, 3).

        Busca subcadenas (case-insensitive) del texto del servicio contra
        cada tupla de keywords. La primera regla que coincide determina el
        groupId, que se traduce con _FACILITY_GROUP_MAP[groupId][lang_key].

        Args:
            text     : texto del servicio tal como aparece en el DOM
            lang_key : código ISO-639-1 de 2 letras ("en","es","de","fr","it","pt")

        Returns:
            Cadena de categoría traducida (e.g. "Room Amenities", "Comodidades")
            o "" si ninguna regla coincide.
        """
        t = text.lower()
        for keywords, gid in self._SERVICE_CATEGORY_RULES:
            for kw in keywords:
                if kw in t:
                    group_langs = self._FACILITY_GROUP_MAP.get(gid, {})
                    return (
    ...
    def _extract_all_services(self) -> List[Dict[str, str]]:
        ...
        seen: set = set()
        # BUG-SVC-CAT-STR3-FIX (Build 86): lang_key necesario para
        # _infer_service_category() en estrategias sin categoría DOM.
        ...
        def _add(text: str, category: str = "") -> None:
            ...
            if (
                ...
                seen.add(t)
                # BUG-SVC-NULL-001-FIX (Build 101): when the DOM strategy does not
                # provide a category heading (category=""), apply keyword-based
                # inference as automatic fallback BEFORE persisting the item.
        ...
        # BUG-SVC-DOM-001-FIX (Build 99): DOM strategies now run BEFORE Apollo
        # JSON (Strategy 0 moved to position 3 as fallback). Root cause of
        # catastrophic data loss (87% for rich hotels like wild & bolz eMotel):
        #   - Strategy 0 (Apollo JSON) fired first with threshold >=5.
        #   - Apollo JSON only contains 5-20 BaseFacility objects per hotel
        #     (hotel-level subset). The rendered Booking.com facilities page
        #     shows 17+ sections and 30-45 items per hotel.
        #   - Returning early from Strategy 0 prevented DOM strategies from
        #     running on the EXPANDED page_source (post Selenium click).
        # Fix: DOM strategies (1, 1.5, 2) are PRIMARY. Apollo JSON is now
        # Strategy 3 fallback (runs only if all DOM strategies yield 0 items).
        # This matches the audit finding: "Change the scraper to extract from
        # the VISIBLE HTML facility block instead of the embedded JSON."
        # Reference: FINAL_Audit_Data_Extraction_EN.md § 8.1 (2026-05-05).
            ...
            if section:
                ...
                if fac_groups_in_section:
                    ...
                    if services:
                        ...
                        return services
                # Fallback: flat list — inferir categoría por keyword
                # BUG-SVC-CAT-STR3-FIX (Build 86): sin facility-group,
                # se infiere categoría mediante _infer_service_category().
                for li in section.find_all("li"):
                    svc_text = li.get_text(" ", strip=True)
                    inferred = self._infer_service_category(svc_text, lang_key)
                    _add(svc_text, category=inferred)
                if services:
                    logger.debug(
        ...
        try:
            ...
            for group in groups:
                for li in group.find_all("li"):
                    # BUG-SVC-CAT-STR3-FIX (Build 86): strategy2 no extrae
                    # grupo del DOM — inferir categoría por keyword.
                    svc_text = li.get_text(" ", strip=True)
                    inferred = self._infer_service_category(svc_text, lang_key)
                    _add(svc_text, category=inferred)
            if services:
                logger.debug(
                    "AllServices: strategy2 (facilit groups+inferred_cat): %d items lang=%s",
                    len(services), self.language,
                )
                return services
        except Exception as exc:
        ...
        # ── Estrategia 3 (ex-0): Apollo/GraphQL JSON cache — FALLBACK ─────────
        # BUG-SVC-DOM-001-FIX (Build 99): Movido desde posicion primaria a
        # fallback de ultimo recurso. Se ejecuta SOLO si las estrategias DOM
        # (1, 1.5, 2) no produjeron resultados, indicando que Selenium no pudo
        # expandir el bloque de instalaciones. El umbral >=5 se conserva.
        try:
            json_services = self._extract_services_from_json()
            if json_services and len(json_services) >= 5:
                logger.debug(
                    "AllServices: strategy3-json (Apollo JSON fallback): %d items lang=%s",
            ...
            logger.debug(
                "AllServices strategy3-json (Apollo) failed lang=%s: %s",
                self.language, exc,
            )

        # ── Estrategia 4: fallback popular-wrapper (last resort) ─────────────
        # BUG-SVC-DOM-001-FIX (Build 99): renombrada de "strategy3" a "strategy4".
        # Solo se ejecuta si DOM y JSON producen 0 items (bloqueo completo).
        try:
            wrapper = self.soup.find(
                attrs={"data-testid": "property-most-popular-facilities-wrapper"}
            )
            if wrapper:
                for li in wrapper.find_all("li"):
                    # BUG-SVC-CAT-STR3-FIX (Build 86):
                    # popular-wrapper es lista plana sin grupos DOM → service_category
                    # siempre quedaba NULL. Se infiere la categoría por keyword lookup.
                    svc_text = li.get_text(" ", strip=True)
                    inferred = self._infer_service_category(svc_text, lang_key)
                    _add(svc_text, category=inferred)
                if services:
                    logger.debug(
                        "AllServices: strategy4 (popular fallback+inferred_cat): %d items lang=%s",
                        len(services), self.language,
                    )
        except Exception as exc:
    ...
    def _extract_services_from_json(self) -> List[Dict[str, str]]:
        ...
        # BUG-SVC-NULL-001-FIX (Build 101): append GenericFacilityHighlight objects
        # (no groupId — infer category via keyword lookup).
        for gh in generic_highlights:
            gh_title = gh["title"]
            if gh_title not in seen_titles:
                seen_titles.add(gh_title)
                inferred_cat = self._infer_service_category(gh_title, lang_key)
                result.append({"service": gh_title, "service_category": inferred_cat})
