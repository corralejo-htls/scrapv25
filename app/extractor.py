"""
extractor.py — BookingScraper Pro v6.0.0 build 60
==================================================
Cambios v60:
...
class HotelExtractor:
    ...
    def _extract_policies(self) -> List[Dict[str, str]]:
        ...
        Booking.com muestra políticas en el bloque data-testid="property-section--policies"
        o en la sección de reglas de la casa. Cada política tiene un título (policy_name)
        y un texto de detalle (policy_details).

        Estructura DOM típica (React):
          <div data-testid="property-section--policies">
            <div class="...">
              <h3 | strong | span class="title">Check-in</h3>
              <p | div class="...">From 15:00 ...</p>
            </div>
            ...
          </div>

        Retorna: lista de dicts [{policy_name: str, policy_details: str}, ...]
        """
        policies: List[Dict[str, str]] = []

        # Estrategia 1: bloque data-testid específico de políticas
        try:
            ...
            if section:
                ...
                if policies:
                    logger.debug("Policies extracted via property-section--policies: %d", len(policies))
                    return policies
        except Exception as exc:
            logger.debug("policies section extraction failed: %s", exc)

        # Estrategia 2: id/class legacy de reglas del alojamiento
        try:
            for selector in [
                ...
                if section:
                    policies = self._parse_policy_section(section)
                    if policies:
                        logger.debug("Policies extracted via legacy selector: %d", len(policies))
                        return policies
        except Exception as exc:
            logger.debug("policies legacy extraction failed: %s", exc)

        # Estrategia 3: buscar pares label/value en el DOM genérico
        try:
            ...
            if policies:
                logger.debug("Policies extracted via generic data-testid: %d", len(policies))
        except Exception as exc:
    ...
    def _extract_legal(self) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            # ── Paso 2: recopilar párrafos del cuerpo ────────────────────────────
            # FIX-LEGAL-003 (v55): Búsqueda también recursiva para capturar
            # contenido en DOMs con divs intermedios anidados.
            paragraphs = section.find_all(["p", "div"], recursive=False)
            legal_info = ""
            legal_details = ""

            # Índice del primer párrafo útil (salta el que contiene el título si
            # fue detectado como <p>)
            body_started = False
            for para in paragraphs:
                # FIX-LEGAL-001: si el título vino de un <p>, saltarlo
                if title_from_paragraph and title_el is not None:
                    if para is title_el or para == title_el:
                        continue
                    # También saltarlo si el título_el está DENTRO de este párrafo
                ...
                # FIX-LEGAL-003 (v55): Si el texto del párrafo es idéntico al título,
                # saltarlo — es una duplicación del título, no contenido real
            ...
            # BUG-LEGAL-SELECTOR (v56): extracción de contenido por siblings del h2.
            # La estructura real es <div class="k2-hp--cpc_regulation"><div><h2/><div.body/></div></div>
            # recursive=False sobre el contenedor externo solo encuentra un <div> hijo,
            # que a su vez contiene el h2 y el cuerpo. Se buscan siblings del título.
            if not legal_info and title_el:
                try:
            ...
            # FIX-LEGAL-003 (v55): Si no se encontraron párrafos con recursive=False,
            # intentar búsqueda recursiva para capturar contenido en divs anidados
            if not legal_info:
                ...
                for el in all_text_els:
                    ...
                    if not el_text or len(el_text) < 10:
                        continue
                    # Saltar si es el mismo texto que el título
                    if legal_title and el_text.strip() == legal_title.strip():
                        continue
                    # Saltar si es un contenedor padre que contiene sub-elementos ya procesados
                    if el.find(["p", "div"]):
                        continue
                    legal_info = el_text
                    break

            # Fallback: si no se encontraron párrafos separados,
            # tomar el texto del bloque completo excluyendo el título
            if not legal_info:
                soup_section = section.__copy__() if hasattr(section, "__copy__") else section
                if title_el:
                    # Clonar para no mutar el DOM original
                    import copy as _copy
                    section_copy = _copy.copy(section)
                    for t in section_copy.find_all(
                        ["h2", "h3", "h4", "strong", "p"] if title_from_paragraph else ["h2", "h3", "h4", "strong"]
                    ):
    ...
    def _extract_all_services(self) -> List[str]:
        """
        STRUCT-014 (v57 — BUG-ALL-SERVICES-001):
        Extrae la lista COMPLETA de servicios/instalaciones del hotel.

        Estrategias (en orden de prioridad):
          0. Apollo/GraphQL JSON cache embebido: fuente más completa,
             idioma-agnóstica en IDs, traducida por el mapa copyTags.
          1. DOM selector 'property-section--facilities' (legacy, no presente
             en HTML actual de Booking.com — se mantiene por compatibilidad).
          2. DOM selector con 'facilit' (FIX: antes era 'facility', no matcheaba
             'facilities-wrapper' — corregido a substring "facilit").
          3. Fallback: 'property-most-popular-facilities-wrapper' (~10 items).

        Retorna: lista de strings (un servicio por elemento), deduplicados.
        """
        services: List[str] = []
        seen: set = set()
        ...
        try:
            ...
            # BUG-SVC-002-FIX: threshold raised from >= 5 to >= 30.
            # The Apollo JSON cache in the initial DOM only contains ~6-10
            # "popular" facilities — the same partial set visible without
            # expanding the full list.  With >= 5 the method returned that
            # truncated list before Strategies 1-3 could act on the expanded
            # DOM (populated by _expand_facilities() in scraper.py).
            # >= 30 ensures Strategy 0 only short-circuits when the JSON cache
            # actually contains a meaningfully complete set of facilities.
            if json_services and len(json_services) >= 30:
                logger.debug(
                    "AllServices: strategy0 (JSON cache): %d items lang=%s",
        ...
        # ── Estrategia 1: sección facilities completa (DOM legacy) ──────────
        try:
            section = self.soup.find(
            ...
            if section:
                ...
                if services:
                    logger.debug(
                        "AllServices: strategy1 (section): %d items lang=%s",
        ...
        # ── Estrategia 1.5: facility-group (AUDIT-HTML-001) ─────────────────
        # AUDIT-HTML-001-FIX: data-testid="facility-group" es el contenedor
        # semántico de cada grupo de facilidades en el DOM expandido (tras
        # clic en "See all N facilities", BUG-SVC-001-FIX).  Es más preciso
        # que el regex "facilit" (Strategy 2) porque excluye elementos de icono
        # (facility-group-icon) y wrappers de conteo, evitando texto duplicado.
        #
        # Estructura DOM verificada (Booking.com React):
        #   <div data-testid="facility-group">
        #     <div data-testid="facility-group-icon">...</div>
        #     <div>
        #       <div>General</div>          ← nombre de categoría
        #       <ul><li>WiFi</li>...</ul>   ← ítems de servicio
        #     </div>
        #   </div>
        try:
            ...
            if fac_groups:
                for group in fac_groups:
                    # Excluir sub-elementos de icono para no capturar texto de SVG
        ...
        # ANTES: re.compile(r"facility") → no matcheaba "facilities-wrapper"
        # AHORA: re.compile(r"facilit")  → captura "facility-icon", "facilities-*"
        try:
    ...
    def _extract_services_from_json(self) -> List[str]:
        """
        NUEVA (v57 — BUG-ALL-SERVICES-001):
        Extrae servicios del Apollo/GraphQL cache embebido en
        <script type="application/json"> presente en el HTML de Booking.com.

        Estructura verificada en HTML real (EN/ES/DE/IT):
          - Script con key 'copyTags.hotelfacility/name.items': dict {id: name}
            con ~372 entradas (mapa de nombres según idioma del request).
          - Scripts con keys 'BaseFacility:{...}': objetos del hotel específico
            con 'id', 'groupId', 'slug'.

        El método cruza los IDs de BaseFacility del hotel con el mapa de nombres
        para obtener los nombres traducidos en el idioma correcto.

        Retorna: lista de strings deduplicados, vacía si no hay datos suficientes.
        """
        facility_names: Dict[str, str] = {}
        base_facility_ids: List[int] = []
        seen_ids: set = set()
        ...
        for script in self.soup.find_all("script", type="application/json"):
            ...
            # Mapa de nombres traducidos (copyTags)
            if "copyTags" in data and not facility_names:
                names_block = (
                    data["copyTags"]
                    .get("hotelfacility/name", {})
                    .get("items", {})
                )
                if isinstance(names_block, dict):
                    facility_names.update(names_block)

            # BaseFacility objects del hotel (Apollo normalized cache)
            for key, val in data.items():
                if (
                    key.startswith("BaseFacility:")
                    and isinstance(val, dict)
                ):
                    fid = val.get("id")
                    if fid is not None and fid not in seen_ids:
                        seen_ids.add(fid)
                        base_facility_ids.append(int(fid))

        if not facility_names or not base_facility_ids:
            logger.debug(
                "AllServices JSON: insufficient data lang=%s "
                "(names=%d, base_facilities=%d)",
                self.language, len(facility_names), len(base_facility_ids),
            )
            return []

        services: List[str] = []
        seen_names: set = set()
        for fid in base_facility_ids:
            name = facility_names.get(str(fid), "").strip()
            if name and name not in seen_names:
                seen_names.add(name)
                services.append(name)

        logger.debug(
            "AllServices JSON: resolved %d/%d facilities lang=%s",
            len(services), len(base_facility_ids), self.language,
        )
        return services
    ...
    def _extract_room_types(self) -> List[Dict[str, Any]]:
        """
        STRUCT-018 (Audit-HTML-001):
        Extrae los tipos de habitación disponibles del hotel.

        Estructura DOM verificada (Booking.com React):
          <div data-testid="room-block">
            <a  data-testid="room-type-title">Deluxe Double Room</a>
            <div data-testid="room-description">Spacious room with sea view...</div>
            <div data-testid="room-facilities">
              <span>Free WiFi</span>
        ...
        Nota sobre ocupación: los campos occupancy-max-guests / occupancy-max-children
        usan íconos SVG cuyo recuento depende del viewport renderizado.
        Selenium con window.stop() puede cortar la carga de SVGs → se omiten
        para evitar recuentos incorrectos (siempre 0).

        Retorna: List[Dict] con claves:
          - name        : str — nombre del tipo de habitación
          - description : str — descripción larga (puede ser vacía)
          - facilities  : List[str] — facilidades específicas de la habitación
        """
        ...
        try:
            ...
            if not blocks:
                logger.debug("RoomTypes: no room-block elements found lang=%s", self.language)
                return rooms
            ...
            logger.debug(
                "RoomTypes: extracted %d room types lang=%s", len(rooms), self.language
            )
        except Exception as exc:
            logger.debug("RoomTypes extraction failed lang=%s: %s", self.language, exc)
    ...
    def _extract_nearby_places(self) -> List[Dict[str, str]]:
        """
        STRUCT-022 (v76): Extrae los lugares de interés cercanos al hotel.

        Estructura DOM verificada (Booking.com React):
          <div data-testid="location-highlight">
            <h3>Aeropuerto Internacional</h3>
            <span class="...distance...">2.1 km</span>
            <svg data-testid="icon-airport">...</svg>
          </div>
        ...
        try:
            ...
            if not highlights:
                logger.debug("NearbyPlaces: no location-highlight elements lang=%s", self.language)
                return places
            ...
            for item in highlights:
                ...
                # Distance: span with class containing "distance"
                dist_el = item.find("span", class_=re.compile(r"distance", re.I))
                if not dist_el:
                    # Fallback: any span with numeric content + unit
                ...
                # Category: from SVG data-testid attribute
                category = ""
            ...
            logger.debug("NearbyPlaces: extracted %d places lang=%s", len(places), self.language)

        except Exception as exc:
            logger.debug("NearbyPlaces extraction failed lang=%s: %s", self.language, exc)
