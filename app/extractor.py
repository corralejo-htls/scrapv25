"""
extractor.py — BookingScraper Pro v6.0.0 build 56
==================================================
Cambios v55:
...
Cambios v52:
  ...
  STRUCT-005 : _extract_amenities() ahora usa la sección COMPLETA de instalaciones
...
class HotelExtractor:
    ...
    def extract_all(self) -> Dict[str, Any]:
        ...
        v51 — cambios en el dict retornado:
          - 'amenities'        : lista COMPLETA de instalaciones (→ hotels_amenities)
        ...
        data: Dict[str, Any] = {
            ...
            "amenities": self._extract_amenities(),                  # STRUCT-005: lista completa
    ...
    def _extract_popular_services(self) -> List[str]:
        ...
        Se almacena en hotels_popular_services (tabla independiente de hotels_amenities).

        Anteriormente era la Estrategia 1 de _extract_amenities() (v50).
        """
        popular: List[str] = []
        try:
    ...
    def _extract_amenities(self) -> List[str]:
        """
        STRUCT-005: Extrae la lista COMPLETA de amenidades del hotel.

        Estrategia principal: sección completa data-testid="property-section--facilities".
        Fallback 1: popular-facilities-wrapper (si no hay sección completa).
        Fallback 2: selector legacy data-testid="facility-list-item".

        Nota: amenidades duplicadas son eliminadas preservando orden de aparición.
        """
        amenities: List[str] = []
        seen: set = set()
        ...
        # Estrategia 1: sección completa de instalaciones (lista larga)
        try:
            ...
            if section:
                for span in section.find_all("span"):
                    ...
                    if (
                        ...
                        and len(text) < 120
                        and not re.match(
                            r"(Ver |See |Alle |Voir )(los |all |les |alle |\d)", text, re.I
                        )
                    ):
                        _add(text)
                if amenities:
                    logger.debug(
                        "Amenities extracted via facilities section: %d", len(amenities)
                    )
                    return amenities
        except Exception as exc:
            logger.debug("amenities facilities-section extraction failed: %s", exc)

        # Fallback 1: popular-facilities-wrapper (subset, mejor que nada)
        try:
            wrapper = self.soup.find(
                attrs={"data-testid": "property-most-popular-facilities-wrapper"}
            )
            if wrapper:
                ...
                if amenities:
                    logger.debug(
                        "Amenities extracted via popular-wrapper fallback: %d", len(amenities)
                    )
                    return amenities
        except Exception as exc:
            logger.debug("amenities popular-wrapper fallback failed: %s", exc)

        # Fallback 2: selector legacy
        try:
            ...
            if amenities:
                logger.debug(
                    "Amenities extracted via legacy facility-list-item: %d", len(amenities)
                )
        except Exception as exc:
    ...
    def _extract_fine_print(self) -> Optional[str]:
        ...
        try:
            # Extraer y sanitizar el HTML del bloque
            html_raw = str(section)
            sanitized = self._sanitize_html_fragment(html_raw)

            if not sanitized or len(sanitized.strip()) < 20:
                logger.debug(
                    "FinePrint: sanitized HTML too short (%d chars) for lang=%s",
                    len(sanitized or ""), self.language,
                )
                return None

            logger.debug(
                "FinePrint extracted for lang=%s: %d chars (sanitized)",
                self.language, len(sanitized),
            )
            return sanitized
    ...
    def _extract_property_highlights(self) -> Optional[str]:
        """
        STRUCT-017 (v53): Extrae el bloque "Property Highlights" del hotel.

        El HTML se sanitiza antes de almacenarse:
          - <svg> eliminados completamente (con todo su contenido interno).
          - <img>, <picture>, <source>, <canvas>, <video> eliminados.
          - Atributos HTML eliminados de TODAS las etiquetas restantes:
            class, id, height, width, aria-hidden, focusable, data-*, style,
            role, tabindex, etc.
          - Estructura HTML preservada: <div>, <ul>, <li>, <p>, <span>, etc.
        ...
        Resultado sanitizado:
          <div><ul><li><div>Beach access</div></li>
          <li><div><p>Breakfast included</p><p>American breakfast</p></div></li></ul></div>

        Estrategias de localización:
          1. data-testid="property-highlights" o variantes
          2. class pattern: property-highlights, propertyHighlight
          3. Heading con keyword "highlights" por idioma

        Retorna: HTML sanitizado como string, o None si no se encuentra / está vacío.
        """
        # ── Selectores data-testid y class/id ─────────────────────────────────
        ...
        # ── Keywords de heading por idioma ────────────────────────────────────
        ...
        # Intento 2: buscar por keyword de heading
        if not section:
            lang_key = self.language[:2].lower()
            keywords = (
            ...
            for kw in keywords:
                try:
                    ...
                    if el:
                        parent = el.parent
                        # Verificar que el padre contiene lista de items
        ...
        try:
            ...
            if not sanitized or len(sanitized.strip()) < 10:
                logger.debug(
                    "PropertyHighlights: sanitized HTML too short (%d chars) lang=%s",
                    len(sanitized or ""), self.language,
                )
                return None

            logger.debug(
                "PropertyHighlights extracted for lang=%s: %d chars (sanitized)",
                self.language, len(sanitized),
            )
            return sanitized
