...
from app.models import (
    Base, Hotel, HotelAmenity, HotelDescription, HotelLegal,
...
def get_hotel(hotel_id: str) -> Dict[str, Any]:
    ...
    with get_db() as session:
        ...
        # STRUCT-005: amenities from hotels_amenities
        amenity_rows = (
            session.query(HotelAmenity)
            .filter_by(hotel_id=hid, language=row.language)
            .order_by(HotelAmenity.id)
            .all()
        )
        ...
        return {
            ...
            # STRUCT-005
            "amenities": [a.amenity for a in amenity_rows],
            # STRUCT-008
