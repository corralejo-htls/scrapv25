...
CAMBIOS v4.0 [COMPLETENESS API]:
  ...
  PREREQUISITO: install_clean_v31.sql ejecutado en PostgreSQL (instalación limpia).
  # [FIX MED-40-003] Referencia corregida — migration_v2_url_language_status.sql
  # no existe. Toda la definición de url_language_status está en install_clean_v31.sql.
...
# [FIX CRIT-006] Replace threading.Lock with Redis-backed distributed lock.
# threading.Lock() is process-local: in multi-process uvicorn deployments,
# two processes can both pass the acquire() check simultaneously.
# Redis SET NX (set-if-not-exists) with TTL is the correct cross-process lock.
# Fallback: in-memory threading.Lock for single-process mode (Redis unavailable).
_force_now_lock_memory = threading.Lock()
_FORCE_NOW_LOCK_TTL    = 30  # seconds — TTL prevents deadlock on process crash


def _acquire_force_now_lock() -> bool:
    """
    [FIX CRIT-006] Acquire the force-now distributed lock.
    Tries Redis NX first; falls back to in-memory threading.Lock.
    ...
    try:
        ...
        if r is not None:
            ...
            result = r.set("bsp:force_now_lock", "1", nx=True, ex=_FORCE_NOW_LOCK_TTL)
            return bool(result)
    except Exception:
        pass  # Redis unavailable — fall through to memory lock
    return _force_now_lock_memory.acquire(blocking=False)


def _release_force_now_lock() -> None:
    """[FIX CRIT-006] Release the force-now lock in both Redis and in-memory."""
    try:
        from app.scraper_service import _get_redis
        r = _get_redis()
        if r is not None:
            r.delete("bsp:force_now_lock")
    except Exception:
        pass
    try:
        _force_now_lock_memory.release()
    except RuntimeError:
        pass  # Was not held — ignore
...
def scraping_force_now(
    ...
    [FIX CRIT-006] Protegido con lock Redis distribuido — seguro en multi-proceso.
    """
    # [FIX CRIT-006] Redis-backed distributed lock prevents concurrent force-now
