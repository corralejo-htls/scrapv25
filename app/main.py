"""
main.py — BookingScraper Pro v6.0.0 build 127
BUILD 127 — Forensic audit Build 126: L-01 constant-time API key comparison
  (secrets.compare_digest) + empty-key 503 guard; H-01 config-driven bind
  (API_HOST/API_PORT) so non-loopback exposure is fail-closed at startup.
Fixes applied:
...
async def lifespan(app: FastAPI):
    ...
    # ── Build 98: Language configuration — single source of truth ─────────────
    # Carga y valida languages.json contra ENABLED_LANGUAGES.
    # BLOQUEA el arranque si cualquier idioma habilitado tiene cobertura
    # incompleta en cualquier sección del JSON (lanza LanguageConfigError).
    try:
        from app.language_config import init_language_config, LanguageConfigError
        enabled_langs = [
            lg.strip() for lg in cfg.ENABLED_LANGUAGES.split(",") if lg.strip()
        ]
        # 'en' siempre primero (contrato _API_.md)
        if "en" not in enabled_langs:
            enabled_langs.insert(0, "en")
        elif enabled_langs[0] != "en":
            enabled_langs = ["en"] + [lg for lg in enabled_langs if lg != "en"]

        init_language_config(
            config_file=cfg.LANGUAGES_CONFIG_FILE,
            enabled_langs=enabled_langs,
        )
        logger.info(
            "LanguageConfig loaded: %d languages active %s",
            len(enabled_langs), enabled_langs,
        )
    except LanguageConfigError as _lang_err:
        # Error crítico — no se puede arrancar sin configuración de idiomas completa
        logger.critical(
            "STARTUP BLOCKED — Language configuration incomplete:\n%s", _lang_err
        )
        raise SystemExit(1) from _lang_err
    except Exception as _lang_exc:
        logger.critical(
            "STARTUP BLOCKED — Failed to load language configuration: %s", _lang_exc
        )
        raise SystemExit(1) from _lang_exc
    yield
    dispose_engine()
...
def _check_api_key(request: Request) -> None:
    # BUILD 127 — L-01/M-01 (forensic audit Build 126):
    #   * Constant-time comparison via secrets.compare_digest (no timing oracle).
    #   * Defense-in-depth: reject when REQUIRE_API_KEY=True but API_KEY is empty,
    #     so a bare "Authorization: Bearer " header can never authenticate even if
    #     the config validator were bypassed. (Startup validator in config.py is
    #     the primary guard; this is the second layer.)
    cfg = get_settings()
    if not cfg.REQUIRE_API_KEY:
        return
    configured = cfg.API_KEY or ""
    if not configured:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="API authentication required but no API_KEY is configured.",
        )
...
# ---------------------------------------------------------------------------
# API Export endpoints (Build 93 — STRUCT-EXPORT-004)
#
# ENABLED_LANGUAGES como fuente de verdad para idiomas de exportación.
# EXT_API_DEFAULT_LANGUAGES ahora es opcional y solo para restringir subconjuntos.
# El panel UI muestra checkboxes dinámicos cargados desde /export/languages.
#
# Menú en /docs (tag "Export"):
#   GET  /export/ui                → panel HTML de exportación para el usuario
#   GET  /export/languages         → idiomas scrapeados (ENABLED_LANGUAGES)
...
@app.get(
    ...
    # SEC-UI-001-FIX (Build 120): /export/ui ahora requiere Bearer token si
    # REQUIRE_API_KEY=True. Anteriormente era accesible sin autenticación,
    # exponiendo el panel de exportación a cualquier cliente en red local.
    # El panel llama internamente a los endpoints /export/*, que también
    # están protegidos individualmente — esta capa añade defensa en profundidad.
    dependencies=[Depends(_check_api_key)],
)
def export_ui_page() -> HTMLResponse:
    ...
    Acceso: **http://localhost:8000/export/ui**  (requiere Bearer token si REQUIRE_API_KEY=True)
    ...
    SEC-UI-001-FIX (Build 120): protegido con Bearer token al igual que
    el resto de endpoints de la API.
    """
    return HTMLResponse(content=build_export_ui_html(), status_code=200)


# ── GET /export/languages ─────────────────────────────────────────────────────

@app.get(
    "/export/languages",
    tags=["Export"],
    summary="Idiomas configurados para scraping (ENABLED_LANGUAGES)",
    # SEC-UI-001-FIX (Build 120): añadida protección Bearer token.
    dependencies=[Depends(_check_api_key)],
)
def export_languages() -> Dict[str, Any]:
    """
    Devuelve los idiomas configurados en `ENABLED_LANGUAGES` (los que el
    scraper realmente procesa) y el override de exportación
    `EXT_API_DEFAULT_LANGUAGES` si está configurado.

    El panel UI usa este endpoint para construir los checkboxes de idiomas
    dinámicamente, sin depender de valores hardcodeados.

    **Respuesta:**
    - `scraped_languages` — lista ISO 639-1 de todos los idiomas scrapeados
    - `export_languages`  — idiomas que se incluirán en el payload de exportación
      (= scraped_languages si EXT_API_DEFAULT_LANGUAGES está vacío)
    - `restricted`        — true si EXT_API_DEFAULT_LANGUAGES restringe a un subconjunto
    - `source`            — origen del valor export_languages: 'ENABLED_LANGUAGES' | 'EXT_API_DEFAULT_LANGUAGES'
    """
    cfg = get_settings()
    scraped = _get_enabled_languages(cfg)
    export  = _get_export_languages(cfg)
    ext_override = getattr(cfg, "EXT_API_DEFAULT_LANGUAGES", "").strip()
    return {
        "scraped_languages": scraped,
        "export_languages":  export,
        "restricted":        bool(ext_override) and set(export) != set(scraped),
        "source":            "EXT_API_DEFAULT_LANGUAGES" if ext_override else "ENABLED_LANGUAGES",
        "note": (
            "EXT_API_DEFAULT_LANGUAGES no configurado — usando ENABLED_LANGUAGES"
            if not ext_override
            else f"EXT_API_DEFAULT_LANGUAGES='{ext_override}' sobreescribe ENABLED_LANGUAGES"
        ),
    }
...
def export_resolve(
    ...
    **Regla de negocio:** solo los hoteles con `is_complete=true` son
    ...
    """
    cfg       = get_settings()
    all_langs = _get_enabled_languages(cfg)  # fuente de verdad: ENABLED_LANGUAGES
    ...
    with get_db() as session:
        if mode == "all":
            ...
            return {"hotels": [], "total": 0, "scraped_languages": all_langs}
...
def _get_enabled_languages(cfg) -> List[str]:
    """
    Devuelve la lista de idiomas de ENABLED_LANGUAGES (fuente de verdad).
    'en' siempre en primer lugar (_API_.md lo requiere).
    """
    raw = getattr(cfg, "ENABLED_LANGUAGES", "en,es,de,it,fr,pt")
    langs = [ln.strip() for ln in raw.split(",") if ln.strip()]
    # Garantizar 'en' en primera posición
    if "en" in langs:
...
def _get_export_languages(cfg) -> List[str]:
    """
    Devuelve los idiomas que se incluirán en el payload de exportación.

    Lógica de prioridad:
      1. Si EXT_API_DEFAULT_LANGUAGES está configurado (no vacío) → usar ese valor
         como restricción sobre ENABLED_LANGUAGES.
      2. Si EXT_API_DEFAULT_LANGUAGES está vacío (por defecto) → usar ENABLED_LANGUAGES
         completo (todos los idiomas scrapeados).

    En ambos casos 'en' siempre es el primero.
    """
    override = getattr(cfg, "EXT_API_DEFAULT_LANGUAGES", "").strip()
    if override:
        # Restricción explícita — solo los idiomas del override que también
        # están en ENABLED_LANGUAGES (no exportar idiomas no scrapeados)
        scraped = set(_get_enabled_languages(cfg))
        requested = [ln.strip() for ln in override.split(",") if ln.strip()]
        filtered = [l for l in requested if l in scraped]
        if not filtered:
            # Override inválido — caer a ENABLED_LANGUAGES completo
            return _get_enabled_languages(cfg)
        # Garantizar 'en' primero
        if "en" not in filtered:
            filtered = ["en"] + filtered
        else:
            filtered = ["en"] + [l for l in filtered if l != "en"]
        return filtered
    # Sin override → todos los idiomas scrapeados
    return _get_enabled_languages(cfg)


def _parse_languages(languages_str: Optional[str], cfg) -> List[str]:
    """
    Resuelve los idiomas para una petición de exportación concreta.

    Prioridad:
      1. Si la petición especifica idiomas explícitamente → usar esos
         (permite al usuario seleccionar un subconjunto desde el panel UI).
      2. Si no → usar _get_export_languages(cfg), que a su vez usa
         ENABLED_LANGUAGES o EXT_API_DEFAULT_LANGUAGES según configuración.
    """
    if languages_str:
        langs = [ln.strip() for ln in languages_str.split(",") if ln.strip()]
        if langs:
            # Garantizar 'en' primero
            if "en" not in langs:
                langs = ["en"] + langs
                ...
                langs = ["en"] + [l for l in langs if l != "en"]
            return langs
    return _get_export_languages(cfg)
...
if __name__ == "__main__":
    ...
    # BUILD 127 — H-01: bind host/port now come from Settings (API_HOST/API_PORT).
    # Defaults remain 127.0.0.1:8000. Any non-loopback API_HOST is rejected at
    # Settings instantiation unless REQUIRE_API_KEY=True + a strong API_KEY is set.
    _cfg = get_settings()
    uvicorn.run(
        "app.main:app",
        host=_cfg.API_HOST,
