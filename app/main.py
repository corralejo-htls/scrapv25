...
# ---------------------------------------------------------------------------
# API Export endpoints (Build 88 — STRUCT-EXPORT-001)
# ---------------------------------------------------------------------------

@app.post(
    "/export/payload/{url_id}",
    tags=["Export"],
    summary="Generate API payload for a single hotel (dry run)",
    dependencies=[Depends(_check_api_key)],
)
def export_payload_preview(
    url_id: str,
    fields: Optional[str] = None,
    languages: Optional[str] = None,
) -> Dict[str, Any]:
    """
    Genera el payload filtrado para un hotel sin enviarlo a la API.

    Parámetros opcionales (query string):
    - **fields**: lista separada por comas de campos a incluir
      (ej. `name,address,scoreReview`). Por defecto: campos recomendados.
    - **languages**: lista separada por comas de idiomas
      (ej. `en,es,de`). 'en' siempre incluido. Por defecto: todos los disponibles.

    Útil para previsualizar y validar el payload antes de exportar.
    """
    from app.api_export_system import (
        APIConfig, APIExporter, APIField, ExportSelection, ExportTemplate,
    )
    try:
        uid = UUID(url_id)
    except ValueError:
        raise HTTPException(status_code=400, detail=f"Invalid url_id: {url_id}")

    # Parse optional filters
    field_list = None
    if fields:
        try:
            field_list = [APIField(f.strip()) for f in fields.split(",")]
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=f"Invalid field name: {exc}")

    lang_list = None
    if languages:
        lang_list = [ln.strip() for ln in languages.split(",")]

    template = ExportTemplate(
        name="preview",
        fields=field_list or APIField.recommended(),
        languages=lang_list or ["en", "es", "de", "fr", "it", "pt"],
    )
    selection = ExportSelection()
    selection.add_url_id(uid)

    with get_db() as session:
        exporter = APIExporter(session, APIConfig(), template, selection)
        result = exporter.export_single(uid, dry_run=True)
...
@app.post(
    "/export/batch",
    tags=["Export"],
    summary="Export all done hotels to API (or dry run)",
    dependencies=[Depends(_check_api_key)],
)
def export_batch_to_api(
    dry_run: bool = True,
    fields: Optional[str] = None,
    languages: Optional[str] = None,
    base_url: Optional[str] = None,
    api_key_ext: Optional[str] = None,
) -> Dict[str, Any]:
    """
    Exporta todos los hoteles con status=done a la API externa.

    Parámetros:
    - **dry_run**: `true` (defecto) — genera payloads sin enviar.
      `false` — envía a la API. Requiere `base_url` y `api_key_ext`.
    - **fields**: campos a incluir (separados por coma).
    - **languages**: idiomas a incluir (separados por coma, 'en' siempre presente).
    - **base_url**: URL base de la API destino (ej. `https://web.com/api`).
    - **api_key_ext**: clave de la API destino.
    """
    from app.api_export_system import (
        APIConfig, APIExporter, APIField, ExportSelection, ExportTemplate,
    )

    if not dry_run and (not base_url or not api_key_ext):
        raise HTTPException(
            status_code=400,
            detail="base_url y api_key_ext son obligatorios cuando dry_run=false",
        )
    ...
    config = APIConfig(
        base_url=base_url or "https://web.com/api",
        api_key=api_key_ext or "",
    )

    with get_db() as session:
        selection = ExportSelection.from_db_all_pending(session)
        if not selection.url_ids:
            return {"total": 0, "message": "No hotels with status=done found"}
        exporter = APIExporter(session, config, template, selection)
        result = exporter.export_batch(dry_run=dry_run)
