"""
BookingScraper/app/main.py  v2.2
...
CAMBIOS v2.1:
  ...
  [NEW] /vpn/connect — conecta VPN a un país específico
  [NEW] /scraping/test-url — prueba extracción en una URL concreta (diagnóstico)
...
CAMBIOS v2.2:
  [FIX CRITICO] /urls/load: acepta lista plana de URLs sin cabecera (formato del proyecto).
               csv.DictReader usaba la 1ª URL como nombre de columna → row.get("url")=None
  ...
  [FIX] /urls/load: respuesta añade campos 'format' y 'errors' para diagnóstico.
"""
                ...
                try:
                    result = db.execute(
                        text("""
                            ...
                            ON CONFLICT (url) DO NOTHING
                        """),
                        {"url": url, "lang": language, "pri": priority}
                    )
                    if result.rowcount > 0:
                        inserted += 1
                    else:
