"""
api_payload_builder.py — BookingScraper Pro v6.0.0 Build 98
============================================================

BUG-SVC-002-FIX (Build 98):
    _load_services(): cuando el extractor usa las estrategias DOM de
    fallback (strategy 1 / 1.5 / 2) en lugar de Apollo JSON primario,
    los grupos de amenidades de habitación (groupId=15 en el DOM de
    Booking.com) se incluyen incorrectamente en hotels_all_services
    junto con las instalaciones del hotel. Esto produce items como
    "Papel higiénico", "Secador de pelo", "TV de pantalla plana" en
    el array services[] de la API, que deben pertenecer a rooms[].

    Fix: añadir conjunto _ROOM_LEVEL_CATEGORIES con los nombres
    canónicos de groupId=15 en cada idioma. En _load_services(),
    filtrar filas cuyo service_category (en minúsculas) pertenezca
    a este conjunto antes de construir el payload services[].

    Idiomas cubiertos: EN, ES, DE, IT, PT.
    NOTA FR: "Équipements" (groupId=15 FR) colisiona con la etiqueta
    de instalaciones de hotel en FR ("Équipements" = hotel_services).
    Para evitar filtrar instalaciones legítimas de hotel en francés,
    FR queda excluido del filtro. Ver comentario en _ROOM_LEVEL_CATEGORIES.

    Impacto: services[] deja de contener amenidades de habitación
    cuando el DOM fallback es activado (Apollo JSON siempre correcto).
    Confirmado con _table__hotels_all_services__.csv filas ES/DE.

    Archivos modificados: app/api_payload_builder.py
...
class ApiPayloadBuilder:
    ...
    def _load_services(self, url_id: UUID) -> Dict[str, List[Dict]]:
        ...
        BUG-SVC-002-FIX (Build 98): filtra filas cuyo service_category
        corresponde a amenidades de habitación (groupId=15) antes de
        construir el payload. Estas filas se persisten correctamente
        en hotels_all_services cuando el extractor usa DOM fallback, pero
        no deben aparecer en services[] de la API (pertenecen a rooms[]).
        """
        rows = (
        ...
        skipped = 0
        room_filter = _lc().room_level_filter  # frozenset from languages.json
        for r in rows:
            # BUG-SVC-002-FIX: descartar amenidades de habitación (groupId=15)
            # El filtro proviene de languages.json → room_level_category_labels
            cat_lower = (r.service_category or "").strip().lower()
            if cat_lower in room_filter:
                skipped += 1
                continue
        ...
        if skipped:
            logger.debug(
                "BUG-SVC-002-FIX: %d room-level amenity rows filtered from services[] "
                "for url_id=%s",
                skipped, url_id,
            )
    ...
    def _build_category_scores(
        ...
        """
        lc = _lc()
        key_map = lc.category_key_map  # {text_lower: api_key} — de languages.json
        result: Dict[str, Dict[str, Any]] = {}
        for r in rows:
            raw_cat = (r.reviews_categories or "").strip().lower()
            api_key = key_map.get(raw_cat)
            ...
            if api_key is None:
                logger.warning(
                    ...
                    "Add to category_key_map in languages.json.",
                    r.reviews_categories, lang,
                )
                continue

            label = lc.get_category_label(api_key, lang) or r.reviews_categories
