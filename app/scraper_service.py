"""
scraper_service.py — BookingScraper Pro v6.0.0 build 61
========================================================
BUG-LANG-001-FIX (v61) — Missing Languages / Anti-bot detection:

  CAUSA RAÍZ: Booking.com detecta y bloquea la IP cuando 6 idiomas se
  procesan en sucesión inmediata sin delay entre ellos. Resultado: solo
  1-2 idiomas exitosos por URL, los demás retornan páginas Cloudflare
  de 3-5 KB que los engines registran como "All scraping engines failed".

  FIX-LANG-001-A: Inter-language delay en _process_url().
    Aplica LANG_SCRAPE_DELAY + jitter(0..LANG_SCRAPE_JITTER) segundos
    antes de cada idioma excepto el primero ('en'). Simula lectura humana
    entre páginas y evita el patrón de bot de requests consecutivos.

  FIX-LANG-001-B: Forced VPN rotation en _process_url().
    Cuando MAX_CONSECUTIVE_LANG_FAILURES fallos seguidos son detectados,
    se invoca vpn.rotate(force=True) para obtener nueva IP inmediatamente,
    sin esperar VPN_ROTATION_INTERVAL. El contador se reinicia después de
    cada éxito o después de cada rotación.

  FIX-LANG-001-C: Short HTML fast-fail en scraper.py (ver ese módulo).
    CloudScraper y Selenium retornan None de inmediato ante páginas < 5KB,
    evitando reintentos inútiles con la misma IP bloqueada.

  Parámetros nuevos (.env / config.py):
    LANG_SCRAPE_DELAY=10            (seg, base delay entre idiomas)
    LANG_SCRAPE_JITTER=5            (seg, jitter máximo aleatorio)
    MAX_CONSECUTIVE_LANG_FAILURES=2 (umral para forzar VPN)
    SHORT_HTML_THRESHOLD=5000       (bytes mínimos página válida)

Cambios v60 — BUG-DB-002-FIX (Silent Legal Persistence Failure):

  BUG-DB-002-FIX : _upsert_legal() — inserción incondicional en hotels_legal.

    SÍNTOMA: 4 URLs con 6/6 idiomas en hotels/hotels_description/hotels_policies
             pero 0 registros en hotels_legal. El fallo era completamente silencioso:
             sin log de WARNING, sin registro vacío, sin excepción visible.

    CAUSA:   Doble guard de silencio en la cadena de persistencia:
             1) En _scrape_language(): `if legal: self._upsert_legal(...)` omite la
                llamada si _extract_legal() devuelve None (sección no encontrada).
             2) En _upsert_legal(): `if not legal or not isinstance(legal, dict): return`
                segunda barrera que descarta None/vacío sin trazar ningún mensaje.
             Resultado: páginas sin bloque legal HTML (hoteles de BR/UY en Booking.com)
             quedan sin registro en hotels_legal — inconsistencia de datos silenciosa.

    FIX:     1) _scrape_language(): llamada a _upsert_legal() ahora es incondicional —
                se invoca siempre, pasando `legal or {}` (dict vacío si no hay sección).
             2) _upsert_legal(): el guard `if not legal` se convierte en lógica de
                has_legal_content para registrar la ausencia explícitamente.
             3) Nuevo campo `has_legal_content` (BOOLEAN) en HotelLegal y schema SQL:
                TRUE  → bloque legal encontrado con contenido real
                FALSE → página procesada, sección legal no presente en HTML
             4) Log WARNING cuando has_legal_content=False para facilitar auditoría.

  MODEL-002 : HotelLegal.has_legal_content — nuevo campo bool diagnóstico.

  SCHEMA-002: hotels_legal.has_legal_content — columna añadida en schema_v60.

  BUILD-SYNC-002: Todos los encabezados Python sincronizados a build 60.

Cambios v59 — (heredados):
  HIGH-2-LOG-001: _process_url() añade log WARNING cuando lang_results y
                  actual_count divergen.
  BUILD-SYNC-001: Encabezados sincronizados a build 59.
...
class ScraperService:
    ...
    def _process_url(self, url_obj: URLQueue) -> None:
        ...
        try:
            ...
            # BUG-LANG-001-FIX (v61): contadores para rotación VPN por fallos consecutivos.
            consecutive_failures: int = 0
            max_consec_failures: int = self._cfg.MAX_CONSECUTIVE_LANG_FAILURES

            for i, lang in enumerate(languages):

                # FIX-LANG-001-A: Inter-language delay — aplica a todos excepto el primero.
                # Simula comportamiento humano; evita detección de bot por Booking.com.
                # LANG_SCRAPE_DELAY (base) + random jitter(0..LANG_SCRAPE_JITTER).
                if i > 0:
                    base_delay  = self._cfg.LANG_SCRAPE_DELAY
                    jitter      = random.uniform(0.0, self._cfg.LANG_SCRAPE_JITTER)
                    total_delay = base_delay + jitter
                    logger.info(
                        "BUG-LANG-001-FIX: inter-language delay %.1fs (base=%.1f jitter=%.1f) "
                        "before %s/%s",
                        total_delay, base_delay, jitter, url_id_str, lang,
                    )
                    time.sleep(total_delay)

                # FIX-LANG-001-B: Forced VPN rotation ante fallos consecutivos.
                # Si MAX_CONSECUTIVE_LANG_FAILURES fallaron en fila, rotamos IP de inmediato
                # sin esperar VPN_ROTATION_INTERVAL. El delay extra de la rotación es aceptable
                # porque sin IP nueva los siguientes idiomas también fallarán.
                if consecutive_failures >= max_consec_failures and self._cfg.VPN_ENABLED:
                    logger.warning(
                        "BUG-LANG-001-FIX: %d consecutive failures for URL %s — "
                        "forcing VPN rotation before %s",
                        consecutive_failures, url_id_str, lang,
                    )
                    try:
                        rotated = self._vpn.rotate(force=True)
                        if rotated:
                            logger.info(
                                "BUG-LANG-001-FIX: VPN rotated (forced) — consecutive_failures reset. "
                                "Continuing with %s/%s",
                                url_id_str, lang,
                            )
                            consecutive_failures = 0  # Reset: nueva IP, nueva oportunidad
                        else:
                            logger.error(
                                "BUG-LANG-001-FIX: forced VPN rotation failed for %s — "
                                "continuing with same IP",
                                url_id_str,
                            )
                    except Exception as vpn_exc:
                        logger.warning(
                            "BUG-LANG-001-FIX: forced VPN rotation error for %s: %s",
                            url_id_str, vpn_exc,
                        )
                ...
                try:
                    ...
                    if ok:
                        ...
                        consecutive_failures = 0  # Éxito: resetear contador
                    else:
            ...
            # HIGH-2 (v59): detectar discrepancia entre intentos (lang_results) y
            # commits reales en DB. Un idioma puede retornar True en _scrape_language
            # pero fallar silenciosamente en el upsert — actual_count lo revela.
            expected_success_count = len(success_langs)
            if actual_count != expected_success_count:
                logger.warning(
                    "URL %s: DB count mismatch — lang_results says %d OK, "
                    "hotels table has %d. Possible silent upsert failure.",
                    url_id_str, expected_success_count, actual_count,
                )
    ...
    def _scrape_language(self, url_obj: URLQueue, lang: str) -> bool:
        ...
        # STRUCT-017 (v59 FIX-PH-STRUCTURE-001): Separar property_highlights —
        # ahora retorna List[Dict[str,str]] con pares category/detail.
        property_highlights: Optional[List[Dict[str, Any]]] = hotel_data.pop("property_highlights", None)
        ...
        try:
            ...
            # STRUCT-007 / BUG-DB-002-FIX (v60): Persistir información legal en
            # hotels_legal de forma INCONDICIONAL — siempre se inserta un registro,
            # aunque _extract_legal() haya devuelto None (sección no presente en HTML).
            # has_legal_content=False marca la ausencia de forma auditable.
            # ANTES (v59): `if legal: self._upsert_legal(...)` → silencio total si None.
            # AHORA (v60): llamada incondicional, `legal or {}` garantiza dict válido.
            self._upsert_legal(url_obj, lang, legal or {})
    ...
    def _upsert_legal(
        ...
        """
        STRUCT-007 / BUG-DB-002-FIX (v60): Persiste información legal en hotels_legal.

        Estrategia: upsert via UniqueConstraint (hotel_id, language).
        Un solo registro por hotel/idioma — si existe se actualiza.
        Campos: legal, legal_info, legal_details, has_legal_content.

        BUG-DB-002-FIX (v60):
          Comportamiento anterior (v59): si `legal` es None o vacío, return inmediato.
          Resultado: 4 URLs con hotels_legal=0 registros (hoteles BR/UY sin sección legal).
          Comportamiento nuevo (v60): siempre inserta un registro.
            has_legal_content=True  → sección legal encontrada con contenido real.
            has_legal_content=False → página procesada sin sección legal en HTML.
          El log WARNING facilita auditoría sin generar ruido en el flujo normal.
        """
        # BUG-DB-002-FIX (v60): determinar si hay contenido real de forma explícita.
        # Un dict vacío `{}` indica que _extract_legal() devolvió None (sección ausente).
        has_content: bool = bool(
            legal
            and isinstance(legal, dict)
            and any(legal.get(k) for k in ("legal", "legal_info", "legal_details"))
        )

        if not has_content:
            logger.warning(
                "BUG-DB-002-FIX: No legal section found for url_id=%s lang=%s "
                "— inserting empty record (has_legal_content=False)",
                url_obj.id, lang,
            )
        ...
        try:
            with get_db() as session:
                ...
                logger.debug(
                    "Legal upserted for hotel_id=%s lang=%s has_content=%s",
                    hotel.id, lang, has_content,
                )
    ...
    def _upsert_property_highlights(
        ...
        url_obj: URLQueue,
        lang: str,
        property_highlights: List[Dict[str, Any]],
    ) -> None:
        """
        FIX-PH-STRUCTURE-001 (v59): Persiste highlights de propiedad con
        estructura categoría/detalle. Un registro por par (category, detail)
        por hotel/idioma.

        CAMBIOS v59 respecto a v58:
          ANTES: recibía str (HTML blob), parseaba <li> y guardaba texto plano
                 en columna 'highlight'.
          AHORA: recibe List[Dict[str, str]] con claves 'category' y 'detail',
                 generado directamente por extractor._extract_property_highlights().
                 Persiste en columnas 'highlight_category' y 'highlight_detail'.
        ...
        Estrategia de upsert:
          ...
          ON CONFLICT via UniqueConstraint uq_hph_hotel_lang_cat_detail.

        Requiere que el Hotel ya exista en DB (llamar después de _upsert_hotel).
        """
        if not property_highlights:
            return

        # Filtrar entradas inválidas (sin category ni detail)
        valid_items = [
            item for item in property_highlights
            if isinstance(item, dict)
            and item.get("detail", "").strip()
        ]
        if not valid_items:
            logger.debug(
                "PropertyHighlights: no valid items to upsert for url_id=%s lang=%s",
                url_obj.id, lang,
            )
            return
        ...
        try:
            with get_db() as session:
                ...
                if not hotel:
                    logger.warning(
                        "Cannot upsert property_highlights — Hotel not found "
                        "for url_id=%s lang=%s",
                        url_obj.id, lang,
                    )
                    return
                ...
                for item in valid_items:
                    category = (item.get("category") or "").strip()[:256]
                    detail   = item["detail"].strip()[:512]
