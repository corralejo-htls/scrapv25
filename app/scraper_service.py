"""
BookingScraper/app/scraper_service.py  v3.0  [FIXED - American English + VPN US-first]
Servicio de scraping directo - BookingScraper Pro

CAMBIOS v2.6 [FIXES DE IDIOMA]:
  [FIX BUG #2] scrape_one(): queue_lang (url_queue.language) ahora se lee
    Y SE USA para logging y trazabilidad. Antes se leía pero se descartaba.
  [FIX BUG #7] _download_images(): eliminado Accept-Language "es-ES" hardcodeado.
    Las imagenes del CDN bstatic.com no tienen idioma; el header es irrelevante
    para el contenido pero era incorrecto para hoteles en otros idiomas.
...
def _download_images(url_id: int, img_urls: List[str], lang: str, driver=None):
    ...
    try:
        ...
        session.headers.update({
            ...
            # [FIX BUG #7] Eliminado "es-ES" hardcodeado. Las imágenes del CDN
            # bstatic.com no tienen idioma, por lo que Accept-Language no afecta
            # el contenido, pero es incorrecto enviar español para hoteles en otros idiomas.
