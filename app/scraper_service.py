"""
BookingScraper/app/scraper_service.py  v3.1  [FIXED - Block detection + Image download]
Servicio de scraping directo - BookingScraper Pro

CAMBIOS v3.1 [FIX IMAGEN + IDIOMA + DETECCIÓN BLOQUEO]:
  [FIX CRÍTICO #16] Descarga de imágenes del PRIMER idioma exitoso (no solo 'en').
    Antes: imágenes solo descargadas si lang == 'en'. Si el scraping en inglés fallaba
    (por falso positivo de _is_blocked), las imágenes NUNCA se descargaban para el hotel.
    Ahora: imágenes descargadas del primer idioma que retorne datos válidos, ya que el CDN
    bstatic.com sirve las mismas imágenes independientemente del idioma solicitado.
  [FIX #17] _download_images() ahora retorna int (count) para actualizar images_count en BD.
    Antes la función era void — images_count en hotels quedaba siempre en 0.
  [FIX #18] Verificación de idioma recibido vs. solicitado: si Booking.com devuelve
    contenido en idioma diferente al pedido (GeoIP VPN incorrecto), se loguea warning
    y se acumula en lang_mismatch_count. Al llegar a 3 mismatches → VPN rota.
  [FIX #19] _stats["lang_mismatch_count"] añadido al diccionario de estadísticas.

CAMBIOS v3.0 [AMERICAN ENGLISH + VPN US-FIRST]:
  [FIX #13] process_batch(): VPN conecta a US como primer país si está inactiva.
  Correlacionado con scraper.py v3.0 fix en LANG_COOKIE_LOCALE["en"] = "en-us".

CORRECCIONES v2.6: queue_lang trazabilidad + Accept-Language CDN.
CORRECCIONES v2.4: inglés siempre primero + un driver por hotel.
CORRECCIONES v2.3: _vpn_lock thread safety + max_workers=1.
CORRECCIONES v2.2: _save_hotel CAST jsonb + rollback en except.
CORRECCIONES v2.1: VPN integración + Selenium un driver por hotel.
"""
...
_stats = {
    ...
    "lang_mismatch_count": 0,     # [v3.1] idiomas incorrectos consecutivos (indica GeoIP VPN erróneo)
}
...
def scrape_one(url_id: int) -> Dict:
    ...
    try:
        ...
        # [v3.1] Flag: imágenes descargadas del PRIMER idioma exitoso (no solo de 'en').
        # Las imágenes del CDN bstatic.com son independientes del idioma del contenido.
        ...
        try:
            for lang in languages:
                ...
                try:
                    ...
                    # [v3.1] Verificar si el idioma devuelto coincide con el solicitado.
                    # Si no coincide, loguear advertencia pero guardar igualmente
                    # (el contenido puede ser útil aunque esté en idioma incorrecto).
                    detected = data.get("detected_lang")
                    if detected and detected != lang:
                        logger.warning(
                            f"  ⚠️ [{url_id}][{lang}] IDIOMA PÁGINA INCORRECTO: "
                            f"solicitado='{lang}', recibido='{detected}' "
                            f"(posible GeoIP VPN incorrecto)"
                        )
                        # Contabilizar como fallo de idioma VPN para rotación
                        with _stats_lock:
                            _stats["lang_mismatch_count"] = _stats.get("lang_mismatch_count", 0) + 1
                        # Si hay 3+ mismatches consecutivos → rotar VPN
                        if _stats.get("lang_mismatch_count", 0) >= 3:
                            logger.warning(
                                f"  🔄 [{url_id}] {_stats['lang_mismatch_count']} mismatches "
                                f"de idioma → rotando VPN para cambiar GeoIP..."
                            )
                            _maybe_rotate_vpn(force=True)
                            with _stats_lock:
                                _stats["lang_mismatch_count"] = 0
                    else:
                        # Idioma correcto → reset contador de mismatches
                        with _stats_lock:
                            _stats["lang_mismatch_count"] = 0
                    ...
                    # [FIX v3.1 CRÍTICO] Descargar imágenes del PRIMER idioma exitoso,
                    # no solo de lang == DEFAULT ('en'). Antes, si el inglés fallaba
                    # (por falso positivo en _is_blocked), no se descargaban imágenes
                    # para ningún idioma. Las imágenes de Booking.com CDN (bstatic.com)
                    # son idénticas para todos los idiomas — no dependen del idioma.
                    if not images_downloaded and settings.DOWNLOAD_IMAGES:
                        ...
                        if imgs:
                            ...
                            # [FIX v3.1] Actualizar images_count en BD tras descarga exitosa
                                ...
                                try:
                                    db.execute(
                                        text("""
                                            ...
                                            WHERE url_id = :url_id
                                        """),
                                        {"count": n_downloaded, "url_id": url_id}
                                    )
                                    db.commit()
                                    logger.debug(
                                        f"  📷 [{url_id}] images_count={n_downloaded} "
                                        f"actualizado en BD (idioma fuente: {lang})"
                                    )
                                except Exception as upd_err:
                                    logger.debug(f"  ⚠️ No se pudo actualizar images_count: {upd_err}")
                        images_downloaded = True
                        logger.debug(f"  📷 [{url_id}] Imagenes marcadas como descargadas (lang={lang})")
