"""
scraper_service.py — BookingScraper Pro v6.0.0 build 55
========================================================
Cambios v55:
...
class ScraperService:
    ...
    def _process_url(self, url_obj: URLQueue) -> None:
        ...
        try:
            ...
            # BUG-LANG-001 (v55): Ensure 'en' is always present and processed FIRST.
            # Rule: always scrape in 'en' language even if it is not in the
            # environment variable and always scrape 'en' language first,
            # then continue with the rest of the languages.
            if 'en' in languages:
                languages.remove('en')
    ...
    def _upsert_faqs(
        ...
        Estrategia: DELETE WHERE (hotel_id, language) + INSERT bulk.
        Una pregunta por fila (campo ask — TEXT).
            ...
            with get_db() as session:
                ...
                for question_text in faqs:
                    ...
                    session.add(HotelFAQ(
                        ...
                        url_id=url_obj.id,
                        language=lang,
                        ask=t,  # TEXT — sin truncar, máxima fidelidad
                    ))
    ...
    def _upsert_property_highlights(
        ...
        """
        STRUCT-017 (v53): Persiste el HTML sanitizado de Property Highlights.

        Estrategia: upsert via UniqueConstraint (url_id, language).
        Un solo registro por hotel/idioma. Si existe, se actualiza el campo highlights.

        El HTML ya ha sido sanitizado por extractor._extract_property_highlights():
          - SVG, img, picture, source, canvas, video eliminados.
        ...
        try:
            with get_db() as session:
                ...
                if existing:
                    existing.highlights = property_highlights
                    existing.hotel_id = hotel.id
                else:
                ...
                logger.debug(
                    "PropertyHighlights upserted for hotel_id=%s lang=%s len=%d",
                    hotel.id, lang, len(property_highlights),
                )
