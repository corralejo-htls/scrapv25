...
class ScraperService:
    ...
    def _persist_hotel_data(
        ...
        with get_db() as session:
            ...
            self._upsert_hotel_amenities(session, url_obj, hotel, lang, data)
    ...
    def _upsert_hotel_amenities(
        ...
        """Upsert HotelAmenity rows. BUG-PERSIST-001 (Build 63-fix): método faltante.
        amenities = List[str] — una fila por amenidad.
        """
        from app.models import HotelAmenity
        ...
        for amenity in amenities:
            ...
            exists = (
                ...
                .filter_by(hotel_id=hotel.id, language=lang, amenity=amenity)
                .first()
            )
            if not exists:
                session.add(HotelAmenity(
                    hotel_id=hotel.id, url_id=url_obj.id, language=lang,
                    amenity=amenity, created_at=now,
                ))
