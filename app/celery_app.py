...
celery_app.conf.update(
    ...
    # [FIX MED-40-004] Global defaults aligned with tasks.py per-task limits.
    # process_pending_urls overrides these via CELERY_TASK_SOFT_TIME_LIMIT /
    # CELERY_TASK_TIME_LIMIT env vars (default 150s/180s). These global values
    # apply to all OTHER tasks that do not declare their own limits.
    # cleanup_old_logs: 120s/180s (declared in task)
    # save_system_metrics: 30s/60s (declared in task)
    # reset_stale_urls: 30s/60s (declared in task)
    # create_next_partition: 30s/60s (declared in task)
    task_soft_time_limit     = 150,   # matches process_pending_urls default
    task_time_limit          = 180,   # matches process_pending_urls default
)
