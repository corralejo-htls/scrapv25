...
class Hotel(Base):
    ...
    # [FIX MED-40-001] Stale reference to install_clean_v32.sql removed.
    # cleanup_orphaned_hotels() is not yet implemented. Hotels with url_id=NULL
    # but url IS NOT NULL are preserved as searchable by URL.
    # When a url_queue row is deleted, url_id is set NULL via ON DELETE SET NULL.
...
class ScrapingLog(Base):
    ...
    PARTITIONED TABLE: scraping_logs uses PARTITION BY RANGE (timestamp).
    PostgreSQL does not allow FK constraints on partitioned tables when the
    referenced column is not the partition key.
    [FIX CRIT-40-001] ForeignKey removed from url_id. Referential integrity
    is enforced by the trigger trg_scraping_logs_fk_check defined in
    install_clean_v31.sql. This model must NEVER declare ForeignKey("url_queue.id")
    as that causes create_all() to fail with FeatureNotSupported.
    """
    __tablename__ = "scraping_logs"

    id       = Column(BigInteger, primary_key=True, autoincrement=True)
    # [FIX CRIT-40-001] No ForeignKey here — partitioned table constraint.
    # Integrity enforced by DB trigger trg_scraping_logs_fk_check.
    url_id   = Column(Integer, nullable=True, index=True)
