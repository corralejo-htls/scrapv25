"""
models.py — BookingScraper Pro v6.0.0 build 55
===============================================
Cambios estructurales v53:
  ...
  STRUCT-015 : HotelFAQ — nueva tabla v53.
               ...
               Campo: ask (TEXT). Una pregunta por fila.
...
class HotelFAQ(Base):
    """
    Preguntas frecuentes del hotel — una pregunta (ask) por fila.

    STRUCT-015 (v53): Extrae el bloque de preguntas frecuentes (FAQ)
    de la página de Booking.com. Solo se almacena el texto de la pregunta.
...
class HotelPropertyHighlights(Base):
    """
    Bloque "Property Highlights" del hotel como HTML sanitizado.

    STRUCT-017 (v53): Extrae el bloque de puntos destacados de la propiedad.
    Proceso de sanitización del HTML:
      - <svg> eliminados completamente (junto con todo su contenido interno).
    ...
    highlights: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True,
        comment="HTML sanitizado: SVG/img eliminados, todos los atributos HTML eliminados"
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=_utcnow
    )
    ...
    __table_args__ = (
        UniqueConstraint("url_id", "language", name="uq_hph_url_lang"),
