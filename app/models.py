"""
models.py — BookingScraper Pro v6.0.0 build 120
# Note: File last functionally modified at Build 97. Header updated to Build 120
# for documentation hygiene. ORM models verified against schema_v77_complete.sql.
===============================================
Cambios v60:
...
Fixes heredados:
  ...
  STRUCT-005 : HotelAmenity — ELIMINADA en build 65 (tabla hotels_amenities).
...
class Hotel(Base):
    ...
    v51 — cambios estructurales:
      - amenities  : ELIMINADO (build 65 — hotels_amenities suprimida del sistema)
    ...
    Campos schema.org / JSON-LD (NEW-COLS-001, v49):
      ...
      accommodation_type — JSON-LD @type (GAP-EXTRACT-001-FIX, v76 patch)
      price_range       — precio visible en disponibilidad (STRUCT-019, v76)
      rooms_quantity    — nº tipos de habitación (STRUCT-020, v76)
    """
    ...
    # STRUCT-005: amenities ELIMINADO — hotels_amenities suprimida en build 65
    # STRUCT-006: policies  ELIMINADO — ver tabla hotels_policies

    # STRUCT-019 (v76): precio de disponibilidad (NULL en páginas sin fechas)
    price_range: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True,
        comment="Precio visible en sección disponibilidad. NULL sin parámetros de fecha (STRUCT-019, v76)"
    )
    # STRUCT-020 (v76): cantidad de tipos de habitación en DOM
    rooms_quantity: Mapped[Optional[int]] = mapped_column(
        SmallInteger, nullable=True,
        comment="Nº tipos de habitación visibles (room-block). Fuente: JSON-LD numberOfRooms o count(room-block). (STRUCT-020, v76)"
    )
    # GAP-EXTRACT-001-FIX (v76 patch): tipo de alojamiento del JSON-LD @type.
    # Valores típicos: 'Hotel', 'Apartment', 'GuestHouse', 'Villa', 'LodgingBusiness'.
    accommodation_type: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True,
        comment="Tipo de alojamiento del JSON-LD @type (e.g. 'Hotel', 'Apartment'). GAP-EXTRACT-001-FIX"
    )
    # STRUCT-CITY-001 (Build 88) / STRUCT-CITY-002 (Build 89):
    # Metadatos urbanos de Booking.com.
    # BUG-ENV-REGEX-001-FIX (Build 89): regex corregidas para atnm_en y city_name
    #   (el HTML usa atnm_en/city_name sin prefijo b_, no b_atnm_en/b_city_name).
    city_name: Mapped[Optional[str]] = mapped_column(
        String(256), nullable=True,
        comment="Ciudad normalizada — JS city_name o breadcrumb DOM (STRUCT-CITY-001, Build 88; regex fix Build 89)"
    )
    dest_ufi: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True,
        comment="Booking.com UFI — JS b_ufi (STRUCT-CITY-001, Build 88)"
    )
    dest_id: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True, index=True,
        comment="Booking.com destination ID — JS context_dest_id (STRUCT-CITY-002, Build 89)"
    )
    atnm_en: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True,
        comment="Tipo alojamiento inglés — JS atnm_en (STRUCT-CITY-001 Build 88; regex fix Build 89)"
    )
    region_name: Mapped[Optional[str]] = mapped_column(
        String(256), nullable=True,
        comment="Región/Estado — breadcrumb items[3] no genérico (STRUCT-CITY-002, Build 89)"
    )
    district_name: Mapped[Optional[str]] = mapped_column(
        String(256), nullable=True,
        comment="Distrito — breadcrumb items[5] cuando n==7 no genérico (STRUCT-CITY-002, Build 89)"
    )
    ...
    __table_args__ = (
        ...
        # STRUCT-CITY-002 (Build 89)
        Index("ix_hotels_dest_id", "dest_id"),
        Index("ix_hotels_region_name", "region_name"),
        Index("ix_hotels_district_name", "district_name"),
        # STRUCT-005: ix_hotels_amenities_gin ELIMINADO — hotels_amenities suprimida en build 65
...
class HotelPopularService(Base):
    ...
    Selección editorial del proveedor (normalmente 8-12 ítems) de los servicios más destacados.
    (build 65: hotels_amenities eliminada — hotels_popular_services es la fuente canónica.)
...
class ImageDownload(Base):
    ...
    url: Mapped[str] = mapped_column(String(2048), nullable=False)
    name_jpg: Mapped[Optional[str]] = mapped_column(String(255), nullable=True,
        comment="Nombre de archivo extraído de la URL (basename sin query string), p.ej. '146312477.jpg'")
...
class ImageData(Base):
    ...
    # ── Build 109: clasificación de imágenes ──────────────────────────────
    # Distingue el subconjunto exportable a la API (gallery_visible=TRUE) del
    # resto del inventario interno (clasificado por subcategory para uso futuro).
    gallery_visible: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False,
        comment="Build 109: TRUE si la foto está en el modal de galería (exportable a la API)")
    source: Mapped[str] = mapped_column(
        String(16), nullable=False, default="js_array",
        comment="Build 109: origen — gallery_modal | js_array | dom_scan | unknown")
    subcategory: Mapped[str] = mapped_column(
        String(16), nullable=False, default="unknown",
        comment="Build 109: clase — gallery | room | facility | exterior | thumbnail | review_photo | unknown")
    gallery_order: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True,
        comment="Build 109: orden en el modal de galería (NULL si no es de galería)")
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "source IN ('gallery_modal','js_array','dom_scan','unknown')",
            name="chk_imgdata_source",
        ),
        CheckConstraint(
            "subcategory IN ('gallery','room','facility','exterior','thumbnail','review_photo','unknown')",
            name="chk_imgdata_subcategory",
        ),
        CheckConstraint("gallery_order IS NULL OR gallery_order >= 0", name="chk_imgdata_gallery_order"),
        Index("ix_imgdata_hotel_id", "hotel_id"),
        Index("ix_imgdata_gallery_visible", "hotel_id", "gallery_visible"),
        Index("ix_imgdata_subcategory", "hotel_id", "subcategory"),
    )

    def __repr__(self) -> str:
        return (
            f"<ImageData id_photo={self.id_photo} hotel_id={self.hotel_id} "
            f"gallery={self.gallery_visible} subcat={self.subcategory}>"
        )
...
class HotelAllService(Base):
    ...
    STRUCT-014 (v53): Lista completa de todos los servicios del hotel.
    completa de servicios desde la sección de instalaciones de Booking.com.
    (build 65: hotels_amenities eliminada — hotels_all_services es la fuente completa.)
    hotels_all_services captura los ítems completos de la lista expandida.

    BUILD-82-FIX (GAP-API-001): columna service_category añadida.
    La API destino espera servicios categorizados {service_category, service}.
    La categoría se extrae del heading del bloque facility-group de Booking.com.
    ...
    Estructura (como HotelPopularService — BIGSERIAL, muchas filas por hotel/lang):
      ...
      service_category — grupo al que pertenece (e.g. "Internet", "Parking"). NULL si no disponible.
    """
    ...
    # BUILD-82-FIX (GAP-API-001): grupo/categoría del servicio
    service_category: Mapped[Optional[str]] = mapped_column(
        String(128), nullable=True,
        comment="Categoría del grupo (e.g. 'Internet', 'Parking', 'Pool'). NULL si estrategia sin categoría."
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=_utcnow
    )
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"category={self.service_category!r} service={self.service!r}>"
        )
...
# ---------------------------------------------------------------------------
# HotelExtraInfo  (STRUCT-021 — nueva tabla v76)
# ---------------------------------------------------------------------------

class HotelExtraInfo(Base):
    """
    Información importante del alojamiento por idioma.

    STRUCT-021 (v76): Almacena el bloque "Good to know" / "Información importante"
    de Booking.com (data-testid="property-important-info"). Distinto del Fine Print
    (hotels_fine_print), que corresponde a la sección "Fine Print" / "Letra pequeña".

    Estructura: 1 registro por hotel/idioma.
      hotel_id   — FK a hotels.id
      url_id     — FK a url_queue.id
      language   — código ISO 639-1
      extra_info — texto del bloque de información importante
    """
    __tablename__ = "hotels_extra_info"
    ...
    extra_info: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True,
        comment="Texto del bloque property-important-info de Booking.com"
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=_utcnow
    )
    ...
    __table_args__ = (
        UniqueConstraint("url_id", "language", name="uq_hei_url_lang"),
        Index("ix_hei_hotel_id", "hotel_id"),
        Index("ix_hei_language", "language"),
    )

    def __repr__(self) -> str:
        return (
            f"<HotelExtraInfo hotel_id={self.hotel_id} lang={self.language} "
            f"len={len(self.extra_info or '')}>"
        )


# ---------------------------------------------------------------------------
# HotelNearbyPlace  (STRUCT-022 — nueva tabla v76)
# ---------------------------------------------------------------------------

class HotelNearbyPlace(Base):
    """
    Lugares de interés cercanos al hotel — una fila por lugar.

    STRUCT-022 (v76): Extrae el bloque location-highlight de Booking.com.
    Incluye nombre, distancia aproximada y categoría de ícono.

    BUILD-82-FIX (GAP-SCHEMA-003): columna category_code SMALLINT añadida.
    La API destino espera category como INTEGER (código numérico de ícono).
    La columna category (VARCHAR) se conserva para debugging y legibilidad.
    Mapa de conversión: 1=airport 2=restaurant 3=beach 4=transport 5=nature 6=attraction

    Estructura (BIGSERIAL, muchas filas por hotel/lang):
      hotel_id      — FK a hotels.id
      url_id        — FK a url_queue.id
      language      — código ISO 639-1
      place_name    — nombre del lugar de interés
      distance      — distancia textual (e.g. "2.1 km", "500 m")
      category      — categoría texto (e.g. "airport", "restaurant")
      category_code — código numérico API (1-6, NULL si no mapeado)
    """
    __tablename__ = "hotels_nearby_places"
    ...
    place_name: Mapped[str] = mapped_column(
        String(256), nullable=False,
        comment="Nombre del lugar cercano (e.g. 'Aeropuerto Internacional')"
    )
    distance: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True,
        comment="Distancia textual (e.g. '2.1 km', '500 m')"
    )
    category: Mapped[Optional[str]] = mapped_column(
        String(128), nullable=True,
        comment="Categoría texto de Booking.com (e.g. 'airport', 'restaurant')"
    )
    # BUILD-82-FIX (GAP-SCHEMA-003): código numérico de categoría para la API
    category_code: Mapped[Optional[int]] = mapped_column(
        SmallInteger, nullable=True,
        comment="Código numérico API: 1=airport 2=restaurant 3=beach 4=transport 5=nature 6=attraction"
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=_utcnow
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "hotel_id", "language", "place_name",
            name="uq_hnp_hotel_lang_place",
        ),
        Index("ix_hnp_hotel_id", "hotel_id"),
        Index("ix_hnp_language", "language"),
        Index("ix_hnp_place_name", "place_name"),
    )

    def __repr__(self) -> str:
        return (
            f"<HotelNearbyPlace hotel_id={self.hotel_id} lang={self.language} "
            f"place={self.place_name!r} dist={self.distance!r} code={self.category_code}>"
        )


# ---------------------------------------------------------------------------
# HotelRoomType  (STRUCT-023 — nueva tabla v76)
# ---------------------------------------------------------------------------

class HotelRoomType(Base):
    """
    Tipos de habitación normalizados — complemento a hotels.room_types (JSONB).

    STRUCT-023 (v76): Tabla normalizada que permite queries SQL directas sobre
    room_name, description y facilities. Complementa hotels.room_types (JSONB)
    que se mantiene para acceso rápido y compatibilidad.

    GAP-SCHEMA-002-FIX (v76 patch): Columnas adults, children, images e info
    añadidas para conformidad con el payload _API_.md rooms[].
    adults/children son NULL cuando SVGs no cargan (Selenium window.stop()).

    Estructura (BIGSERIAL, muchas filas por hotel/lang):
      hotel_id    — FK a hotels.id
      url_id      — FK a url_queue.id
      language    — código ISO 639-1
      room_name   — nombre del tipo de habitación
      description — descripción larga (puede ser vacía)
      facilities  — facilidades específicas (JSONB array de strings)
      adults      — ocupación máxima adultos (NULL si SVG no cargó)
      children    — ocupación máxima niños (NULL si SVG no cargó)
      images      — URLs de imágenes de la habitación (JSONB array)
      info        — información adicional de la habitación (nullable)
    """
    __tablename__ = "hotels_room_types"
    ...
    room_name: Mapped[str] = mapped_column(
        String(256), nullable=False,
        comment="Nombre del tipo de habitación (e.g. 'Deluxe Double Room')"
    )
    description: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True,
        comment="Descripción larga del tipo de habitación"
    )
    facilities: Mapped[Optional[List]] = mapped_column(
        JSONB, nullable=True, default=list,
        comment="Facilidades de la habitación como array JSON (e.g. ['WiFi gratis', 'TV'])"
    )
    # GAP-SCHEMA-002-FIX (v76 patch): campos requeridos por _API_.md rooms[]
    adults: Mapped[Optional[int]] = mapped_column(
        SmallInteger, nullable=True,
        comment="Ocupación máxima adultos. NULL si SVGs no cargaron (Selenium window.stop())"
    )
    children: Mapped[Optional[int]] = mapped_column(
        SmallInteger, nullable=True,
        comment="Ocupación máxima niños. NULL si SVGs no cargaron"
    )
    images: Mapped[Optional[List]] = mapped_column(
        JSONB, nullable=True, default=list,
        comment="URLs de imágenes de la habitación (JSONB array de strings)"
    )
    info: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True,
        comment="Información adicional de la habitación (nullable en _API_.md)"
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=_utcnow
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "hotel_id", "language", "room_name",
            name="uq_hrt_hotel_lang_name",
        ),
        Index("ix_hrt_hotel_id", "hotel_id"),
        Index("ix_hrt_language", "language"),
        Index("ix_hrt_room_name", "room_name"),
    )

    def __repr__(self) -> str:
        return (
            f"<HotelRoomType hotel_id={self.hotel_id} lang={self.language} "
            f"room={self.room_name!r}>"
        )


# ---------------------------------------------------------------------------
# HotelSEO  (STRUCT-024 — nueva tabla v76)
# ---------------------------------------------------------------------------

class HotelSEO(Base):
    """
    Meta tags SEO del hotel por idioma.

    STRUCT-024 (v76): Extrae <meta name="description"> y <meta name="keywords">
    del <head> de cada página de Booking.com en cada idioma.

    Estructura: 1 registro por hotel/idioma.
      hotel_id        — FK a hotels.id
      url_id          — FK a url_queue.id
      language        — código ISO 639-1
      seo_description — content de meta[name=description] o og:description
      keywords        — content de meta[name=keywords] o og:keywords
    """
    __tablename__ = "hotels_seo"
    ...
    seo_description: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True,
        comment="Contenido de meta[name=description] o meta[property=og:description]"
    )
    keywords: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True,
        comment="Contenido de meta[name=keywords] o meta[property=og:keywords]"
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=_utcnow
    )
    ...
    __table_args__ = (
        UniqueConstraint("url_id", "language", name="uq_hseo_url_lang"),
        Index("ix_hseo_hotel_id", "hotel_id"),
        Index("ix_hseo_language", "language"),
    )

    def __repr__(self) -> str:
        return (
            f"<HotelSEO hotel_id={self.hotel_id} lang={self.language} "
            f"seo_desc_len={len(self.seo_description or '')}>"
        )
...
# BUG-IMPORT-004 (Build 63-fix):
#   scraper_service.py:475 imports 'HotelPolicies' (plural) inside
#   _upsert_hotel_policies().  The ORM class is defined as 'HotelPolicy'
#   (singular).  All 13 URLs × 6 languages failed at the DB write step with:
#     ImportError: cannot import name 'HotelPolicies' from 'app.models'
#   Adding this alias resolves the ImportError and unblocks persistence for
#   every language of every URL already scraped.
HotelPolicies = HotelPolicy


# ---------------------------------------------------------------------------
# HotelIndividualReview  (STRUCT-025 — nueva tabla v76 patch 2026-04-04)
# ---------------------------------------------------------------------------

class HotelIndividualReview(Base):
    """
    Reseñas individuales de huéspedes — distinto de HotelGuestReview.

    GAP-SCHEMA-001-FIX (v76 patch):
      HotelGuestReview almacena PUNTUACIONES POR CATEGORÍA (Limpieza: 9.3).
      Esta tabla almacena RESEÑAS TEXTUALES INDIVIDUALES con nombre del
      revisor, título, comentario positivo/negativo, país y booking_id.
      Cubre el campo reviews[] del payload _API_.md.
    """
    __tablename__ = "hotels_individual_reviews"
    ...
    score: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True,
        comment="Puntuación individual de la reseña (0-10)"
    )
    title: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    positive_comment: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True,
        comment="Texto positivo — campo 'Liked' de Booking.com"
    )
    negative_comment: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True,
        comment="Texto negativo — campo 'Disliked' de Booking.com"
    )
    reviewer_country: Mapped[Optional[str]] = mapped_column(String(128), nullable=True)
    booking_id: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=_utcnow
    )
    ...
    __table_args__ = (
        ...
        Index("ix_hir_hotel_id", "hotel_id"),
        Index("ix_hir_language", "language"),
        Index("ix_hir_score", "score"),
    )

    def __repr__(self) -> str:
        return (
            f"<HotelIndividualReview hotel_id={self.hotel_id} lang={self.language} "
            f"reviewer={self.reviewer_name!r} score={self.score}>"
        )
