"""
models.py — BookingScraper Pro v6.0.0 build 56
===============================================
...
# por ítem de highlight, no un bloque HTML por hotel/idioma.
# ---------------------------------------------------------------------------

class HotelPropertyHighlights(Base):
    """
    Highlights de la propiedad normalizados: UN registro por highlight por hotel/idioma.

    BUG-PH-NORMALIZATION-001 FIX (v56): Estructura rediseñada de 1 registro
    HTML por hotel/idioma a N registros de texto plano por hotel/idioma.

    Motivación:
      - Permite filtrar hoteles por highlight específico sin parsear HTML.
      - Permite indexar y buscar con GIN en columna de texto.
      - Coherente con el patrón de hotels_popular_services (BIGSERIAL pk).
      - Elimina dependencia de clases CSS volátiles de Booking.com.

    Estructura: 1 fila por highlight por hotel/idioma:
      hotel_id  — FK a hotels.id
      url_id    — FK a url_queue.id
      language  — código ISO 639-1
      highlight — texto del highlight (e.g. "Free WiFi", "Swimming pool")
    """
    ...
    highlight: Mapped[str] = mapped_column(
        String(512), nullable=False,
        comment="Texto del highlight individual (e.g. 'Free WiFi', 'Swimming pool')"
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=_utcnow
    )
    ...
    __table_args__ = (
        ...
        UniqueConstraint("hotel_id", "language", "highlight", name="uq_hph_hotel_lang_highlight"),
