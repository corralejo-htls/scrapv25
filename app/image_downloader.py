...
class ImageDownloader:
    ...
    def _download_one(
        ...
        try:
            ...
            # [FIX HIGH-009] Cap image download size BEFORE loading into memory.
            # Content-Length check first (fast path), then streaming cap.
            # Prevents memory exhaustion from maliciously large or corrupt images.
            _MAX_IMAGE_BYTES = 20 * 1024 * 1024  # 20 MB hard cap
            content_length = resp.headers.get("Content-Length")
            if content_length and int(content_length) > _MAX_IMAGE_BYTES:
                logger.warning(
                    "[HIGH-009] Image size %s bytes exceeds cap %s — skipped: %s",
                    content_length, _MAX_IMAGE_BYTES, url[:80],
                )
                return None

            # Stream with per-chunk size cap
            chunks = []
            total  = 0
            for chunk in resp.iter_content(chunk_size=_CHUNK_SIZE):
                if chunk:
                    ...
                    if total > _MAX_IMAGE_BYTES:
                        logger.warning(
                            "[HIGH-009] Streaming image exceeded %s bytes cap — "
                            "download aborted: %s", _MAX_IMAGE_BYTES, url[:80],
                        )
                        return None
                    chunks.append(chunk)
