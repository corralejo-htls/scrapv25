"""
image_downloader.py — BookingScraper Pro Build 108
Windows 11 compatible image downloader.

Fixes applied:
  BUG-IMG-NAME-001 (Build 108): nuevo campo name_jpg en image_downloads.
    Se extrae el basename de la URL (sin query string) y se persiste en
    cada fila de descarga, tanto en éxito como en error. Helper:
    _name_jpg_from_url(). Idéntico para las 3 tallas de la misma foto.

  BUG-IMG-CAP-002-FIX (Build 106): removed hard task_cap=900.
    Previous cap silently dropped photos for hotels with >300 images.
    All extracted photos are now downloaded without a count ceiling.
...
def _name_jpg_from_url(url: Optional[str]) -> Optional[str]:
    """
    BUG-IMG-NAME-001 (Build 108): extrae el nombre de archivo de imagen de una
    URL de Booking.com / bstatic.com, descartando el query string (k=, o=, hp=).

    Ejemplos:
      .../hotel/max1024x768/146312477.jpg?k=...&o=&hp=1  -> '146312477.jpg'
      .../hotel/max200/146312477.jpg?k=...&o=&hp=1       -> '146312477.jpg'

    El nombre es idéntico para las 3 tallas (max200/max1024x768/max1280x900)
    de la misma foto, ya que solo cambia el segmento de tamaño en la ruta.

    Independiente de plataforma: usa posixpath (no os.path), porque las URLs
    siempre usan '/' y os.path.basename trataría '\\' como separador en Windows.
    Devuelve None ante URL vacía/None o cualquier error de parseo.
    """
    if not url:
        return None
    try:
        path = urlparse(url).path           # descarta ?k=...&o=...&hp=1
        base = posixpath.basename(unquote(path)).strip()
        return base or None
    except Exception:  # noqa: BLE001 — nunca debe romper la descarga
        return None
...
class ImageDownloader:
    ...
    def download_photo_batch(
        ...
        # BUG-IMG-CAP-002-FIX (Build 106): removed hard cap on download tasks.
        # Previous cap of 900 (300 photos × 3 sizes) could silently drop photos
        # for large hotels.  extract_hotel_photos_from_html() already extracts
        # all available photos without a count limit (BUG-PHOTO-LIMIT-001-FIX,
        # Build 90).  Capping the download tasks contradicted this design.
        # Dynamic timeout scales proportionally with actual batch size.
            ...
            with get_db() as session:
                for photo in photos:
                    ...
                    if existing:
                        ...
                        # Build 109: clasificación. La galería tiene prioridad:
                        # una vez marcada gallery_visible, no se degrada.
                        if photo.get("gallery_visible"):
                            existing.gallery_visible = True
                            existing.source = photo.get("source") or existing.source
                            existing.subcategory = photo.get("subcategory") or "gallery"
                            if photo.get("gallery_order") is not None:
                                existing.gallery_order = photo["gallery_order"]
                        elif not existing.gallery_visible:
                            # Solo refina la subcategoría si aún no es de galería.
                            if photo.get("source"):
                                existing.source = photo["source"]
                            if photo.get("subcategory") and existing.subcategory == "unknown":
                                existing.subcategory = photo["subcategory"]
                    else:
                        ...
                        session.add(ImageData(
                            ...
                            # Build 109: clasificación (defaults seguros si ausente).
                            gallery_visible=bool(photo.get("gallery_visible", False)),
                            source=photo.get("source") or "js_array",
                            subcategory=photo.get("subcategory") or "unknown",
                            gallery_order=photo.get("gallery_order"),
                        ))
        except Exception as exc:
        ...
        try:
            ...
            with get_db() as session:
                ...
                if existing:
                    ...
                    if not existing.name_jpg:
                        existing.name_jpg = _name_jpg_from_url(url)
                else:
                    session.add(ImageDownload(
                        ...
                        category=category,
                        url=url,
                        name_jpg=_name_jpg_from_url(url),
            ...
            with get_db() as session:
                ...
                if row:
                    ...
                    if not row.name_jpg:
                        row.name_jpg = _name_jpg_from_url(url)
                else:
                    session.add(ImageDownload(
                        hotel_id=hotel_id,
                        id_photo=id_photo,
