# app/config.py — BookingScraper Pro v6.0.0 Build 127
# BUILD 127 — Forensic audit Build 126: H-01 fail-closed bind guard + API_HOST/
#   API_PORT fields; M-01 model_validator rejects REQUIRE_API_KEY=True with
#   empty/weak API_KEY; canonical fallback bumped to 127.
# BUILD 126 — Auditoría forense Build 125 (audit_scrapv25_build125_EN.md):
#   FINDING-A-FIX (CRITICAL, tasks.py + scraper_service.py): poison-pill / el
#       watchdog os._exit(1) saltaba _finalize_url() y retry_count nunca subía →
#       max_retries inalcanzable. Ahora retry_count se incrementa en el CLAIM
#       (dispatch_batch), reset_stale marca 'error' permanente al agotar el techo,
#       y el SELECT de pending excluye retry_count>=max_retries.
#   FINDING-C-FIX (scraper_service.py/config): SCRAPER_MAX_WORKERS default 2→1
#       (el navegador ya está serializado; el 2º hilo solo producía carreras).
#   FINDING-B-FIX (scraper_service.py): galería EN con 0 gallery_visible y badge>0
#       marca el idioma 'incomplete' (no 'done') → fuera de export + reprocesable.
#   FINDING-D-FIX (scraper.py): User-Agent dinámico vía CDP (fin de la recurrencia
#       de BUG-USER-AGENT-STALE-001 en cada auto-update de Brave).
#   FINDING-E-FIX (schema_v77_complete.sql): cabecera build 120→126.
# BUILD 124 — BUG-EXTRAINFO-SELECTOR-001-FIX (extractor.py): Strategy 5+6 en
#              _extract_extra_info(). BUG-FAQ-FP-001-FIX (extractor.py): denylist
#              de falsos positivos en _extract_faqs(). BUG-VPN-LOCK-DEADLOCK-001-FIX
#              (vpn_manager_windows.py): acquire(timeout) en rotate().
#              Nuevo campo: VPN_LOCK_ACQUIRE_TIMEOUT_S (default 80).
# BUILD 123 — BUG-GALLERY-OPEN-ALL-LANGS-001-FIX (scraper.py): _open_gallery()
#              llamado incondicionalmente para los 6 idiomas disparaba detección
#              bot Cloudflare en cada pase no-EN. 92/121 eventos de challenge en
#              sesión 2026-06-12 eran de idiomas no-EN. Fix: movido al bloque EN.
#              BUG-USER-AGENT-STALE-001-FIX (scraper.py): UA Chrome/131 desajustado
#              con ChromeDriver 149.0.7827.115 en producción. Actualizado a Chrome/149.
#              BUG-NEARBY-CATMAP-001-FIX (extractor.py): _CATEGORY_MAP tenía 5 gaps
#              para DE e IT: "Flughäfen","Verkehrsmittel","Naturschönheiten","Aeroporti",
#              "Trasporti" mapeaban incorrectamente a "attraction". Verificado contra
#              9 HTMLs de pruebas/ (análisis DOM 2026-06-13). Fix: regex ampliados.
#              BUG-NEARBY-SKELETON-001-FIX (extractor.py): warning genérico distinguido
#              en 3 causas raíz: skeleton React, sin datos, sin sección.
# BUILD 122 — BUG-WATCHDOG-CANCEL-001-FIX (tasks.py) + BUG-GALLERY-2X-001-FIX (image_classifier.py)
#              Ver BUG_REPORT_Build122_EN.md / BUG_REPORT_Build122_ES.md para detalles.
...
# ---------------------------------------------------------------------------
# FINDING-004-FIX (Build 125): BUILD_VERSION canónico — fuente única
# ---------------------------------------------------------------------------
# BUG-CONFIG-SYNC-001 (Build 112) recurrió en Build 124: config.py mantenía un
# literal default (123) que divergía de BUILD_VERSION en app/__init__.py (124),
# de modo que Settings().BUILD_VERSION reportaba 123 sin variable de entorno.
# Solución de raíz: importar el literal en lugar de duplicarlo. app/__init__.py
# no importa nada (es puro), por lo que esta importación NO crea ciclo: cuando
# Python ejecuta app/config.py ya ha ejecutado app/__init__.py.
try:
    from app import BUILD_VERSION as _CANONICAL_BUILD_VERSION
except Exception:  # pragma: no cover — fallback defensivo si el paquete cambia
    _CANONICAL_BUILD_VERSION = 127
...
class Settings(BaseSettings):
    ...
    BUILD_VERSION: int = Field(
        default_factory=lambda: _CANONICAL_BUILD_VERSION,
        description=(
            "Incremental build number. "
            "FINDING-004-FIX (Build 125): el default ya NO es un literal duplicado; "
            "se importa de app/__init__.py (_CANONICAL_BUILD_VERSION) para eliminar "
            "de raíz la recurrencia de BUG-CONFIG-SYNC-001 (la deriva 123 vs 124). "
            "Sigue siendo sobre-escribible vía variable de entorno BUILD_VERSION."
        ),
    )
    ...
    # BUILD 127 — H-01/M-01 (forensic audit Build 126). Bind host is now
    # config-driven so the fail-closed startup guard can detect exposure to a
    # routable interface. Defaults preserve the historic localhost-only posture.
    API_HOST: str = Field(
        default="127.0.0.1",
        description="Interface the FastAPI/uvicorn server binds to. "
                    "Non-loopback values require REQUIRE_API_KEY=True + a strong API_KEY.",
    )
    API_PORT: int = Field(
        default=8000, ge=1, le=65535,
        description="TCP port the FastAPI/uvicorn server binds to.",
    )
    ...
    SCRAPER_MAX_WORKERS: int = Field(
        ...
        description="Number of parallel URL workers (ThreadPoolExecutor). "
                    "FINDING-C-FIX (Build 126): default lowered 2→1. The "
                    "SeleniumEngine uses a single threading.Lock(), so browser "
                    "operations are ALREADY serialized — a 2nd worker yields zero "
                    "throughput benefit while NordVPNManager.rotate()/reset_browser() "
                    "and the per-language loop are shared mutable state touched by "
                    "both threads (observed: duplicate 'Browser reset complete' pairs, "
                    "one thread resetting the browser mid-scrape of the other, and a "
                    "watchdog kill from a poison-pill thread discarding the concurrent "
                    "legitimate URL). Keep at 1 unless each worker gets its own "
                    "SeleniumEngine in a separate Celery process. Effective browser "
                    "concurrency = 1 URL at a time regardless of this value.",
    )

    # ── Build 120 (BUG-DISPATCH-001-FIX) — Batch size decoupled from worker count ──
    # Root cause: dispatch_batch() used .limit(SCRAPER_MAX_WORKERS) = 2 URLs per
    # Beat tick. With LOCK_TTL=280s < typical batch duration (~490s for 2 URLs),
    # the Redis lock expired before the batch completed, allowing uncontrolled
    # batch overlaps. Decoupling batch size from worker count enables operators to
    # tune throughput independently of concurrency.
    # Default=2 preserves backward compatibility. Increase to 10-20 for faster
    # processing of large queues (update LOCK_TTL_S accordingly).
    SCRAPER_BATCH_SIZE: int = Field(
        default=2, ge=1, le=500,
        description=(
            "BUG-DISPATCH-001-FIX (Build 120): URLs fetched per dispatch_batch() call. "
            "Previously hardcoded to SCRAPER_MAX_WORKERS (2). Decoupled so operators "
            "can process more URLs per Beat tick without changing thread count. "
            "Default=2 (backward compat). Recommended: 10-20 for queues >50 URLs. "
            "Ensure TASK_WATCHDOG_TIMEOUT_S > SCRAPER_BATCH_SIZE × 250s per URL."
        ),
    )
    ...
    VPN_LOCK_ACQUIRE_TIMEOUT_S: int = Field(
        default=80, ge=10, le=280,
        description=(
            "BUG-VPN-LOCK-DEADLOCK-001-FIX (Build 124): Timeout en segundos para "
            "adquirir self._lock dentro de rotate(). Sustituye el 'with self._lock:' "
            "original (bloqueo indefinido) por lock.acquire(timeout=N). Si el lock "
            "no se puede adquirir en N segundos (porque un daemon thread zombi de una "
            "rotación previa lo sigue sosteniendo tras expirar VPN_ROTATE_TIMEOUT_S), "
            "rotate() retorna False inmediatamente, desbloqueando el ciclo de scraping. "
            "Debe ser < VPN_ROTATE_TIMEOUT_S para que el wrapper externo detecte "
            "el fallo antes de que expire su propio join(). Default=80s < 90s."
        ),
    )
    ...
    BROWSER_COMMAND_TIMEOUT_S: int = Field(
        default=120, ge=15, le=600,
        description=(
            "FINDING-001-FIX (Build 125): Timeout de lectura a NIVEL DE COMANDO "
            "del executor remoto de Selenium (command_executor.set_timeout). "
            "set_page_load_timeout()/NAVIGATION_TIMEOUT_S solo cubren driver.get(); "
            "NO cubren find_element/find_elements, click(), execute_script(), "
            "current_url ni la interacción del modal de galería. En Windows 11, si "
            "el pipe IPC ChromeDriver↔Brave se atasca (confirmado por "
            "RemoteDisconnected en logs), cualquiera de esos comandos bloquea la "
            "lectura del socket sin límite, manteniendo la tarea viva hasta que el "
            "watchdog dispara os._exit(1) a los TASK_WATCHDOG_TIMEOUT_S (600s). "
            "Este timeout hace que el comando lance una excepción y la lengua "
            "falle limpiamente para reintentarse. Default 120s."
        ),
    )
    ...
    # ── BUILD 127 — fail-closed security validator ───────────────────────────
    # Forensic audit Build 126: H-01 (default-open posture) + M-01 (empty-key
    # bypass) + supports L-01. Enforced once at Settings instantiation (startup).
    #   M-01: REQUIRE_API_KEY=True with an empty/weak API_KEY would let a bare
    #         "Authorization: Bearer " header pass (auth[7:] == "" == API_KEY).
    #   H-01: binding to a non-loopback interface without enforced auth exposes
    #         every endpoint (incl. POST /export/send, POST /export/config,
    #         DELETE /urls/{id}). We refuse to start in that configuration.
    MIN_API_KEY_LENGTH: int = Field(
        default=16, ge=1,
        description="Minimum accepted API_KEY length when REQUIRE_API_KEY=True.",
    )

    @model_validator(mode="after")
    def _validate_api_security(self) -> "Settings":
        loopback = {"127.0.0.1", "::1", "localhost"}
        host = (self.API_HOST or "").strip().lower()
        key = self.API_KEY or ""
        # M-01 — auth requested but key missing/weak -> fail closed at startup.
        if self.REQUIRE_API_KEY and len(key) < self.MIN_API_KEY_LENGTH:
            raise ValueError(
                "REQUIRE_API_KEY=True but API_KEY is empty or shorter than "
                f"{self.MIN_API_KEY_LENGTH} chars. Set a strong API_KEY "
                "(e.g. python -c \"import secrets; print(secrets.token_hex(32))\")."
            )
        # H-01 — exposing a routable interface without enforced auth is refused.
        if host not in loopback and not (
            self.REQUIRE_API_KEY and len(key) >= self.MIN_API_KEY_LENGTH
        ):
            raise ValueError(
                f"API_HOST={self.API_HOST!r} is not a loopback interface. "
                "Binding to a routable interface requires REQUIRE_API_KEY=True "
                "and a strong API_KEY. Refusing to start in an open posture."
            )
        return self
