"""
config.py — BookingScraper Pro v6.0.0 build 61
================================================
BUG-LANG-001-FIX (v61): Cuatro nuevos parámetros de configuración para mitigar
  detección anti-bot de Booking.com en scraping multi-idioma:
    LANG_SCRAPE_DELAY           — delay base entre idiomas (seg, default 10)
    LANG_SCRAPE_JITTER          — jitter aleatorio sobre el delay (seg, default 5)
    MAX_CONSECUTIVE_LANG_FAILURES — fallos consecutivos antes de forzar VPN (default 2)
    SHORT_HTML_THRESHOLD        — bytes mínimos para página válida (default 5000)

BUILD-SYNC-002: Encabezado sincronizado a build 60. Sin cambios funcionales.
...
class Settings(BaseSettings):
    ...
    # ── BUG-LANG-001 Fix: Anti-bot detection mitigation ──────────────────────
    # Delay base (segundos) entre idiomas consecutivos para el mismo URL.
    # Simula comportamiento humano; reduce detección de bot por Booking.com.
    LANG_SCRAPE_DELAY: float = Field(default=10.0, ge=0.0, le=120.0)
    # Jitter aleatorio (0..N segundos) sumado al delay base para romper patrones.
    LANG_SCRAPE_JITTER: float = Field(default=5.0, ge=0.0, le=30.0)
    # Número de fallos consecutivos de idioma antes de forzar rotación de VPN.
    MAX_CONSECUTIVE_LANG_FAILURES: int = Field(default=2, ge=1, le=6)
    # Umbral en bytes para detectar páginas de bloqueo (Cloudflare challenge).
    # Páginas reales de Booking.com miden 150K-500K bytes.
    SHORT_HTML_THRESHOLD: int = Field(default=5000, ge=1000, le=20000)
