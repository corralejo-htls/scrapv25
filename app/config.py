...
# [FIX HIGH-40-002] Resolve .env relative to this file, not the CWD.
# Fixes Task Scheduler / shortcut launch where CWD != project root.
_THIS_DIR  = os.path.dirname(os.path.abspath(__file__))   # .../app/
_REPO_ROOT = os.path.dirname(_THIS_DIR)                    # .../BookingScraper/
...
class Settings(BaseSettings):
    ...
    # [FIX CRIT-40-003] ClassVar[Set[str]] — prevents Pydantic v2 from treating
    # this as a model field. In Pydantic v2 BaseSettings, any class attribute
    # without ClassVar annotation is interpreted as a field, causing
    # PydanticUserError for unsupported types like `set`.
    ...
    # ── RUTAS ─────────────────────────────────────────────────────────────────
    # [FIX HIGH-40-002 / FIX BUG-A-11] All paths are config-file-relative.
    # _THIS_DIR and _REPO_ROOT are computed at module level (above the class)
    # so they are stable regardless of CWD and can be referenced as defaults.
    # [FIX LOW-40-004] Using module-level constants (defined above class) so
    # the defaults are correct even when the class is instantiated in tests
    # without reimporting the module.
    _DEFAULT_DATA: ClassVar[str] = os.path.join(_REPO_ROOT, "data")

    BASE_DATA_PATH: str = os.path.join(_REPO_ROOT, "data")
    IMAGES_PATH:    str = os.path.join(_REPO_ROOT, "data", "images")
    EXPORTS_PATH:   str = os.path.join(_REPO_ROOT, "data", "exports")
    LOGS_PATH:      str = os.path.join(_REPO_ROOT, "data", "logs")
    ...
    model_config = SettingsConfigDict(
        # [FIX HIGH-40-002] Absolute path so .env is found regardless of CWD.
        env_file=_DOTENV_PATH,
        env_file_encoding="utf-8",
        case_sensitive=True,
        # [FIX MED-40-007] "forbid" raises ValidationError immediately on
        # unrecognised env var names (e.g. DB_PASWORD typo), preventing silent
        # fallback to default values that are later caught as cryptic errors.
        extra="forbid",
    )
