...
TABLA REQUERIDA:
  url_language_status — definida en install_clean_v31.sql (Section 4, tabla 3).
  [FIX MED-40-002] Referencias anteriores a migration_v2_url_language_status.sql
  y bookingscraper_schema_v6.sql eran incorrectas — esos archivos no existen.
  El esquema completo está en install_clean_v31.sql (único archivo de instalación).
"""
...
class CompletenessService:
    ...
    def initialize_url_processing(self, url_id: int, db=None) -> None:
        ...
        try:
            # [FIX HIGH-40-004] Use validated integer literal, not f-string.
            # SET LOCAL does not support parameterized binding (:param).
            # Clamp to reasonable range; str() of int is injection-safe.
            _lock_timeout_ms = max(100, min(int(os.getenv("COMPLETENESS_LOCK_TIMEOUT_MS", "5000")), 60000))
            db.execute(text("SET LOCAL lock_timeout = " + str(_lock_timeout_ms)))
