"""
completeness_service.py — BookingScraper Pro v6.0.0 Build 120
Note: This file was last functionally modified at Build 48.
      The header is updated to Build 120 for documentation hygiene.
      No functional changes between Build 48 and 120 were required.
      Verified: state machine (_VALID_TRANSITIONS) matches schema_v77_complete.sql
      CHECK constraint on url_language_status.status:
        {pending, processing, done, error, skipped, incomplete} ✓
      Verified: update_language_status() uses SELECT FOR UPDATE ✓ (SCRAP-BUG-023)
      Verified: invalid state transitions rejected ✓ (SCRAP-BUG-034)

Fixes applied (original Build 48):
