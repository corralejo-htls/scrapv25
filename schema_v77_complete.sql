-- =============================================================================
-- BookingScraper Pro — Schema Completo v6.0.0 build 127
-- Build 127: VERSION-RECONCILE-001 — Cabecera del schema sincronizada con el
--            BUILD_VERSION canónico del código (app/__init__.py BUILD_VERSION = 127).
--            El schema declaraba "build 126" mientras app/__init__.py, app/config.py,
--            app/tasks.py y app/main.py ya estaban en 127 (desfase de 1 build).
--            SIN cambios estructurales: el nombre de archivo (versión de estructura
--            schema_v77) y TODAS las tablas, vistas, índices, triggers y constraints
--            permanecen byte-idénticos. El CHECK de status de url_queue y de
--            url_language_status ya admite 'incomplete', por lo que la corrección de
--            lógica de aplicación FINDING-D (app/scraper_service.py: finalización
--            'incomplete' + guarda de reconciliación) opera sobre el schema existente
--            sin requerir ningún cambio DDL. Solo se modificó esta cabecera.
-- Build 126: FINDING-E-FIX — Cabecera de versión sincronizada con el código
--            (app/__init__.py BUILD_VERSION=126). SIN cambios estructurales: las
--            correcciones del Build 126 (FINDING-A/B/C/D) son de lógica de
--            aplicación (tasks.py, scraper_service.py, scraper.py, config.py);
--            no requieren tocar tablas, vistas ni constraints. url_queue ya
--            expone retry_count/max_retries y el CHECK de status ya admite
--            'error' e 'incomplete', por lo que los fixes operan sobre el schema
--            existente sin riesgo en la recreación de la BD.
-- Build 120: BUG-SCHEMA-COUNT-001-FIX — Comentario de cabecera corregido de
--            "22 total" a "21 total". La tabla hotels_individual_reviews fue
--            añadida en Build 77 elevando el conteo a 21 tablas de usuario.
--            El comentario nunca se actualizó y siempre indicó 22 (incorrecto).
--            Impacto: solo cosmético. No hay cambio estructural.
-- Build 120: BUG-PARTITION-002-FIX — 12 particiones para 2029 añadidas
--            (2029-01 a 2029-12). Sin estas, los logs del año 2029 caerían en
--            la partición DEFAULT perdiendo el beneficio de partition pruning.
--            La tarea Beat ensure_log_partitions crea particiones months_ahead=2,
--            por lo que las particiones de 2029 serán creadas automáticamente
--            en producción cuando llegue noviembre/diciembre 2028. Sin embargo,
--            incluirlas en el schema garantiza cobertura desde el arranque.
-- Build 120: GAP-VIEW-001-FIX — v_scraping_summary ahora incluye columna
--            is_complete (BOOLEAN). TRUE cuando languages_done = languages_tracked
--            y languages_tracked > 0. FALSE en cualquier otro caso (parcial,
--            error, sin datos). Alinea la vista con el contrato documentado
--            en readme.md y en la respuesta del endpoint /scraping/status.
-- Build 120: BUG-SCHEMA-VALIDATE-001-FIX — Comentario de validación corregido:
--            "22 tablas" → "21 tablas" (línea 1521). El conteo esperado "21 filas"
--            en la siguiente línea era correcto; el encabezado era inconsistente.
...
--            para retornar exactamente 1 fila por foto con mejor URL disponible
--            (highres>large>thumb). Elimina conteos inflados (foto×3 tallas).
...
--     Phase 1 ("Great for your stay") ELIMINADA. Solo Phase 2.
--     _FACILITY_GROUP_MAP, _SERVICE_CATEGORY_RULES eliminados.
--     languages.json: room_level_category_labels, category_key_map,
--       facility_group_map, service_category_rules eliminados.
--     service_category verbatim desde DOM de Booking.com.
--     Sin cambios estructurales en el schema SQL.
-- v77 (Build 99):
...
--                        (incluye is_complete BOOLEAN desde Build 120)
--
-- NOTA WINDOWS 11:
...
-- BUG-PARTITION-002-FIX (Build 120): 12 particiones para 2029 añadidas.
-- ensure_log_partitions (Beat task, daily 00:05, months_ahead=2) las creará
-- automáticamente en producción ca. noviembre 2028, pero incluirlas en el
-- schema garantiza cobertura completa desde el primer arranque.
CREATE TABLE IF NOT EXISTS scraping_logs_2029_01 PARTITION OF scraping_logs FOR VALUES FROM ('2029-01-01') TO ('2029-02-01');
CREATE TABLE IF NOT EXISTS scraping_logs_2029_02 PARTITION OF scraping_logs FOR VALUES FROM ('2029-02-01') TO ('2029-03-01');
CREATE TABLE IF NOT EXISTS scraping_logs_2029_03 PARTITION OF scraping_logs FOR VALUES FROM ('2029-03-01') TO ('2029-04-01');
CREATE TABLE IF NOT EXISTS scraping_logs_2029_04 PARTITION OF scraping_logs FOR VALUES FROM ('2029-04-01') TO ('2029-05-01');
CREATE TABLE IF NOT EXISTS scraping_logs_2029_05 PARTITION OF scraping_logs FOR VALUES FROM ('2029-05-01') TO ('2029-06-01');
CREATE TABLE IF NOT EXISTS scraping_logs_2029_06 PARTITION OF scraping_logs FOR VALUES FROM ('2029-06-01') TO ('2029-07-01');
CREATE TABLE IF NOT EXISTS scraping_logs_2029_07 PARTITION OF scraping_logs FOR VALUES FROM ('2029-07-01') TO ('2029-08-01');
CREATE TABLE IF NOT EXISTS scraping_logs_2029_08 PARTITION OF scraping_logs FOR VALUES FROM ('2029-08-01') TO ('2029-09-01');
CREATE TABLE IF NOT EXISTS scraping_logs_2029_09 PARTITION OF scraping_logs FOR VALUES FROM ('2029-09-01') TO ('2029-10-01');
CREATE TABLE IF NOT EXISTS scraping_logs_2029_10 PARTITION OF scraping_logs FOR VALUES FROM ('2029-10-01') TO ('2029-11-01');
CREATE TABLE IF NOT EXISTS scraping_logs_2029_11 PARTITION OF scraping_logs FOR VALUES FROM ('2029-11-01') TO ('2029-12-01');
CREATE TABLE IF NOT EXISTS scraping_logs_2029_12 PARTITION OF scraping_logs FOR VALUES FROM ('2029-12-01') TO ('2030-01-01');
...
COMMENT ON COLUMN image_downloads.name_jpg  IS 'Nombre de archivo extraído de la URL (basename sin query string), p.ej. ''146312477.jpg''. Idéntico para las 3 tallas (max200/max1024x768/max1280x900) de la misma foto. Build 108: BUG-IMG-NAME-001.';
...
CREATE TABLE IF NOT EXISTS image_data (
    ...
    -- =====================================================================
    -- Build 109 — Sistema de clasificación de imágenes
    -- BUG-IMG-CLASS-001: el pipeline captura un SUPERCONJUNTO (todo id_photo
    --   con token k= en el HTML); la galería pública (modal) es un subconjunto.
    --   Estas columnas distinguen lo exportable a la API (gallery_visible=TRUE)
    --   del resto del inventario, que se conserva clasificado para uso futuro.
    -- NOTA: la BD se recrea SIEMPRE desde este archivo (NUNCA migración), por
    --   lo que las columnas se declaran aquí directamente.
    -- =====================================================================
    gallery_visible  BOOLEAN     NOT NULL DEFAULT FALSE,
    source           VARCHAR(16) NOT NULL DEFAULT 'js_array',
    subcategory      VARCHAR(16) NOT NULL DEFAULT 'unknown',
    ...
    CONSTRAINT chk_imgdata_source CHECK (
        source IN ('gallery_modal','js_array','dom_scan','unknown')
    ),
    CONSTRAINT chk_imgdata_subcategory CHECK (
        subcategory IN ('gallery','room','facility','exterior','thumbnail','review_photo','unknown')
    ),
    CONSTRAINT chk_imgdata_gallery_order CHECK (gallery_order IS NULL OR gallery_order >= 0)
);

CREATE INDEX IF NOT EXISTS ix_imgdata_hotel_id        ON image_data (hotel_id);
-- Build 109: índices para filtrado de galería y por subcategoría.
CREATE INDEX IF NOT EXISTS ix_imgdata_gallery_visible ON image_data (hotel_id, gallery_visible);
CREATE INDEX IF NOT EXISTS ix_imgdata_subcategory     ON image_data (hotel_id, subcategory);
...
COMMENT ON COLUMN image_data.gallery_visible IS 'Build 109: TRUE si la foto está en el modal de galería (subconjunto exportable a la API).';
COMMENT ON COLUMN image_data.source          IS 'Build 109: origen de captura — gallery_modal | js_array | dom_scan | unknown.';
COMMENT ON COLUMN image_data.subcategory     IS 'Build 109: clase de foto — gallery | room | facility | exterior | thumbnail | review_photo | unknown.';
COMMENT ON COLUMN image_data.gallery_order   IS 'Build 109: orden de aparición en el modal (NULL si no es de galería).';

-- =============================================================================
-- VISTA v_api_export_images (Build 109 — corregida Build 112 BUG-VIEW-DEDUP-001)
-- Une image_data (clasificación) con image_downloads (URLs descargadas) y
-- expone SOLO las fotos visibles en galería, en orden, para el campo images[]
-- del payload de la API.
--
-- BUG-VIEW-DEDUP-001-FIX (Build 112):
--   La vista original hacía un JOIN simple sin DISTINCT ON, devolviendo hasta
--   3 filas por id_photo (una por cada variante: thumb_url/large_url/highres_url).
--   El comentario decía "prioriza highres > large > thumb" pero la SQL no lo
--   implementaba — el conteo resultante era foto_count × 3. La deduplicación
--   ocurría en Python (_load_gallery_image_urls) pero la vista directa daba
--   conteos inflados (p. ej. 59 fotos → 177 filas).
--   Fix: subconsulta con DISTINCT ON (hotel_id, id_photo) ordenada por prioridad
--   de talla (highres=1, large=2, thumb=3), envuelta en ORDER BY gallery_order.
--   Resultado: exactamente 1 fila por foto con la mejor URL disponible.
-- =============================================================================
...
COMMENT ON VIEW v_api_export_images IS
    'Build 109 (fix Build 112 BUG-VIEW-DEDUP-001): 1 fila por foto de galería con la mejor URL (highres>large>thumb), ordenadas por gallery_order. Fuente para images[] de la API. El DISTINCT ON garantiza conteo exacto igual al modal público.';
...
SELECT
    ...
    -- GAP-VIEW-001-FIX (Build 120): is_complete = TRUE cuando todos los idiomas
    -- registrados tienen status='done' y al menos uno existe.
    -- FALSE cuando: ningún idioma registrado, alguno pendiente/error/procesando.
    (COUNT(uls.id) > 0 AND
...
COMMENT ON VIEW v_scraping_summary IS
    'Resumen de completitud de scraping por URL. is_complete=TRUE cuando languages_done=languages_tracked>0.';
...
/*
-- 1. Verificar que existen exactamente 21 tablas de usuario (v77: 21 tablas confirmadas)
--    BUG-SCHEMA-COUNT-001-FIX (Build 120): corregido de "22" a "21".
SELECT table_name
...
-- =============================================================================
-- FIN DEL SCHEMA v77  (Build 120 — última actualización: 2026-06-11)
-- BUG-SCHEMA-COUNT-001-FIX  : 22→21 tablas en comentario de cabecera
-- BUG-PARTITION-002-FIX     : 12 particiones 2029 añadidas
-- GAP-VIEW-001-FIX           : v_scraping_summary.is_complete añadida
-- BUG-SCHEMA-VALIDATE-001-FIX: comentario validación 22→21 tablas corregido
-- =============================================================================
