-- =============================================================================
-- BookingScraper Pro — Schema Completo v6.0.0 build 54
-- PostgreSQL 14+ / Windows 11 Single-Node Deployment
-- =============================================================================
--
-- INSTRUCCIONES DE EJECUCIÓN (Windows 11):
--   psql -U postgres -f schema_v54_complete.sql
--   O desde pgAdmin 4: Tools > Query Tool > Ejecutar este archivo
--
-- CAMBIOS v54 respecto a v53:
--
--   FIX-LEGAL-002 : Corrección mejorada de datos en hotels_legal para ES.
--                   Maneja el caso donde legal = legal_info (duplicación 100%).
--                   La condición anterior solo detectaba legal NULL/vacío.
--                   Ahora detecta y corrige ambos patrones.
--
--   PARTITION-2027: Particiones mensuales 2027 añadidas a scraping_logs.
--                   Las particiones 2025-2026 se mantienen sin cambios.
--
--   BUG-EXTR-010 : Documentado: los selectores regex de _extract_guest_reviews()
--                   en extractor.py no coinciden con data-testid="review-subscore".
--                   Requiere fix en extractor.py (no en SQL).
--
...
COMMENT ON TABLE  url_queue IS 'Cola de URLs a scrapeear con estado y prioridad (v54)';
...
COMMENT ON TABLE  hotels IS 'Datos principales del hotel por idioma (v54)';
...
-- =============================================================================
-- CORRECCIÓN DE DATOS: hotels_legal ES (FIX-LEGAL-002, v54)
-- =============================================================================
-- Mejora de FIX-LEGAL-001 (v52): ahora también detecta y corrige el caso donde
-- legal = legal_info (duplicación 100%), además del caso original donde legal
-- es NULL/vacío. BUG confirmado en auditoría de 2026-03-23.

DO $$
DECLARE
    v_updated INTEGER := 0;
BEGIN
    -- Caso 1 (FIX-LEGAL-001 original): legal es NULL/vacío, legal_info tiene el título
    IF EXISTS (
        SELECT 1
        FROM hotels_legal
        WHERE language = 'es'
        ...
        RAISE NOTICE 'FIX-LEGAL-002 caso 1: Corrigiendo registros ES con título en legal_info...';
        UPDATE hotels_legal
        SET
        ...
        RAISE NOTICE 'FIX-LEGAL-002 caso 1: % registros corregidos.', v_updated;
    ELSE
        RAISE NOTICE 'FIX-LEGAL-002 caso 1: No se encontraron registros — omitido.';
    END IF;

    -- Caso 2 (NUEVO v54): legal = legal_info (ambos campos idénticos)
    -- El extractor asignó el título del bloque legal a ambos campos en ES.
    -- Corrección: legal conserva el valor, legal_info se limpia.
    v_updated := 0;
    IF EXISTS (
        SELECT 1
        FROM hotels_legal
        WHERE language = 'es'
          AND legal IS NOT NULL
          AND legal_info IS NOT NULL
          AND TRIM(legal) = TRIM(legal_info)
          AND LENGTH(TRIM(legal)) > 0
    ) THEN
        RAISE NOTICE 'FIX-LEGAL-002 caso 2: Corrigiendo registros ES con legal = legal_info...';
        UPDATE hotels_legal
        SET
            legal_info    = '',
            legal_details = ''
        WHERE language = 'es'
          ...
          AND LENGTH(TRIM(legal)) > 0;
        GET DIAGNOSTICS v_updated = ROW_COUNT;
        RAISE NOTICE 'FIX-LEGAL-002 caso 2: % registros corregidos (legal_info limpiada).', v_updated;
    ELSE
        RAISE NOTICE 'FIX-LEGAL-002 caso 2: No se encontraron registros duplicados — omitido.';
    END IF;
END $$;
...
-- v54: FIX-LEGAL-002 + PARTITION-2027 aplicados
