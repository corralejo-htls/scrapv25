...
REM BookingScraper Pro v6.0.0 Build 116 | Windows 11
...
REM
REM BUG-ECHO-BAT-001-FIX (Build 116):
REM   ECHO. falla con "No se esperaba ." en CMD.EXE de Windows 11 tanto en
REM   lineas directas como dentro de strings cmd /k pasados a start.
REM   Fix 1: ECHO. standalone → ECHO( en todo el script.
REM   Fix 2: doble comilla cierre ("") al final de la linea del Worker → (" ).
...
ECHO   [Celery] Worker iniciando (con reinicio automatico - Build 115)...
REM BUG-WORKER-NORESTART-001-FIX (Build 115/116): Se llama start_celery.bat en lugar
REM de inline el comando python. start_celery.bat incluye bucle de reinicio
REM automatico si el worker muere con exit code != 0 (watchdog kill).
start "BSP - Celery Worker" cmd /k "cd /d "%BASE%" && call "%VENV_ACTIVATE%" && SET PYTHONPATH=%BASE% && SET PYTHONUNBUFFERED=1 && SET FORKED_BY_MULTIPROCESSING=1 && ECHO( && call "%BASE%\start_celery.bat"
