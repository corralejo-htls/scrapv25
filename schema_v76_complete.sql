-- =============================================================================
-- BookingScraper Pro — Schema Completo v6.0.0 build 76
...
COMMENT ON TABLE  hotels_nearby_places IS
    'Lugares de interés cercanos — bloque location-highlight de Booking.com (STRUCT-022, v76)';
...
COMMENT ON COLUMN hotels_nearby_places.category IS
    'Categoría del ícono de Booking.com (e.g. "airport", "restaurant", "beach")';
...
COMMENT ON TABLE  hotels_room_types IS
    'Tipos de habitación normalizados — complemento a hotels.room_types JSONB (STRUCT-023, v76)';
COMMENT ON COLUMN hotels_room_types.room_name IS
    'Nombre del tipo de habitación (e.g. "Deluxe Double Room", "Superior Suite")';
COMMENT ON COLUMN hotels_room_types.facilities IS
    'Lista JSON de facilidades de la habitación (e.g. ["WiFi gratis", "TV"])';
...
SELECT
    ...
    -- price_range y rooms_quantity (STRUCT-019/020, v76)
    h.price_range,
    h.rooms_quantity,
...
COMMENT ON VIEW v_hotels_full IS
    ...
    'popular_services, policies, guest_reviews, legal, extra_info, seo, nearby_places, room_types_detail — v76';
...
/*
-- 1. Verificar que existen exactamente 21 tablas (v76: +5 tablas nuevas)
SELECT table_name
...
ORDER BY table_name;
-- Esperado: 20 filas (scraping_logs es particionada, cuenta separado)
