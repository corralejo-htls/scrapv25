-- =============================================================================
-- BookingScraper Pro — Schema Completo v6.0.0 build 58
-- PostgreSQL 14+ / Windows 11 Single-Node Deployment
-- =============================================================================
--
-- INSTRUCCIONES DE EJECUCIÓN (Windows 11):
--   psql -U postgres -f schema_v57_complete.sql
--   O desde pgAdmin 4: Tools > Query Tool > Ejecutar este archivo
--
-- CAMBIOS v58 respecto a v57:
--
--   BUG-INTEGRITY-001 : scraper_service.py — _process_url() corregido.
--                       ANTES: self._mark_done(url_obj, all_ok=True)  ← hardcoded.
--                       AHORA: Strategy E — decisión basada en COUNT(*) de hotels.
--                       3 casos: completo → done | parcial → error+preservar |
--                       total fallo → cleanup + error.
--                       Ref: Strategy_E_Implementation_Guide_EN.md
--
--   STRATEGY-E-001    : url_queue — 2 columnas nuevas:
--                         languages_completed VARCHAR(64) DEFAULT ''
--                         languages_failed    VARCHAR(64) DEFAULT ''
--                       Permite diagnóstico rápido sin JOIN a url_language_status.
--
--   STRATEGY-E-002    : url_queue.status CHECK constraint ampliado:
--                       AÑADE 'incomplete' al set de estados válidos.
--                       ANTES: pending|processing|done|error|skipped
--                       AHORA: pending|processing|done|error|skipped|incomplete
--
--   STRATEGY-E-003    : url_language_status ya tenía 'incomplete' en su CHECK.
--                       Sin cambios en esa tabla.
--
--   BUILD-SYNC-001    : Comentarios internos y COMMENT ON TABLE sincronizados a b58.
--
...
--                          Vista v_hotels_full actualizada: highlights como array de texto.
--
...
CREATE TABLE IF NOT EXISTS hotels_property_highlights (
    ...
    url_id      UUID        NOT NULL,
    language    VARCHAR(10) NOT NULL,
    -- highlight: texto plano de un único punto destacado (e.g. "Free WiFi", "Swimming pool")
    ...
    CONSTRAINT fk_hph_url   FOREIGN KEY (url_id)
        REFERENCES url_queue (id) ON DELETE CASCADE ON UPDATE CASCADE,
    -- Un registro por combinación hotel/idioma/highlight
...
COMMENT ON TABLE  hotels_property_highlights IS
    'Highlights de propiedad normalizados: 1 registro por highlight por hotel/idioma (STRUCT-017, v57 fix)';
COMMENT ON COLUMN hotels_property_highlights.highlight IS
    'Texto plano del highlight individual extraído del DOM de Booking.com';
...
SELECT
    ...
    hfp.fp,
    -- Property highlights normalizados como array (STRUCT-017, v57 fix)
    COALESCE(
...
COMMENT ON VIEW v_hotels_full IS
    'Vista denormalizada de hotel con description, fine_print, highlights (array), amenities, '
